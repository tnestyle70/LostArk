	...
	const bool dimensionMasterGroundTargetOnly)
{
	...
	}
	{
		...
		const std::vector<std::string> sequenceSteps{
			"PATTERNSEQUENCESTEP\tENCOUNTER_VALTAN\tsequence.valtan.server-authored.v1\t0\tVALTAN_ENTRANCE_CINEMATIC_IDLE",
		...
		const bool madeDuplicateSequenceStep = replaceBootstrapRow(
			...
			"sequence.valtan.server-authored.v1\t1\tVALTAN_ENTRANCE_CINEMATIC_IDLE");
	...
	}
	{
		...
		const bool reactiveTopologyExact = nullptr != patterns &&
			34u == valtanLivePatternCount && 130u == valtanStageCount &&
			24u == valtanStageActionCount &&
			140u == valtanRuntimeBranchCount && 5u == valtanMotionCount &&
		...
		tests.Require(reactiveTopologyExact,
			"Load all 130 current Valtan stages with the exact 24 actions, 140 runtime branches, five motions, and terminal counter slam");
		tests.Require(
...

	{
		...
		const bool heldOrderedStepWhileDead =
			pausedOrder == std::vector<std::string>{ "VALTAN_ENTRANCE_CINEMATIC_IDLE" } &&
		...
		const bool emptyRoomFailedClosed =
			disconnectedOrder ==
				std::vector<std::string>{ "VALTAN_ENTRANCE_CINEMATIC_IDLE" } &&
