    ...
    const std::vector<SERVER_WORLD_ENTITY>* stagedBosses)
{
	...
	const bool ownsRun = m_KoukuSaydonPatternAudition.iRoomAuditionEpoch != 0u;
	CPacketWriter shape;
	...
	if (continueRaid && (!ownsRun || running || restart ||
		(request.eOperation != OP::PLAY_SELECTED && request.eOperation != OP::PLAY_BUNDLE) ||
		!Is_KoukuRaidRunning() || m_KoukuRaid.iOwnerSessionId != sessionId ||
		m_KoukuRaid.iAuditionEpoch != m_KoukuSaydonPatternAudition.iRoomAuditionEpoch ||
		m_KoukuSaydonPatternAudition.iOwnerSessionId != sessionId ||
		m_KoukuRaid.State.strGateId != request.Scope.strGateId ||
		m_KoukuSaydonPatternAudition.Request.Scope.strGateId != request.Scope.strGateId ||
		m_KoukuSaydonPatternAudition.PinnedGameplayRevision != request.Scope.ExpectedGameplayRevision ||
	...
	staged.iRoomAuditionEpoch = continueRaid ? m_KoukuSaydonPatternAudition.iRoomAuditionEpoch : m_iNextKoukuSaydonPatternAuditionEpoch;
	...
	for (const auto& sourceMember : requestedMembers)
	{
		...
		for (std::size_t index = 0; index < toCheck.size(); ++index)
		{
			...
			// Encore has the same MN_RPCT_05 body/staff. Reuse the exact saved
			// local attack; gate layouts, player modes and camera scenes keep their owner.
			const bool encoreReplay = !bundleRequest &&
				placement->strPlacementId == "boss.kakulsaydon.bingo.saydon" &&
				boss->strArchetypeId == "BOSS_KAKULSAYDON_BINGO_SAYDON" &&
				pattern->strGateId == "GATE3" && pattern->strTargetBossPlacementId == "boss.kakulsaydon.g3.saydon" &&
				std::find(pattern->AuditionBossArchetypeIds.begin(), pattern->AuditionBossArchetypeIds.end(),
					"BOSS_KAKULSAYDON_G3_SAYDON") != pattern->AuditionBossArchetypeIds.end() &&
				pattern->WorldSequences.empty() && pattern->SceneProfiles.empty() &&
				pattern->MechanicTriggers.empty() && pattern->LogicWindows.empty() && !pattern->BossMotion &&
				(request.Scope.strGateId.empty() || request.Scope.strGateId == "BINGO");
			if (!encoreReplay && (!admitsBody || (bundleRequest && (pattern->strGateId != request.Scope.strGateId || pattern->strTargetBossPlacementId != placement->strPlacementId)) ||
			...
			for (const auto& window : pattern->LogicWindows)
			{
				...
				for (const auto* outcomes : {&window.OnSuccess, &window.OnFail, &window.OnTimeout}) for (const auto& result : *outcomes)
				{
					...
					if (result.eKind == BOSS_PATTERN_LOGIC_RESULT_KIND::FOLLOWUP_PATTERN ||
						(result.eKind == BOSS_PATTERN_LOGIC_RESULT_KIND::MARIO_ENTER && !result.strPatternId.empty())) toCheck.push_back(result.strPatternId);
				}
			}
	...
	if (request.iMarioTestStartStage > 4u || (!request.iMarioTestStartStage && request.iMarioTestSeed) ||
		(request.iMarioTestStartStage && request.eOperation != KOUKUSAYDON_PATTERN_AUDITION_OPERATION::PLAY_SELECTED))
		return reject(RESULT::REJECTED_UNSUPPORTED_PATTERN, "Mario test requires selected entry stage 1..4 and a bounded seed");
	if (request.iMarioTestStartStage)
	{
		const auto* entry = findPattern(request.strPatternId);
		if (!entry || staged.Members.size() != 1u || !CKoukuSaydonLogicRuntime::Has_MarioEntry(*entry) ||
	...
	if (continueRaid)
	{
		...
		staged.Tails = std::move(m_KoukuSaydonPatternAudition.Tails);
		staged.WorldPlays = std::move(m_KoukuSaydonPatternAudition.WorldPlays);
		staged.SupportSchedule = std::move(m_KoukuSaydonPatternAudition.SupportSchedule);
	}
	else
	{
		if (ownsRun) Clear_KoukuSaydonPatternAudition();
		Cancel_KoukuWorldBodies(); // An explicitly accepted run replaces persistent encounter objects.
	}
		...
		C2S_DEBUG_KOUKUSAYDON_PATTERN_AUDITION_REQUEST& request)
{
	...
	if (Is_KoukuRaidRunning())
	{
		...
		result.eResult = LostArk::Shared::KOUKUSAYDON_PATTERN_AUDITION_RESULT::REJECTED_BUSY;
		result.PinnedGameplayRevision = m_KoukuRaid.State.PinnedGameplayRevision; result.iPinnedSourceRevision = m_KoukuRaid.State.iActionSourceRevision;
		result.strReason = "Complete Play owns this room; stop its raid epoch before auditioning";
	}
	else
...
LostArk::Server::CGameRoom::Find_KoukuAuditionMember(const LostArk::Shared::NET_ENTITY_ID bossId, const std::uint32_t patternSequence)
{
	for (auto& member : m_KoukuSaydonPatternAudition.Members)
		if (member.iBossEntityId == bossId && (!patternSequence || member.iPatternSequence == patternSequence) &&
			(!patternSequence || member.LogicLedger.Is_Active())) return &member;
	if (patternSequence)
		for (auto& tail : m_KoukuSaydonPatternAudition.Tails)
			if (tail.pOwner->iNetEntityId == bossId && tail.pOwner->iPatternSequence == patternSequence) return &tail.Member;
	return nullptr;
}

LostArk::Server::SERVER_WORLD_ENTITY* LostArk::Server::CGameRoom::Find_KoukuOccurrenceOwner(
	const LostArk::Shared::NET_ENTITY_ID bossId, const std::uint32_t patternSequence)
{
	auto live = std::find_if(m_WorldEntities.begin(), m_WorldEntities.end(),
		[&](const auto& value) { return value.iNetEntityId == bossId && value.iCurrentHp > 0u; });
	if (live == m_WorldEntities.end()) return nullptr;
	if (live->iPatternSequence == patternSequence && !live->strPatternId.empty()) return &*live;
	for (auto& tail : m_KoukuSaydonPatternAudition.Tails)
		if (tail.pOwner->iNetEntityId == bossId && tail.pOwner->iPatternSequence == patternSequence)
		{
			...
			return tail.pOwner.get();
		}
	return nullptr;
}

bool LostArk::Server::CGameRoom::Retain_KoukuPatternTail(
	KOUKUSAYDON_PATTERN_AUDITION_MEMBER& member, const SERVER_WORLD_ENTITY& sourceOwner,
	const BOSS_PATTERN_DEFINITION& pattern, const std::uint32_t serverTick)
{
	...
	durationMs = (std::max)(durationMs, std::uint64_t(pattern.iTimelineDurationMs));
	const auto deadline = Clock::Add_Ticks(sourceOwner.iPatternStartTick, Clock::Ticks_FromMs(static_cast<std::uint32_t>(durationMs)));
	if (Clock::Has_ReachedTick(serverTick, deadline) && serverTick != deadline) return false;
	...
	for (auto& live : m_WorldEntities)
		...
		{
			std::erase_if(live.KoukuRetainedLogicOwners, [](const auto& owner) { return owner.expired(); });
			live.KoukuRetainedLogicOwners.push_back(tail.pOwner);
	...
	member.LogicLedger = {};
	m_KoukuSaydonPatternAudition.Tails.push_back(std::move(tail));
	return true;
}

void LostArk::Server::CGameRoom::Update_KoukuPatternTails(const std::uint32_t serverTick)
{
	auto& run = m_KoukuSaydonPatternAudition;
	std::erase_if(run.WorldPlays, [&](const auto& play) {
		return !play.bUntilDestroyed && play.iDurationMs && CKoukuSaydonLogicRuntime::Has_ReachedTick(serverTick,
			CKoukuSaydonLogicRuntime::Add_Ticks(play.iStartTick, CKoukuSaydonLogicRuntime::Ticks_FromMs(play.iDurationMs)));
	});
	if (run.Tails.empty()) return;
	const auto* product = Resolve_KoukuProductCatalog();
	const auto* base = m_GameplayCatalog.Resolve(run.PinnedGameplayRevision);
	if (!product || !base) { Clear_KoukuSaydonPatternAudition(false, "Row tail lost its pinned catalog"); return; }
	for (auto it = run.Tails.begin(); it != run.Tails.end();)
	{
		auto& tail = *it;
		...
		if (tail.iEndTick && CKoukuSaydonLogicRuntime::Has_ReachedTick(tail.iLastUpdateTick, tail.iEndTick))
		{
			...
			it = run.Tails.erase(it); continue;
		}
		auto live = std::find_if(m_WorldEntities.begin(), m_WorldEntities.end(),
			[&](const auto& boss) { return boss.iNetEntityId == tail.pOwner->iNetEntityId && boss.iCurrentHp > 0u; });
		if (live == m_WorldEntities.end()) { Clear_KoukuSaydonPatternAudition(false, "Row tail owner died or disappeared"); return; }
		std::string status;
		const auto* pattern = CKoukuSaydonBrain::Find_AnimationOnlyPattern(*product, tail.pOwner->strPatternId, status);
		if (!pattern) { Clear_KoukuSaydonPatternAudition(false, "Row tail lost its source pattern"); return; }
		tail.iLastUpdateTick = serverTick;
		// BOSS_CURRENT follows the actual body. Identity, clocks, Spawn and Stage origin stay pinned.
		tail.pOwner->fPositionX = live->fPositionX; tail.pOwner->fPositionY = live->fPositionY;
		tail.pOwner->fPositionZ = live->fPositionZ; tail.pOwner->fYawDegrees = live->fYawDegrees;
		tail.pOwner->iCurrentHp = live->iCurrentHp;
		...
		CKoukuSaydonLogicRuntime::Update(*tail.pOwner, *pattern, tail.Member.LogicLedger, m_Players, *base,
			...
			output, &m_ServerNavigation, &m_ServerCollisionSystem);
		Update_KoukuPlayerTargets(*tail.pOwner, *pattern, tail.Member.LogicLedger, *base, serverTick);
		// Tail results may schedule future rows; they cannot complete the newer actor Pattern.
		output.bEndPatternEarly = false;
		(void)Apply_KoukuLogicOutput(output, *tail.pOwner, serverTick);
		++it; // Keep the terminal tick alive until queued room outputs commit.
	}
}
...
{
	for (auto& member : m_KoukuSaydonPatternAudition.Members)
		if (member.LogicLedger.Is_Active() && member.bOwnsPlayerMode) return &member.LogicLedger;
	for (auto& tail : m_KoukuSaydonPatternAudition.Tails)
		if (tail.Member.LogicLedger.Is_Active() && tail.Member.bOwnsPlayerMode) return &tail.Member.LogicLedger;
	if (m_KoukuSaydonPatternAudition.Members.size() == 1u)
	{
		auto& ledger = m_KoukuSaydonPatternAudition.Members.front().LogicLedger;
		if (ledger.Is_Active()) return &ledger;
		for (auto& tail : m_KoukuSaydonPatternAudition.Tails)
			if (tail.Member.LogicLedger.Is_Active()) return &tail.Member.LogicLedger;
	}
	return nullptr;
}
...
bool LostArk::Server::CGameRoom::Build_KoukuBundleState(LostArk::Shared::S2C_KOUKUSAYDON_BUNDLE_STATE& message) const
{
	...
	for (const auto& member : run.Members)
	{
        ...
        if (member.MarioEntryAnchor && !member.bMarioEntryConsumed && member.iMarioEntryStage >= 1u && member.iMarioEntryStage <= 4u)
        {
            ...
            const auto* root = catalog ? CKoukuSaydonBrain::Find_AnimationOnlyPattern(*catalog, member.strMarioEntryPatternId, status) : nullptr;
            // Only chain roots publish the hold: the Shared writer refuses a pattern ID without a hold, and the
            // Client would freeze the normal boss session; a non-chain entry pattern keeps presenting itself.
            if (root) for (const auto& window : root->LogicWindows)
                if (window.eKind == BOSS_PATTERN_LOGIC_KIND::PATTERN_COMPLETION_COUNT)
                {
                    state.strMarioEntryPatternId = member.strMarioEntryPatternId;
                    state.iMarioEntryStartTick = member.iMarioEntryStartTick;
                    state.iMarioEntryHoldMs = (std::max)(1u, window.iStartMs);
                    state.iMarioEntryStage = member.iMarioEntryStage;
                    state.fMarioEntryX = member.MarioEntryAnchor->fPositionX;
                    state.fMarioEntryY = member.MarioEntryAnchor->fPositionY;
                    state.fMarioEntryZ = member.MarioEntryAnchor->fPositionZ;
                    state.fMarioEntryYawDegrees = member.MarioEntryAnchor->fYawDegrees;
                    break;
                }
        }
...
void LostArk::Server::CGameRoom::Stop_KoukuWorldOwner(const std::string& memberId, const bool finished)
{
	...
	for (auto it = m_KoukuSaydonPatternAudition.Tails.begin(); it != m_KoukuSaydonPatternAudition.Tails.end();)
	{
		if (!memberId.empty() && it->Member.strMemberId != memberId) { ++it; continue; }
		Clear_KoukuPlayerTargets(*it->pOwner, it->Member.LogicLedger);
		CKoukuSaydonLogicRuntime::Discard(it->Member.LogicLedger, m_Players, it->pOwner.get());
		it = m_KoukuSaydonPatternAudition.Tails.erase(it);
	}
...
void LostArk::Server::CGameRoom::Clear_KoukuSaydonPatternAudition(const bool completed, std::string reason)
{
	...
	for (auto& member : run.Members)
	{
		...
		if (!completed) (void)Release_PlayerAttachments(member.iBossEntityId, 0.f, 0u, false, 0u, m_iServerTick ? m_iServerTick : 1u);
		if (member.LogicLedger.Is_Active())
		{
		...
		if (!completed) std::erase_if(m_PendingKoukuMechanicTriggers, [&](const auto& trigger) { return trigger.iBossEntityId == member.iBossEntityId; });
	}
	if (completed) run.ePhase = KOUKUSAYDON_PATTERN_AUDITION_PHASE::INACTIVE;
	else run = {};
...
{
	const auto& run = m_KoukuSaydonPatternAudition;
	if (!run.iRoomAuditionEpoch && run.ePhase == KOUKUSAYDON_PATTERN_AUDITION_PHASE::INACTIVE) return nullptr;
...
void LostArk::Server::CGameRoom::Prepare_KoukuAuditionTick(const std::uint32_t serverTick)
{
	...
	for (auto& member : run.Members)
	{
		...
		if (member.bCompletionChainAwaitingReturn)
		{
			// A successful Parent entry ends phase 1 at the next tick boundary.
			// Stop its remaining children and hits before admitting phase 2 or waiting for solo return.
			if (member.bMarioEntryConsumed && !member.bCompletionChainStarted && !boss->strPatternId.empty())
			{
				const auto completedPattern = boss->strPatternId;
				const auto sequence = boss->iPatternSequence, stage = boss->iPatternStageIndex;
				Stop_KoukuWorldOwner(member.strMemberId);
				std::erase_if(supportSchedule, [&](const auto& support) { return support.strMemberId == member.strMemberId; });
				m_CombatObjectRuntime.Cancel_Source(boss->iNetEntityId);
				std::erase_if(m_PendingKoukuMechanicTriggers, [&](const auto& trigger) { return trigger.iBossEntityId == boss->iNetEntityId; });
				...
				Queue_KoukuSaydonPatternAuditionLifecycle(completedPattern, sequence, stage,
	...
	for (auto& start : starts)
	{
		*start.live = std::move(start.staged); start.member->LogicLedger = std::move(start.ledger);
		const auto* root = catalog->Find_BossPatterns(start.live->strEncounterId);
		const auto definition = root ? std::find_if(root->begin(), root->end(), [&](const auto& row) { return row.strPatternId == start.live->strPatternId; }) : std::vector<BOSS_PATTERN_DEFINITION>::const_iterator{};
		if (root && definition != root->end() && CKoukuSaydonLogicRuntime::Has_MarioEntry(*definition))
		{
			// An authored stage wins over the live counter unless the request fixed a test stage.
			const auto* authored = CKoukuSaydonLogicRuntime::Find_MarioEntryResult(*definition);
			const bool testFixture = m_KoukuSaydonPatternAudition.Request.iMarioTestStartStage != 0u;
			start.member->MarioEntryAnchor = *start.live;
			start.member->strMarioEntryPatternId = start.live->strPatternId;
			start.member->iMarioEntryStartTick = start.live->iPatternStartTick;
			start.member->iMarioEntryStage = (!testFixture && authored && authored->iMarioEntryStage >= 1u && authored->iMarioEntryStage <= 4u) ?
				authored->iMarioEntryStage : m_iNextMarioEntryStage;
...
bool LostArk::Server::CGameRoom::Start_KoukuCompletionChain(
	...
	const BOSS_PATTERN_DEFINITION& pattern, const std::uint32_t serverTick)
{
	if (member.bCompletionChainStarted) return false;
	for (const auto& window : pattern.LogicWindows)
	{
		if (window.eKind != BOSS_PATTERN_LOGIC_KIND::PATTERN_COMPLETION_COUNT ||
			!CKoukuSaydonLogicRuntime::Has_ReachedTick(serverTick, CKoukuSaydonLogicRuntime::Add_Ticks(
				boss.iPatternStartTick, CKoukuSaydonLogicRuntime::Ticks_FromMs(window.iStartMs)))) continue;
		auto candidates = window.PatternIds;
		const auto& request = m_KoukuSaydonPatternAudition.Request;
		std::mt19937 random(request.iMarioTestStartStage ? request.iMarioTestSeed : m_MarioLayoutRandom());
		// Fisher-Yates with explicit modulo keeps a test seed reproducible across STL builds.
		for (std::size_t i = candidates.size(); i > 1u; --i) std::swap(candidates[i - 1u], candidates[random() % i]);
		candidates.resize(window.iCompletionCount);
		const std::size_t insertAt = member.iPatternIndex + 1u;
		member.PatternIds.insert(member.PatternIds.begin() + insertAt, candidates.begin(), candidates.end());
		member.TransitionTicks.insert(member.TransitionTicks.begin() + member.iPatternIndex, candidates.size(), 1u);
...
void LostArk::Server::CGameRoom::Update_KoukuMarioEntry(
	KOUKUSAYDON_PATTERN_AUDITION_MEMBER& member, const std::uint32_t serverTick)
{
	if (!member.MarioEntryAnchor || member.bMarioEntryConsumed || member.iMarioEntryStage < 1u || member.iMarioEntryStage > 4u) return;
	const auto* catalog = Resolve_KoukuProductCatalog();
	std::string status;
	const auto* root = catalog ? CKoukuSaydonBrain::Find_AnimationOnlyPattern(*catalog, member.strMarioEntryPatternId, status) : nullptr;
	if (!root) return;
	...
	if (honorWindowEnd)
	{
		...
		const BOSS_PATTERN_LOGIC_WINDOW* failure = nullptr;
		for (const auto& window : root->LogicWindows)
			...
			{
				lastEnd = (std::max)(lastEnd, CKoukuSaydonLogicRuntime::Ticks_FromMs(window.iStartMs + window.iDurationMs));
				if (!window.OnTimeout.empty()) failure = &window;
			}
		if (lastEnd && age >= lastEnd && failure)
		{
	...
	for (auto& [id, player] : m_Players)
	{
		std::uint8_t matchedStage = 0u;
		if (!CKoukuSaydonLogicRuntime::Is_InsideMarioEntry(*root, *member.MarioEntryAnchor, player, age, honorWindowEnd, &matchedStage) ||
			std::any_of(m_PendingKoukuMarioEntries.begin(), m_PendingKoukuMarioEntries.end(),
				[&](const auto& entry) { return entry.strMemberId == member.strMemberId; })) continue;
		const auto stage = m_KoukuSaydonPatternAudition.Request.iMarioTestStartStage || !matchedStage ? member.iMarioEntryStage : matchedStage;
		m_PendingKoukuMarioEntries.push_back({member.strMemberId, id, member.iMarioEntryStartTick, stage});
		break;
	}
}

void LostArk::Server::CGameRoom::Commit_KoukuMarioEntries()
{
	for (const auto& entry : m_PendingKoukuMarioEntries)
	{
		const auto found = std::find_if(m_KoukuSaydonPatternAudition.Members.begin(), m_KoukuSaydonPatternAudition.Members.end(),
		...
		{
			std::string status;
			if (const auto* phase = CKoukuSaydonBrain::Find_AnimationOnlyPattern(*catalog, member.strCompletionChainSuccessPatternId, status))
				for (const auto& trigger : phase->MechanicTriggers)
					if (trigger.eKind == BOSS_PATTERN_MECHANIC_TRIGGER_KIND::MARIO_PHASE2_PLAYERS)
					{
						player->second.MarioReturnPosition = std::array<float, 3u>{trigger.fTeleportX, trigger.fTeleportY, trigger.fTeleportZ};
						break;
					}
		}
		...
		if (!member.bCompletionChainStarted && !member.strCompletionChainSuccessPatternId.empty())
		{
			member.bCompleted = false;
	...
	m_PendingKoukuMarioEntries.clear();
	if (!m_KoukuSaydonPatternAudition.Members.empty() &&
		std::all_of(m_KoukuSaydonPatternAudition.Members.begin(), m_KoukuSaydonPatternAudition.Members.end(), [](const auto& row) { return row.bCompleted; }))
		Clear_KoukuSaydonPatternAudition(true);
}

bool LostArk::Server::CGameRoom::Commit_KoukuMarioPhasePlayers(
	SERVER_WORLD_ENTITY& boss, const BOSS_PATTERN_MECHANIC_TRIGGER& trigger, const std::uint32_t serverTick)
{
	using namespace LostArk::Shared;
	const auto* catalog = Resolve_KoukuProductCatalog();
	std::string status;
	const auto* pattern = catalog ? CKoukuSaydonBrain::Find_AnimationOnlyPattern(*catalog, boss.strPatternId, status) : nullptr;
	...
	for (const auto& [id, player] : m_Players)
	{
		...
		SERVER_NAV_POINT ground{};
		if (!m_ServerNavigation.Is_PointWalkableExact(x, z) || !m_ServerNavigation.Sample_Position(x, z, ground) ||
			!std::isfinite(ground.y) || std::abs(ground.y - trigger.fTeleportY) > 1.f ||
			!m_ServerCollisionSystem.Is_CirclePositionClear(x, ground.y + .8f, z, .35f, .8f, .35f, boss.iNetEntityId))
	...
	if (living >= 3u && !staged.empty())
	{
		const auto boundIndex = m_MarioLayoutRandom() % staged.size();
		auto& bound = staged[boundIndex].second;
		// Keep the captive at the actual cage anchor, independent of random identity.
		std::swap(bound.fPositionX, staged.front().second.fPositionX);
		std::swap(bound.fPositionY, staged.front().second.fPositionY);
		std::swap(bound.fPositionZ, staged.front().second.fPositionZ);
		bound.bPatternBound = true;
		bound.iPatternBindOwnerNetEntityId = boss.iNetEntityId;
		bound.iPatternBindSequence = boss.iPatternSequence;
		bound.iPatternBindEndTick = deadline;
		bound.fPatternBindRestoreX = bound.fPositionX; bound.fPatternBindRestoreY = bound.fPositionY;
		bound.fPatternBindRestoreZ = bound.fPositionZ; bound.fPatternBindRestoreYawDegrees = bound.fYawDegrees;
	...
	const SERVER_PLAYER& player, const std::string& sourcePlacementId, const std::uint32_t updateTick)
{
	...
	for (auto& member : m_KoukuSaydonPatternAudition.Members)
	{
		...
		SERVER_NAV_POINT ground;
		if (!Resolve_MarioReturnDestination(player, ground) ||
			!std::isfinite(player.fPositionX) || !std::isfinite(player.fPositionY) || !std::isfinite(player.fPositionZ) ||
			std::abs(player.fPositionX - ground.x) > 0.05f ||
			std::abs(player.fPositionZ - ground.z) > 0.05f || std::abs(player.fPositionY - ground.y) > 0.25f)
		{
			m_strStatus = "Mario solo return did not reach its pinned return point";
			Clear_KoukuSaydonPatternAudition(false, m_strStatus); return;
		}
		member.bMarioReturnCompleted = true;
		m_strStatus = "Mario solo entrant returned to Gate 3; phase 2 can begin";
...
bool LostArk::Server::CGameRoom::Update_KoukuSaydonBoss(SERVER_WORLD_ENTITY& boss, const std::uint32_t serverTick)
{
	...
	if (member->bCompletionChainStarted && member->iCompletionChainCompleted < member->iCompletionChainCount &&
		completedIndex == member->iCompletionChainFirstIndex + member->iCompletionChainCompleted)
	{
		++member->iCompletionChainCompleted;
		std::cout << "[MarioPatternChain] completed=" << member->iCompletionChainCompleted << "/" << member->iCompletionChainCount
	...
	// A non-chain entry pattern closes its portal with the pattern. An entry queued by this
	// update commits right after it, so keep the anchor and the queued entry while a later
	// member pattern continues the run; Commit closes the portal once it has consumed them.
	// A last-tick contact commits before a completed run is released.
	if (member->MarioEntryAnchor && !member->bCompletionChainStarted && member->strMarioEntryPatternId == completedPatternId &&
		std::none_of(m_PendingKoukuMarioEntries.begin(), m_PendingKoukuMarioEntries.end(),
			[&](const auto& entry) { return entry.strMemberId == member->strMemberId; }))
	{
		member->MarioEntryAnchor.reset();
	...
	else
	{
		...
		if (m_PendingKoukuMarioEntries.empty() && std::all_of(m_KoukuSaydonPatternAudition.Members.begin(), m_KoukuSaydonPatternAudition.Members.end(), [](const auto& value) { return value.bCompleted; }))
