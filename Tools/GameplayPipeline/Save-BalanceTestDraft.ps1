...
try {
    ...
    foreach ($change in @($draft.changes)) {
        ...
        if (-not $fields.Add("$path#$($change.id).$($change.field)")) { throw 'Duplicate draft field.' }
    ...
    & (Join-Path $PSScriptRoot 'Publish-GameplayBalance.ps1') -Mode Validate -InputOverlayRoot $stage
