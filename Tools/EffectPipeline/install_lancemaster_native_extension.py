...
def install(evidence):
    ...
    for row in programs:
        ...
        cases.extend(['#if ' + ' && '.join(f'!defined(EFFECT_NATIVE_{x}_CARRIER)' for x in other),
    ...
    suffix = suffix.replace('    default: clip(-1.f); return output;',
                            '\n'.join(cases) + '\n    default: clip(-1.f); return output;', 1)
    path.write_text(prefix + suffix, encoding='utf8')

    header_path = ROOT / 'Client/Public/Effect_LanceMasterVAMaterial.h'
    header = header_path.read_text(encoding='utf8')
    ...
    header_path.write_text(header[:table.start()] + replacement + header[table.end():], encoding='utf8')
