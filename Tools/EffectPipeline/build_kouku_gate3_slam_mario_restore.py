...
def complete_small_pentagram(document, evidence):
    """Preserve the original drawing and join its original star-shot emitter set.

    The user rejected the added static gray floor star. Keep the source cast
    bytes and reuse the disabled star-shot system as an explicit independent
    library sequence; the source action enable flags remain in the evidence.
    """
    from build_kouku_ritual_hand_trail import source_projection, complete_small_pentagram as join_star_shot
    _, explosion, shot_time = source_projection(evidence / 'small_pentagram_source')
    return join_star_shot(document, explosion, shot_time)
...
def project(evidence, material_patch, install):
    ...
    for key, (suffix, title) in {**ASSETS, **{k: v[:2] for k, v in LIBRARIES.items()}}.items():
        ...
        if key == 4219951:
            ...
            document, sequence_report = complete_small_pentagram(document, evidence)
            source.write(evidence / 'small_pentagram_sequence.json', sequence_report)
...
if __name__ == '__main__':
    ...
    if arguments.complete_small_pentagram:
        ...
        document, fit = complete_small_pentagram(source.read(destination), arguments.evidence_root)
        assert destination.read_bytes() == before, 'Preserve concurrent authored edits'
        source.write(destination, document)
        source.write(arguments.evidence_root / 'small_pentagram_sequence.json', fit)
