...
def entry_portal(evidence, material_patch=None):
    """Stage the original five-second Magic Pillar Buff as a separate resource.

    The invisible circle owner's action starts this Buff at 15s. A reusable
    entry portal starts its own clock at zero; pattern placement remains owned
    by Composition. Existing center circles and their authored edits are not
    ...
    asset = 'effect.kouku.gate3.mario.entry.portal.full.restore'
    system = 'fx_mn_rpct_07_v.par_v_rpct_light_line_01_loc_int'
    archive = Path('C:/ProgramData/Smilegate/Games/LOSTARK/EFGame/data3.lpk')
    suffix = r'particlesoundnew\9_ef_particle_sound_data_buff_fx_monster_basebuff_koukusaton_magic_pillar.loa'
    with archive.open('rb') as stream, mmap.mmap(stream.fileno(), 0, access=mmap.ACCESS_READ) as packed:
        ...
        entry = next(e for e in lpk.read_index(packed, key) if e['path'].lower().endswith(suffix))
        raw = lpk.extract(packed, entry, key, base)
    digest = hashlib.sha256(raw).hexdigest()
    assert digest == '3e5b4141e349eac31ec65c37186105a93825aef43d39c3282a468508fee63832'
    assert len(raw) == 984
    evidence.mkdir(parents=True, exist_ok=True)
    (evidence / 'KoukuSaton_Magic_Pillar.loa').write_bytes(raw)
    particle = next(t for t in scan_length_prefixed_strings(raw, 0, len(raw))
                    if t['value'].startswith("ParticleSystem'"))
    assert particle['sourceOffset'] == 309 and particle['value'].split("'")[1].lower() == system
    position = list(struct.unpack_from('<3f', raw, 454))
    rotation = list(struct.unpack_from('<3i', raw, 466))
    scale = list(struct.unpack_from('<3f', raw, 514))
    assert position == rotation == [0, 0, 0] and scale == [1, 1, 1]
    assert struct.unpack_from('<i', raw, 526)[0] == 3
    parameters = [dict(name='Color', type='vector', vectorValue=list(struct.unpack_from('<3f', raw, 552))),
                  dict(name='Lifetime', type='scalar', scalarValue=struct.unpack_from('<f', raw, 626)[0]),
                  dict(name='Size', type='vector', vectorValue=list(struct.unpack_from('<3f', raw, 712)))]
    targets = {104: dict(asset=asset, name='진입 포탈', system=system, count=8,
                        countIncludesEmpty=True, sourceActions=['MN_ISTM_00-4:4222001:SkillBuff4219930'])}
    index, notifies, occurrences, records = library.acquire(evidence, targets)
    assert len(notifies) == 1 and len(occurrences) == 8
    notify = notifies[0]
    cue = notify['cue']
    cue.update(sourceKind='ORIGINAL_BUFF_PARTICLE_EFFECT', parameterOverrides=parameters)
    cue['localTransform'] = dict(position=[0, 0, 0], rotationDegrees=[0, 0, 0], scale=scale)
    notify.update(durationSeconds=5, serializedPayload=copy.deepcopy(cue))
    for occurrence in occurrences:
        occurrence['sourceDurationSeconds'] = 5
    source.write(evidence / 'source_notifies.json', notifies)
    source.write(evidence / 'source_occurrences.json', occurrences)
    proof = dict(archive=str(archive), archiveEntry=entry, sourceSha256=digest,
        sourceParticleDataOffset=277, sourceParticleSystemOffset=309,
        sourceActionId=4222001, sourceNpcId=480650, sourceEffectId=422200109,
        sourceBuffId=4219930, sourceBuffArchetype='KoukuSaton_Magic_Pillar',
        sourceActionStartSeconds=15, standaloneStartSeconds=0, buffDurationSeconds=5,
        sourceTransform=cue['localTransform'], parameterOverrides=parameters,
        parameterPolicy='EXACT_NAMED_DISTRIBUTION_ONLY', manualVisualValidation='USER_PENDING')
    source.write(evidence / 'entry_portal_source.json', proof)
    if material_patch is None:
        return
    original_decode = source.decode_typed_payload
    def decode(kind, payload, *args):
        if isinstance(payload, dict) and payload.get('sourceKind') == 'ORIGINAL_BUFF_PARTICLE_EFFECT':
            return copy.deepcopy(payload)
        return original_decode(kind, payload, *args)
    source.decode_typed_payload = decode
    try:
        source.project(evidence, index, notifies, occurrences, records, evidence / 'projected', material_patch)
    finally:
        source.decode_typed_payload = original_decode
    doc = source.read(evidence / 'projected/effect.kouku.gate1.104.full.restore.effect.json')
    doc.update(effectAssetId=asset, displayName='진입 포탈')
    ...
    duration = math.ceil(max(e['detail']['timing']['startDelaySeconds'] + e['detail']['timing']['lifeTimeSeconds']
                            + max(e['detail']['particle']['lifeTimeSeconds']) for e in doc['elements']) * 1000)
    source.write(evidence / 'candidate' / (asset + '.effect.json'), doc)
    source.write(evidence / 'entry_portal_candidate.json', dict(effectAssetId=asset, displayName='진입 포탈',
        path='Data/Effects/Authored/' + asset + '.effect.json', elementCount=8, durationMs=duration,
        nativeProfiles=sorted({e['material']['sourceProfile']['runtimeShaderProfileId'] for e in doc['elements']}),
        sourceProof='entry_portal_source.json', installed=False, manualVisualValidation='USER_PENDING'))
    print(json.dumps(dict(assetId=asset, elementCount=8, durationMs=duration)))
...
def complete_small_pentagram(document, evidence):
    ...
    from build_kouku_ritual_hand_trail import (source_projection, complete_small_pentagram as join_star_shot,
                                              complete_small_pentagram_impact)
    _, explosion, shot_time = source_projection(evidence / 'small_pentagram_source')
    document, report = join_star_shot(document, explosion, shot_time)
    return complete_small_pentagram_impact(document, evidence / 'small_pentagram_impact_source'), report
...
if __name__ == '__main__':
    ...
    parser.add_argument('--entry-portal', action='store_true', help='Acquire or stage only the original Magic Pillar Buff; never install')
    arguments = parser.parse_args()
    if arguments.entry_portal:
        assert not arguments.install, 'Entry portal is staged for independent registration and review'
        entry_portal(arguments.evidence_root, arguments.native_material_patch)
    elif arguments.complete_small_pentagram:
