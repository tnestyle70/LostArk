...
by ``apply_valtan_reviewed_source_family_candidates.py``.
"""
...
DEFAULT_DRAWABLE_SWEEP = PurePosixPath(
    ...
    "Valtan.reviewed-source-families.drawable-sweep.v1.json"
)
...
class DrawableProofError(RuntimeError):
    """The machine sweep cannot prove the exact reviewed candidate corpus."""
...
def build_drawable_proof(
    ...
    *,
    ...
    if len(candidates) != EXPECTED_TARGET_COUNT:
        raise DrawableProofError("candidate receipt must contain exactly 36 documents")
    ...
    if (
        ...
        raise DrawableProofError("candidate identity is invalid or duplicated")
    ...
    if len(documents) != EXPECTED_TARGET_COUNT:
        raise DrawableProofError("drawable sweep must contain exactly 36 documents")
    ...
    for candidate, sweep_document in zip(candidates, documents, strict=True):
        ...
        if (
            not candidate_elements
            or len(candidate_elements) != candidate.get("candidateElementCount")
            or any(
            ...
            or len(set(candidate_pairs)) != len(candidate_elements)
    ...
    if len(global_pairs) != EXPECTED_ELEMENT_COUNT:
        raise DrawableProofError("drawable proof must cover exactly 279 source elements")
    return {
        "schema": PROOF_SCHEMA,
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        f"[PASS] {mode} Valtan reviewed source-family drawable proof "
