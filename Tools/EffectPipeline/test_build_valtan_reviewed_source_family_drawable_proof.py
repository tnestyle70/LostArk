#!/usr/bin/env python3
"""Tests for reviewed Valtan source-family machine proof sealing."""
...
import build_valtan_reviewed_source_family_drawable_proof as sut
import validate_boss_pattern_effects as schema_validator


SOURCE_ROOT = Path(__file__).resolve().parents[2]
SCRIPT_PATH = Path(__file__).with_name(
    "build_valtan_reviewed_source_family_drawable_proof.py"
)
...
class RepositoryFixture:
    def __init__(self) -> None:
        self._temporary = tempfile.TemporaryDirectory(
            prefix="valtan-reviewed-proof-test."
        )
        ...
        self.candidates = list(self.receipt["documents"])
...
class ValtanReviewedSourceFamilyDrawableProofTests(unittest.TestCase):
    def make_fixture(self) -> RepositoryFixture:
        fixture = RepositoryFixture()
        self.addCleanup(fixture.close)
        return fixture

    def test_build_binds_exact_36_documents_and_all_279_elements(self) -> None:
        fixture = self.make_fixture()
        proof = fixture.build()
        self.assertEqual(36, len(proof["targets"]))
        self.assertEqual(
            279, sum(len(target["elements"]) for target in proof["targets"])
        )
        self.assertEqual(
            [row["effectAssetId"] for row in fixture.candidates],
            [row["effectAssetId"] for row in proof["targets"]],
        )
        for candidate, target in zip(
            fixture.candidates, proof["targets"], strict=True
        ):
            self.assertEqual(
