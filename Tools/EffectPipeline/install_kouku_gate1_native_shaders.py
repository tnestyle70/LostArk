...
def install_partitioned_groups(shader_text, case_text):
    ...
    for block in conditional_blocks(route_scene_bloom_samples(shader_text)):
        ...
        name, number = names[0]
        number = int(number)
        assert 2304 <= number <= 4671 and name not in functions, name
...
def append_reviewed(source_dir, resource_root=None):
    ...
    assert len(owned) == len(rows) and owned <= set(range(2304, 4672))
...
def install(source_dir, append_source_dir=None):
    ...
    assert len(identifiers) == len(rows) and set(range(2304, 2342)) <= identifiers <= set(range(2304, 4672))
...
'''
    ...
    for name, expected in (("Shader_VtxEffectDecal.hlsl", 1),
        ...
        def extend_dispatch(text):
            ...
            return re.sub(pattern, 'g_SourceMaterialProfile >= 2304u && g_SourceMaterialProfile <= 4671u', text)
