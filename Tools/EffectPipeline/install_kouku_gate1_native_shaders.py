...
def install(source_dir):
    ...
    assert {r["program"] for r in rows} == set(range(2304, 2342))
    ...
    assert {int(i) for _, i in blocks} == set(range(2304, 2342))
    shaders = ROOT / "Client/Bin/ShaderFiles"
    def carrier_for(row):
        return {"mesh": "MESH", "decal": "DECAL", "animationTrail": "TRAIL"}.get(row["rendererShape"], "PARTICLE")
    ...
    (shaders / "Shader_EffectKoukuNativeGroup2304.hlsli").write_text(
        "// Original Kouku Gate1 material programs; native carrier group 2304.\n" +
...
'''
    ...
    def table(text):
        for carrier in ("MESH", "PARTICLE"):
            label = carrier.title()
            row = f'        EFFECT_SHADER_PROGRAM_ROW({carrier}, ARTIST, 2304u, 2367u, "Shader_VtxEffect{label}Kouku2304.hlsl"),\n'
            if row not in text:
    ...
    print("Installed 38 Kouku native programs and two existing-family shader carriers.")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(__doc__)
    parser.add_argument("--source-dir", type=Path, required=True)
    install(parser.parse_args().source_dir.resolve())
