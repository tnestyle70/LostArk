...
function Invoke-PythonSource {
    ...
    $file = Join-Path ([IO.Path]::GetTempPath()) `
        ('lostark-kouku-saydon-' + [Guid]::NewGuid().ToString('N') + '.py')
...
reader = runpy.run_path(sys.argv[1], run_name="kouku_saydon_wmodel_reader")
path = Path(sys.argv[2])
data = path.read_bytes()
animations = reader["read_wmodel_animation_sections"](data)
mesh_reader = runpy.run_path(sys.argv[3], run_name="kouku_saydon_wmodel_mesh_reader")
...
if (-not $destination.StartsWith($expectedParent, [StringComparison]::OrdinalIgnoreCase)) {
    throw "DestinationRoot must be below the KoukuSaydon map resource directory: $destination"
}
...
$workRoot = Join-Path $tempBase ('lostark-kouku-saydon-animated-props-' + [Guid]::NewGuid().ToString('N'))
...
try {
    ...
    foreach ($asset in $assets) {
        ...
        $receipt = [ordered]@{
            schema = 'lostark.kouku-saydon-interaction-prop-cook'
            formatVersion = 1
    ...
    Write-Host "KOUKU_SAYDON_INTERACTION_PROPS_COOK_OK destination=$destination assets=$($summary.Count)"
    ...
    if ($resolvedWork.StartsWith($tempBase, [StringComparison]::OrdinalIgnoreCase) -and
        [IO.Path]::GetFileName($resolvedWork).StartsWith('lostark-kouku-saydon-animated-props-', [StringComparison]::Ordinal)) {
