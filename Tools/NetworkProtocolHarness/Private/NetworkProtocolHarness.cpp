...
namespace
{
	...
	void Test_MarioMoveProtocol(TEST_RUNNER& testRunner)
	{
		...
		testRunner.Require(NETWORK_PROTOCOL_VERSION == 100u && Is_Known_Packet_Type(PACKET_TYPE::C2S_MARIO_MOVE) &&
			...
			"Mario direction packet retains its appended identity in protocol 100");
	}
	...
	void Test_MarioReturnProtocol(TEST_RUNNER& testRunner)
	{
		...
		testRunner.Require(NETWORK_PROTOCOL_VERSION == 100u &&
			...
			"Protocol 100 appends Mario return without changing existing packet identities");
	}
	...
	void Test_DebugMarioJumpProtocol(TEST_RUNNER& testRunner)
	{
		...
		testRunner.Require(NETWORK_PROTOCOL_VERSION == 100u &&
			...
			"Protocol 100 preserves Mario jump packet identities without renumbering existing peers");
	}
	...
	{
		using namespace LostArk::Shared;
		testRunner.Require(NETWORK_PROTOCOL_VERSION == 100u, "World Object owner lifecycle, fear, zone pulse and wave re-summon use protocol 100");
		testRunner.Require(
	...
	void Test_KoukuInteractionProtocol(TEST_RUNNER& testRunner)
	{
		...
		testRunner.Require(Is_Known_Packet_Type(PACKET_TYPE::C2S_INTERACTION_SLOT) &&
			...
			NETWORK_PROTOCOL_VERSION == 100u,
			"Protocol 100 preserves main trigger identities with WORLD occurrence placement");
	}
	...
	{
		{
			testRunner.Require(100u == NETWORK_PROTOCOL_VERSION,
				"KoukuSaydon Source Pin And Existing Contracts Use Protocol 100");
	...
	void Test_DataRevisionHotReloadProtocol(TEST_RUNNER& testRunner)
	{
		testRunner.Require(
			100u == NETWORK_PROTOCOL_VERSION,
			"World Spawn Pin Complete Play And Two-Revision Restart CAS Use Protocol 100");
