...
namespace
{
	...
	void Test_MarioStageSnapshotProtocol(TEST_RUNNER& testRunner)
	{
		...
		snapshot.Players[0].iMarioCurseReleasedMask = 7u;
		{
			...
			testRunner.Require(written && Read_Message(reader, decoded) && !reader.Get_RemainingSize() &&
				...
				decoded.Players[0].iMarioCurseReleasedMask == 7u, "Mario popped-ball and curse masks round trip");
		}
	...
	void Test_DebugMarioJumpProtocol(TEST_RUNNER& testRunner)
	{
		...
		testRunner.Require(NETWORK_PROTOCOL_VERSION == 79u &&
			...
			"Protocol 79 preserves Mario jump packet identities without renumbering existing peers");
	}
	...
	void Test_WorldObjectMotionProtocol(TEST_RUNNER& testRunner)
	{
		...
		testRunner.Require(
			...
			"Protocol 79 preserves main identities and bundle state");
	...
	void Test_KoukuInteractionProtocol(TEST_RUNNER& testRunner)
	{
		...
		testRunner.Require(Is_Known_Packet_Type(PACKET_TYPE::C2S_INTERACTION_SLOT) &&
			...
			"Protocol 79 preserves main trigger identities with WORLD occurrence placement");
	}
	...
	{
		{
			testRunner.Require(78u == NETWORK_PROTOCOL_VERSION,
				"KoukuSaydon Source Pin And Existing Contracts Use Protocol 79");
	...
	void Test_DataRevisionHotReloadProtocol(TEST_RUNNER& testRunner)
	{
		testRunner.Require(
			78u == NETWORK_PROTOCOL_VERSION,
			"World Spawn Pin Complete Play And Two-Revision Restart CAS Use Protocol 79");
