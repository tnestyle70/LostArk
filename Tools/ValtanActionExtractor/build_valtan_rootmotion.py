...
def build(repo_root: Path) -> tuple[dict, list[str]]:
    ...
    curves = read_root_curves(
        repo_root / "Client/Bin/Resources/Character/Valtan"
                    "/AnimSets/MN_RPBF_01_AnimSet.wmodel")
    ...
    for pattern in encounter["patterns"]:
        ...
        for stage_index, stage in enumerate(pattern["stages"]):
            ...
            travel = max(abs(end_forward - base_forward),
                         abs(end_lateral - base_lateral))
            if authored > 0.0:
                notes.append(
                    f"{pattern['patternId']}/{stage['stageId']}: kept authored "
                    f"{authored:.1f} m, clip {clip_name} bakes "
                    f"{end_forward - base_forward:+.2f} m")
                continue
            ...
            while True:
                ...
                samples.append({
                    "timeMs": time_ms,
                    "forward": round(forward - base_forward, 4),
                    "lateral": round(lateral - base_lateral, 4),
...
def main() -> int:
    ...
    document, notes = build(arguments.repo_root)
