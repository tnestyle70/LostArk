#!/usr/bin/env python3
"""Build reference-only KoukuSaydon action/animation documents.
...
REFERENCE_SCHEMA = "lostark.kouku-saydon-animation-action-reference"
AUTHORED_SCHEMA = "lostark.kouku-saydon-animation-action-bindings"
FORMAT_VERSION = 1
...
PATTERN_SCHEMA = "lostark.kouku-saydon-animation-pattern-bindings"
PATTERN_ROOT_KEYS = {
    "schema",
    "formatVersion",
    "profileId",
...
def validate_pattern_document(document: Any, reference: dict[str, Any]) -> None:
    """Validate the reference-only KoukuSaydon pattern authoring projection.
    ...
    pattern_shape = re.compile(
        rf"kakulsaydon\.{re.escape(profile_id)}\.pattern\.([1-9][0-9]*)"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(
            f"KoukuSaydon animation reference generation failed: {error}",
            file=sys.stderr,
        )
        return 1
