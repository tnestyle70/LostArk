#!/usr/bin/env python3
"""Fail-closed admission report for the KoukuSaydon raid Area.

``KoukuSaton`` remains solely the protected physical resource collection alias;
it never replaces the stable source/Area identity ``LV_LUT_MIDNIGHTC_ED``.
This validator is deliberately
...
class AdmissionReport:
    ...
    def as_json(self, requested_mode: str) -> dict[str, Any]:
        ...
        return {
            "schema": "lostark.kouku-saydon-world-admission-report",
            "formatVersion": 1,
...
def _validate_resource_collection(root: Path) -> ModeResult:
    ...
    if not available_roots:
        stage.issue(
            ...
            "no KoukuSaydon stable resource payload was found under the protected "
            "physical aliases; aliases alone are not assets",
        )
...
def format_human(
    ...
    *,
    ...
    lines = [
        f"KoukuSaydon admission: canonicalArea={AREA_ID}",
