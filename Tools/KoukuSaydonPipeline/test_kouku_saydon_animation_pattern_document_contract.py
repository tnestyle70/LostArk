...
REFERENCE_ROOT = ROOT / "Data" / "Animation" / "Reference" / "KoukuSaydon"
AUTHORED_ROOT = ROOT / "Data" / "Animation" / "Authored" / "KoukuSaydon"
HEADER = ROOT / "Client" / "Public" / "KoukuSaydonAnimationPatternDocument.h"
SOURCE = ROOT / "Client" / "Private" / "KoukuSaydonAnimationPatternDocument.cpp"
...
def _validate_contract(document, reference, available_clips, expected_profile, file_name):
    if set(document) != ROOT_FIELDS:
        return False
    if document["schema"] != "lostark.kouku-saydon-animation-pattern-bindings":
        return False
    ...
    pattern_shape = re.compile(
        rf"kakulsaydon\.{re.escape(expected_profile)}\.pattern\.([1-9][0-9]*)"
    )
...
class KoukuSaydonAnimationPatternDocumentContractTests(unittest.TestCase):
    def _nonempty_document(self, profile="MN_RPCT_05"):
        ...
        pattern_id = f"kakulsaydon.{profile}.pattern.1"
        document = {
    ...
    def test_public_boundary_uses_predictable_members_and_transactions(self):
        ...
        for token in (
            "KOUKU_SAYDON_ANIMATION_PATTERN_CLIP",
            "strOccurrenceId",
            "strStageId",
            "strSlotId",
            "strRuntimeClip",
    ...
    def test_project_and_data_filters_register_the_new_contract(self):
        ...
        entries = (
            "KoukuSaydonAnimationPatternDocument.h",
            "KoukuSaydonAnimationPatternDocument.cpp",
            "MN_RPCT_05.patternbindings.json",
            "MN_RPCT_06.patternbindings.json",
    ...
    def test_animation_tool_exposes_all_profiles_and_pattern_workflow(self):
        ...
        for token in (
            ...
            "Render_KoukuSaydonPatternAuthoring",
            ...
            "CKoukuSaydonAnimationPatternDocument::Save_Atomic",
