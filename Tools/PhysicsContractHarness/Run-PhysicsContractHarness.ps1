...
$executable = Join-Path $PSScriptRoot "Bin\$Configuration\PhysicsContractHarness.exe"

if (-not (Test-Path -LiteralPath $executable)) {
    throw "PhysicsContractHarness was not built: $executable"
}
...
foreach ($directory in $runtimeDirectories) {
    if (-not (Test-Path -LiteralPath $directory)) {
        throw "Physics harness runtime directory is missing: $directory"
    }
}
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "PhysicsContractHarness failed with exit code $LASTEXITCODE"
    }
}
finally {
