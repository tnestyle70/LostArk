...
owner, including Pattern Sound and Effect V2 bindings.  KoukuSaydon remains
...
from Tools.KoukuSaydonPipeline import (  # noqa: E402
    build_kouku_saydon_animation_reference as kouku_saydon_animation,
)
...
KOUKU_SAYDON_ARENA_SOURCE_DOCUMENTS = {
    "BOSS_COMPOSITION": (
...
ARENA_CONTRACT_BY_BOSS_ID = {
    ...
    "boss.composition.kakulsaydon": {
        ...
        "sources": KOUKU_SAYDON_ARENA_SOURCE_DOCUMENTS,
    },
}
...
PRODUCT_RELATIVE_PATHS = {
    ...
    BOSS_AUTHORING[1]: "Bosses/KoukuSaydonGate1.bosscomposition.json",
    SEQUENCER_AUTHORING[0]: "Sequences/ValtanArena.sequencer.json",
    SEQUENCER_AUTHORING[1]: "Sequences/KoukuSaydonArena.sequencer.json",
}

# These names were generated by the pre-migration publisher. They are retired
# only after the replacement generation and its receipt are fully committed.
# A committed journal retries this cleanup on recovery, so a crash cannot leave
# consumers without either the previous or the new complete generation.
LEGACY_PRODUCT_RELATIVE_PATHS = (
...
def _kouku_saydon_identity(
    ...
    for index, row in enumerate(profiles):
        ...
        if profile_id in seen_profiles:
            raise CompositionError(f"duplicate KoukuSaydon profileId: {profile_id}")
        ...
        try:
            kouku_saydon_animation.validate_reference_document(reference)
            kouku_saydon_animation.validate_authored_document(action_binding, reference)
            kouku_saydon_animation.validate_pattern_document(pattern_binding, reference)
        except kouku_saydon_animation.BuildError as exc:
            raise CompositionError(
                f"{context} KoukuSaydon reference-only document is invalid: {exc}"
            ) from exc
...
def _validate_kouku_saydon_coverage(
    ...
    if coverage["kind"] != "KOUKU_SAYDON_ACTION_REFERENCE":
        raise CompositionError(
            "KoukuSaydon boss.coverage.kind must be KOUKU_SAYDON_ACTION_REFERENCE"
        )
    resolved, actual = _kouku_saydon_identity(root, coverage["profiles"], source_paths)
    expected = (
        _require_positive_int(
    ...
    if actual != expected:
        raise CompositionError(
            "KoukuSaydon composition coverage drift: "
            f"expected={expected} actual={actual}"
        )
    return resolved
...
def validate_boss_document(
    ...
    if composition_id == "boss.composition.kakulsaydon":
        if status != "REFERENCE_ONLY":
            raise CompositionError(
                "KoukuSaydon composition must remain REFERENCE_ONLY until gameplay Product exists"
            )
        if document["bossArchetypeId"] is not None or document["encounterId"] is not None:
            raise CompositionError(
                "KoukuSaydon must not invent boss/encounter IDs while REFERENCE_ONLY"
            )
        if document["areaId"] != "LV_LUT_MIDNIGHTC_ED":
            raise CompositionError("KoukuSaydon areaId must be LV_LUT_MIDNIGHTC_ED")
        if document["patterns"] != []:
            raise CompositionError(
                "KoukuSaydon patterns must remain empty while REFERENCE_ONLY"
            )
        profiles = _validate_kouku_saydon_coverage(root, coverage, set(source_paths))
        ...
        if sources != expected_sources:
            raise CompositionError(
                "KoukuSaydon source role/path closure is invalid: "
...
def _validate_camera_shot_source(
    ...
    """Validate the complete KoukuSaydon camera owner contract and linked sequence IDs."""
...
def _cleanup_committed_publish(
    ...
    for relative in LEGACY_PRODUCT_RELATIVE_PATHS:
        legacy, _stage, _backup = _transaction_paths(
    ...
    if errors:
        raise CompositionError(
            "composition Products committed, but legacy Product retirement is "
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "validate":
            ...
            kouku_saydon_profiles = validated[BOSS_AUTHORING[1]]["resolved"][
                "referenceProfiles"
            ]
            kouku_saydon_actions = sum(
                len(row["actionReference"]["actions"]) for row in kouku_saydon_profiles
            )
            print(
                "Composition validation passed: "
                f"Valtan patterns={len(valtan_patterns)}, "
                f"KoukuSaydon profiles={len(kouku_saydon_profiles)} "
                f"actions={kouku_saydon_actions}, "
