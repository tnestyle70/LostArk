...
분석 기준선: `origin/main@d6b27084` (PR #172)
...
현재 `main@d6b27084`에는 tracked DXBC가 176개다. 중앙 `Data/Effects/CookedShaders`의 171개와 Valtan
FrontBackFront evidence의 PS 5개이며, translated HLSL 169개도 함께 tracked다. 이는 PR #160~#166에서
DXBC oracle, literal HLSL translation, named ABI, child→parent resolution, family shader inventory를
차례로 merge한 결과다. **그러나 evidence coverage와 Product coverage는 여전히 다른 축이다.**
같은 main의 runtime catalog `EffectCatalog.runtime.json`(formatVersion 4)이 실제로 소비하는
material-program binding은 `1`개뿐이다. 숫자를 섞어 “DXBC 176개를 확보했으니 복원 완료”라고
판단하지 않는다.

### 5. 현재 main에서 정확히 어디까지 왔는가

기준: `origin/main@d6b27084` (PR #172). PR #156 이후 다음이 merge됐다.

| PR | 내용 | 축 |
|---|---|---|
| #160~#162 | family DXBC oracle, literal HLSL translation, named ABI evidence | evidence |
| #164, #166 | child→parent join key 전수 복구, occurrence-safe family shader inventory | evidence |
| #168 | **Binding 0 material program registry spine** (`CEffectMaterialProgramRegistry`) | runtime |
| #170 | tuple cohort evidence inventory (`effect-tuple-cohort-inventory.v1.json`) | evidence |
| #171 | focused sprite render contract harness | 검증 |
| #172 | **도화가 F sprite 1행 material adapter binding** | runtime |

즉 evidence lane은 크게 열렸고, runtime lane은 **정확히 1개의 binding**만 열렸다.

```text
authored occurrence          7,566
runtime catalog Product       2,554
material-program binding          1   ← effect.artist.skill.31470.unified / sprite.2b3dc6842507e910
compiled adapter                  1   ← SPRITE_PARTICLE alpha_two_sided (RT0 scene color, RT1 zero distortion)
compiled carrier                  1   ← SPRITE_PARTICLE
```

`Materialize_Binding`은 현재 adapterId=sprite, backend=`runtimeMaterialV2`, opcode=`6`,
executionVersion=`1`, `descriptor.layoutId == layout.layoutId`가 아닌 binding을 전부 거부한다.
따라서 Mesh와 Decal은 데이터가 아니라 **compiled adapter 자체가 없어서** 막혀 있다.

**그리고 이 binding은 아직 화면을 바꾸지 않는다.** `Effect_DocumentRenderer.cpp:15215-15235`가
registry Execution이 authored inline Execution과 bit-exact일 것을 요구한 뒤 그 동일한 값을 다시
대입하기 때문이다(오류 메시지도 `inline golden mirror`다). 실제 draw state는 여전히 inline
`Element.Material.eRenderProfile`이 `Select_Pass`(`:7556`)와 renderProfile switch(`:8000`)로
결정한다. 즉 **bound와 unbound의 staged 결과가 동일하며 registry는 현재 픽셀을 0개 바꾼다.**
이는 PR #168이 의도한 Binding 0 단계이지만, 결과적으로 **adapter enum이나 binding을 늘리기
전에 registry를 mirror에서 authority로 뒤집는 것이 선행 조건**이다.
