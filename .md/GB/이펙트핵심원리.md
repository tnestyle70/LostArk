occurrence가 복원 대상이고, carrier는 몸이며, family shader는 겉모습이고, ShaderMap은 배선이며, DXBC는 수학식이다. 이것들을 실제 timing에 결합하고 사용자가 봤을 때 비로소 Product 복원이다.

1. 어느 source occurrence인가?
2. Carrier는 정확한가?
3. Material program은 어느 fidelity인가?
4. Texture/CB/sampler 배선은 닫혔는가?
5. Tool-only인가 Product인가?
6. SOLO를 사용자가 봤는가?
7. 실제 스킬 timing/attachment를 사용자가 봤는가?

도화가 F는 완성된 결과의 기준
DXBC는 수학식의 기준
Carrier는 모양의 기준
Family는 재사용할 재질식의 단위
ShaderMap은 입력 배선의 기준
...
각 항목은 다음과 대응합니다.
질문	담당 증거
언제·어디서?	cue, timing, transform, attachment
어떤 모양에?	carrier
어떤 수학식으로?	DXBC/HLSL/material family
어떤 입력을 넣어?	ShaderMap, ABI, texture lane, CB
어떻게 합성해?	blend, depth, sampler, pass
...
앞으로의 복원 방향
= 스킬 하나를 전용 구현하는 것이 아니라 capability 하나를 공용화한다.
= Program은 수학식, Layout은 GPU 배선, Descriptor는 occurrence별 값, Adapter는 carrier draw이다.
= 같은 Program × Layout × Adapter tuple이면 Descriptor만 추가한다.
= equation이 다르면 Program, ABI가 다르면 Layout, carrier/VF/pass가 다르면 Adapter를 새로 연다.
= 수평 capability를 만들되 대표 occurrence 하나를 수직 canary로 끝까지 검증한다.
= capability 구현은 자동 Product 승격이 아니며 Composition·Admission·사용자 검증은 occurrence마다 닫는다.
= 도화가 F는 버리는 방향이 아니라 공용 구조로 승격할 golden control이다.

위 질문에 대한 대답을 바탕으로, 이펙트를 전수 조사해서 적용하고, 특정 effect를 기준으로 어떤 부분이 잘못되었는지 명확하게 파악할 수 있도록 한다.


핵심 병목
결론적으로 현재 핵심 병목은 DXBC 수량이 아니라 이것이야.
Material equation
× Runtime ABI packet
...
= 검증 가능한 occurrence
특히 가장 오래 걸리는 부분은:
1. texture 의미, channel, sampler, CB, DynamicParameter를 확정하는 ABI packet closure
2. 올바른 carrier, VF, pass, scene input, MRT에 연결하는 dispatch closure
DXBC는 이 과정의 강력한 oracle이지만 전체 복원 과정 자체는 아니야. 모든 DXBC를 먼저 정복하는 것보다, 중요한 family canary 하나의 수직 tuple을 끝까지 닫고 같은 tuple만 확장하는 방향이 맞아.
