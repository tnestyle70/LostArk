...
수정 뒤 발탄 진행 경로 8개 구간을 전부 다시 쟀다. **모든 구간에서 경로가 확보된다.**

| 구간 | 직선 | waypoint | 등반 |
|---|---|---|---|
| spawn → `player_Move.1` | 21.93m | 10 → **2** | 8.85 → 0.89 |
| (순간이동) → `Stage_1` | 2.83m | 2 → 2 | 0.15 → 0.15 |
| `Stage_1` → `Stage_MiniBoss` | 31.11m | 11 → **4** | 12.06 → 0.52 |
| `Stage_MiniBoss` → `Stage_2` | 50.63m | 22 → 21 | 23.11 → 17.66 |
| `Stage_2` → `Stage_3` | 11.31m | 8 → **4** | 9.32 → 5.74 |
| (순간이동) → `Stage_Boss` | 31.09m | 2 → 2 | 4.07 → 2.83 |
| `Stage_Boss` → `Stage_Boss_ArenaEntry` | 19.40m | 10 → 13 | 18.83 → 17.21 |
| (순간이동) → 보스 중심 | 9.58m | 1 → 1 | 0.34 → 0.17 |

여섯 구간은 개선되거나 동일하고 **두 구간이 남는다.**

### 7.1 남은 두 구간은 코드로 고칠 수 없다 — 위상 문제다

`Stage_Boss → Stage_Boss_ArenaEntry`의 경로 Y는 `23.1 → 11.2 → 25.1`로 아래층까지
12m 떨어졌다 14m 올라온다. 해당 셀은 이웃 중앙값과 `+13.106`, `+11.753` 차이다.
`Stage_MiniBoss → Stage_2`는 105셀 중 이상치가 1개뿐이라 성격이 다르다 — 지형 자체가
±2m로 굽이친다.

세 가지를 각각 시험해 코드 여지가 없음을 확인했다.

| 시험 | `Stage_MiniBoss → Stage_2` | `Stage_Boss → ArenaEntry` |
|---|---|---|
| 수직 비용 k 1 → 8 | wp 21 → 19, 최저 Y 불변 | wp 13 → 12, **최저 Y 11.16 불변** |
| 이상치 높이 1,960셀 보정 | wp 21 → 19 | wp 13 → 8, **최저 Y 11.19 여전히 아래층** |
| LOS 허용치 1m → 무한 | wp 21 → 10 | wp 13 → 6 |

수직 비용을 여덟 배로 올려도 최저 Y가 안 바뀐다는 것은 **위층만 밟는 대체 경로가
그리드에 존재하지 않는다**는 뜻이다. 높이를 보정해도 마찬가지다. 세 번째 줄만 숫자가
좋아지는데, 그것은 절벽을 가로지르지 말라는 안전장치를 없애는 것이라 해결이 아니다.

즉 이 두 구간은 XZ 셀당 높이 하나로 두 층을 표현하지 못하는 데이터 한계이며,
MapTool 재저작·재베이크 외에는 방법이 없다. Navigation authoring은 맵 담당 정본이고
`.md/GB/08-22` 세션의 되돌림 사례가 있으므로 이번 변경에서는 손대지 않았다.
