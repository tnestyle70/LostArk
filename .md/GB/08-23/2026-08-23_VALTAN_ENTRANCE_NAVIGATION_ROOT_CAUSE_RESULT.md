...
### 4.2 `Server/Private/ServerGameplayContractTests.cpp` — 회귀 계약 추가
...
| `Server.exe --contract-test` (수정 후) | 707 PASS, **failures : 0** |
| `Server.exe --contract-test` (수정 되돌린 뒤) | **failures : 1**, 새 항목이 FAILURE |
| `git diff --check` | 이상 없음 |
| 인코딩 | `ServerNavigation.cpp` ASCII/CRLF 유지, 테스트 파일 BOM·비ASCII 6바이트·CRLF 유지 |

수정을 되돌리면 새 테스트가 실제로 실패하므로 회귀 가드로 동작한다.
...
3. Bern에서 기존 이동감이 유지되는지 (경로가 평평해진 대신 최대 +0.9% 길어진다)
