# 차원술사 BA 2동작 자동 연계와 LMB/T 확정 입력 수정 구현 계획

## 1. 목표와 기준선

- 기준선은 `origin/main@1eb50f0bf9e33bd38267df41c336c50463b9e940`이다.
- 차원술사 기본 공격 `2050010`은 LMB 한 번으로 `_01 -> _03` 두 제품 동작만 자동 재생하고 `_04` stage에서 종료하던 기존 연결은 제거한다.
- 첫 동작은 `_01 source 0..1800ms / playRate 2.0`, 즉 `900ms` wall clock을 사용한다. 원본의 두 attack occurrence와 현재 BA2 Effect의 두 visible occurrence를 보존하고 첫 stage duration 숫자만 project-tune한다.
- Product Effect stable ID는 `_01 -> ba2`, `_03 -> ba3`만 연결한다. `_04 -> ba1` cue는 제품 LMB chain에서 해제하되 authored Effect 문서와 asset identity는 삭제하지 않는다.
...
이번 제품 계약은 원본 네 clip을 그대로 복제하는 것이 아니라, 사용자가 확정한 `두 번 찌르기 -> 휘두르기`만 두 Server stage로 표현하고 마무리 `_04`를 제품 chain에서 분리하는 project-tuned 구성이다.
...
## G00. 2-stage animation·Server timing·root motion

### 3. 제품 계약

| 제품 stage | 의미 | clip | wall duration | Server hit | advance/window |
|---|---|---|---:|---:|---|
| 1 | 두 번 찌르기 | `_01`, `playMs=1800`, `playRate=2.0` | `900ms` | `50ms` | automatic `900ms`, `0/0` |
| 2 | 휘두르기 | `_03` natural | `1067ms` | `28ms` | terminal `1067ms`, `0/0` |

총 animation wall time은 `1967ms`다. 첫 Effect 문서의 두 번째 visible element는 effect source `1.26s`, 2배속 wall `0.63s`에 시작하므로 `900ms` 전환 전에 발생한다. `stop=natural` tail은 `_03` 시작 뒤에도 자연 종료한다. non-final COMBO의 `inputOpenMs/inputCloseMs == 0/0`은 `comboAdvanceMs == actionDurationMs`인 automatic stage로 정의한다. 기존 non-zero window를 가진 다른 직업 COMBO는 계속 실제 추가 입력을 요구한다.

수정 경로는 다음 하나다.

- `DimensionMaster.skillbindings.json`: 두 group `_01(1800/2x)`, `_03`만 둔다.
- `PlayerSkills.json`: `2050010.comboStages`를 `900/1067ms` 두 행으로 축소하고 첫 행에 automatic `0/0`을 저장한다.
- `DimensionMaster.rootmotion.json`: source `_01 0..1800ms` curve를 2배속 wall clock으로 변환한 stage 0과 기존 `_03` stage 1만 둔다.
...
- Server contract는 단 한 번의 `Try_Start`가 stage `1 -> 2 -> NONE`을 만들고, stage 3이 생성되지 않으며 다른 직업 manual COMBO는 자동화되지 않음을 검증한다.

Server stage 수를 3에서 2로 줄이면 현재 fallback damage 발생도 세 번에서 두 번으로 바뀐다. 이번 요청은 animation/input/Effect 연결 범위이므로 별도 damage-profile 증폭이나 새 hit shape를 추측해 넣지 않고 결과 문서에 이 경계를 명시한다.

## G01. Product Effect stable cue

### 4. 연결만 수정

`DimensionMaster.animevents`의 Product cue는 다음 두 행만 둔다.
...
두 cue는 모두 `root / follow / action_facing / natural`이다. `ba1~ba4.unified` authored 문서, Element, delay, mesh, DDS, material, runtime payload는 수정하지 않는다. 사용자의 로컬 BA2 손튜닝도 stable ID가 같으면 첫 동작에서 한 번 재생된다. 제품 chain에서 분리되는 `_04 -> ba1`, 사용하지 않는 `_02`와 `ba4` 파일도 삭제하지 않는다.
...
- 다른 COMBO가 실행 중이어도 현재 snapshot의 cost/cooldown/stance를 만족하면 Server pending 계약과 같이 T preview/confirm을 허용한다. automatic BA에서는 2동작 전체 종료 뒤 Server가 다시 검증하고 실행한다.
...
- `test_dimensionmaster_2050010_stage_split.py`: 두 clip group, `900/1067ms` 2-stage timing, trimmed root motion, 두 stable cue와 `_04/ba1` 비연결.
- `test_player_skill_catalog_combo_timings.py`: automatic stage parse/rollback과 DM만의 automatic non-final 행.
- `test_effect_tool_buffered_combo_audition.py`: `_01 -> _03`, 두 번째 occurrence 누적 offset `900ms`, final tail.
...
1. LMB 한 번으로 `_01(약 0.9초) -> _03(약 1.067초)`만 자동으로 이어지고 `_04`가 재생되지 않는지 확인한다.
2. 첫 `_01`에서 현재 손튜닝한 `ba2`의 두 visible occurrence가 유지되고 `_03`에서 `ba3` cue가 한 번 시작하며 `ba1` cue는 나오지 않는지 확인한다.
