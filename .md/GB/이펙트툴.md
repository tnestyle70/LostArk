...
현재 구현은 `Published Effects`와 `Active Effect Elements`를 한 창에 섞어 놓았지만,
최종 툴에서는 다음 역할로 분리한다.
...
| Resource Browser | 선택한 슬롯에 넣을 `.wmodel` 또는 `.dds`를 썸네일 그리드로 선택 |
...
Sprite, Particle, Decal, Trail은 Mesh Shape를 제외하고 Material Input 카드만
표시한다. `.dds`는 실제 이미지 썸네일로 표시한다. `.wmodel`은 현재의 `WMODEL`
...
실제 파일도 `Data/Effects/Authored`에 저장돼 있다. 그러나 이것은 **원작 이펙트
11개를 정확히 추출해 복원했다는 뜻이 아니다.** 현재 열한 문서는 총 13개
Element와 네 개의 고유 물리 리소스만 사용한다. Mesh, Sprite, Particle, Decal,
Trail의 저장·로드·catalog·애니메이션 cue·월드 렌더 경로를 입증하기 위해 만든
기능 검증용 구성에 가깝다.

즉, 현재 상태는 다음과 같이 표현해야 정확하다.

- 기능 연결: 11/11
- 원작 파츠 추출: 아직 별도 파이프라인 없음
- 원작 시각 복원: 0/11에서 새로 검수해야 함
- 툴에서 수동 수정·저장하는 기반: 있음

---

## 5. 차원술사 T의 현재 상태와 최종 목표

현재 T의 `effect.dimensionmaster.skill.2050510.effect.json`에는
`thought_trail`이라는 Trail Element 하나만 있다.

현재 Base 텍스처는 다음 하나다.

```text
Effect/DimensionMaster/Textures/FX_TEX_00/fx_a_trail_005_cl.dds
```
...
따라서 지금 본 것은 “T 애니메이션에 원작 전체 이펙트가 복원된 모습”이 아니라
“T 애니메이션의 무기 anchor에 우리 Trail 문서가 정상 부착되어 재생되는 모습”이다.

T의 다음 목표는 원작 T가 실제로 사용하는 Effect graph를 찾아 각 노드를
Mesh/Sprite/Particle/Decal/Trail Element로 변환하는 것이다. 원작에 Mesh,
텍스처 애니메이션, Particle, 충돌 섬광, 바닥 Decal이 있다면 모두 같은
`effect.dimensionmaster.skill.2050510` 문서에 넣고, 현재 animation cue와
anchor를 유지한 채 파츠별 시작 시각과 수명을 조정한다.

현재 문서 포맷에는 다른 Effect Document를 자식으로 참조하는 중첩 Effect 기능이
없다. 그러므로 최종 T는 우선 필요한 Element를 한 문서가 직접 소유한다.
...
## 9. 차원술사 T로 바로 진행할 실제 순서

1. `2050510`이 참조하는 원본 스킬/Effect package를 확정한다.
2. ImportTable에서 직접 Mesh, Material Instance, Texture 의존을 수집한다.
3. Effect export/graph에서 node, timing, transform, color/UV curve를 복구한다.
4. 직접 의존 Mesh와 Texture를 `.wmodel`/`.dds`로 변환한다.
5. node마다 Element를 만들어 `effect.dimensionmaster.skill.2050510` 초안에 넣는다.
6. 현재 임시 `thought_trail`은 원본 node와 대응하면 값을 교체하고, 대응하지 않으면
   제거한다.
7. Effect Tool에서 Element를 하나씩 선택해 썸네일과 Detail을 조정한다.
8. 기존 T 애니메이션, 500 ms 시작, `b_wp_swm_m_1` anchor를 기준으로 Preview한다.
9. 원작 영상과 동일 시각을 나란히 비교해 시작/수명/크기/색/UV를 맞춘다.
10. 문서 전체를 `Data/Effects/Authored/effect.dimensionmaster.skill.2050510.effect.json`
    에 저장하고 authoring load와 실제 runtime 재생을 다시 검증한다.
...
│ Animation [pc_sp_m_00_sk_sk_dimensionalbreak_01 ▼]                │
...
├──────────────────── Resource Browser ──────────────────────────────┤
│ [Mesh] [Base] [Noise] [Mask] [Emissive] [Dissolve]               │
│ Category [DimensionMaster/Noise ▼]  Filter [___________________]  │
│                                                                    │
│ [thumb] [thumb] [thumb] [thumb] [thumb]                           │
│ [thumb] [thumb] [thumb] [thumb] [thumb]                           │
│ [thumb] [thumb] [thumb] [thumb] [thumb]                           │
...
│   Life Time      [ 1.500 ]                                        │
│                                                                    │
│ [Apply Element] [Revert Element]                                  │
└────────────────────────────────────────────────────────────────────┘
```

슬롯 카드를 누르면 해당 카드에 선택 테두리를 표시하고 Resource Browser를 그
role로 필터링한다. 리소스 썸네일을 누르면 선택 Element의 해당 binding만 바꾼다.
Effect Detail의 연속 수치는 좌우 drag 중 local draft를 기존 `CEffectObject`에
live-stage해 바로 비교한다. 이때 active Document는 바꾸지 않으며 Apply 뒤에만
완성 Document에 commit한다. Revert는 active Document 값을 다시 stage한다.
...
│                                                                    │
│ [New] [Save] [Save As] [Reload] [Discard]                         │
...
- 선택 슬롯에 따른 Resource Browser 필터와 binding
...
`Dimension Summon`은 Animation Tool에서 독립 선택·재생되지만 현재 Character F
runtime이 자동 생성하는 상태는 아니다. 최종 F 복원에서는 이 CModel을 스킬 pivot에
생성해 두 원본 clip을 재생하고, Effect Document가 Particle/Texture/Decal/Trail을
그 위에 겹친다. Summon의 네 Mesh와 bone animation을 Effect Mesh Element로 다시
만들거나 중복 렌더링하지 않는다.
...
현재 결과는 `88 LOD0 Emitter -> 83 변환 + 5 미지원 -> Burst 분할 후 97 Element`다.
Imported 문서는 Effect Tool에서 Load/Detail 편집할 수 있으며, Authored placeholder는
자동 교체하지 않는다. 다음 작업은 GPU Preview 눈 검수, Material slot 교정,
Dimension Summon animation cue 연결, 그리고 `Save As -> Authored` 확정이다.
