# G01 Flow 저장을 막는 플레이어 스킬 호환성

## 실제 원인과 수정 경계

Action Workbench/Boss Flow의 Save_FlowDocument는 BalanceTool의
Set_ValtanScriptedSequenceDraft와 Save_ValtanCanonicalProduct를 사용한다.
BalanceTool::Reload는 PlayerSkills도 읽는데, publisher가 허용하는 선택 필드
rootMotionScale을 거부한다. 현재 114개 중 5개 스킬에 1.5가 있어 초기 로드가 실패한다.

기존 저장 경로를 유지하고 선택 필드의 읽기/검증/직렬화부터 수정한다.
원본 PlayerSkills, 미저장 Flow, Server 상태는 변경하지 않는다.
rootMotionScale을 삭제하거나 unknown-field 검증을 제거하지 않는다.
발탄 전용 데이터 소유자 분리와 durable Flow recovery는 이번 긴급 호환성 수정의
완료로 보고하지 않는다. 실행 중인 구버전 Client에는 코드 변경이 적용되지 않는다.

## 파일과 책임

- Client/Public/BalanceTool.h: SKILL_EDIT에 선택적인 rootMotionScale을 보존한다.
- Client/Private/BalanceTool.cpp: IsExactObject의 명시적 선택 필드 검사,
  Reload의 publisher와 동일한 수치 검증, ValidateDraft 재검사,
  SerializeDocuments의 존재 여부/값 보존, 실패 메시지에 스킬 ID를 포함한다.
- 기존 C++ 파일만 수정하므로 project/filter 신규 등록은 없다.

## 검증

설치 데이터 114개의 필드/타입 검사, 5개 선택값 보존, 누락/잘못된 타입/범위/unknown
필드 거부를 검사한다. 가능한 C++ 컴파일을 수행한다. Client가 열려 있으면 링크나
종료를 강행하지 않는다. 사용자 순서를 확보하기 전에 재시작을 요청하지 않는다.
수정된 Client에서 Reload와 Save Flow 및 재로드 확인은 사용자 검증으로 남긴다.

## G02 사용자 스크린샷의 미저장 순서 복구

214802/214813 스크린샷의 연속 70행을 stable pattern ID로 전사한다.
33~36행 뒤는 100ms, 62~68행 뒤는 0ms, 나머지 전이는 1000ms다.
스크린샷의 마지막 행은 End이며, finite linear 순서로 검증된다.
`2026-09-22_VALTAN_FLOW_RECOVERED_70.json`은 최초 revision에 고정된 복구 patch이며
검증 후 기존 commit-canonical-draft 경로로만 반영한다. 이 파일을 최신 revision으로
자동 재작성하거나 이미 저장한 뒤 다시 적용하지 않는다.
# Client/Public/BalanceTool.h 전체 반영 코드
...
NS_END
```
# Client/Private/BalanceTool.cpp 전체 반영 코드
