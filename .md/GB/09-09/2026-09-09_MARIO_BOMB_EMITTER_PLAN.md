# G01. 마리오 2~4의 광대 얼굴 폭탄 반복 발사

## 현재 경계와 연결

사용자가 저장한 Mario2_Boom/1/2, Mario3_Boom/1/2, Mario4_Boom은 enabled=false/events=[]인 triggerBox다. 밟는 트리거가 아닌 위치 marker로 유지한다. Server 승인 iMarioStage 2~4에 맞는 marker만 재생한다. 이번 요청은 피해 없는 비행 표현이며 Server 피해·폭발·네비·플레이어 크기·기존 공 바운스는 변경하지 않는다.

저장 위치 정본은 Data/Worlds/LV_LUT_MIDNIGHTC_ED/Gameplay.world.json이다. 기존 Publish-WorldGameplay.ps1이 내보내는 Client/Bin/DataFiles/World/LV_LUT_MIDNIGHTC_ED.viewer.world.json을 제품에서도 읽는다. source 직접 fallback을 두지 않는다. 각 marker와 연결된 기존 마리오 진행선의 양끝 중 먼 쪽을 종점으로 정한다. marker의 XZ에서 종점 XZ로 수평 직진하며 marker의 Y를 바닥 기준으로 유지한다. halfExtents는 trigger 영역이지 모델 scale이 아니므로 모델은 기존 resource scale 1을 쓴다.

| 발사 marker | 진행선 진입 movePlayer | 진행선 끝 |
|---|---|---|
| Mario2_Boom | Mario2_go | Mario2_Trigger_2 |
| Mario2_Boom_1 | Mario2_Trigger_2 | Mario2_Trigger_4 |
| Mario2_Boom_2 | Mario2_Trigger_4 | Mario2_Trigger_7 |
| Mario3_Boom | Mario3_Trigger_4 | Mario3_Trigger_5 |
| Mario3_Boom_1 | Mario3_Trigger_4 | Mario3_Trigger_5 |
| Mario3_Boom_2 | Mario3_Trigger_8 | Mario3_Trigger_10 |
| Mario4_Boom | Mario4_Tigger_6 | Mario4_Tigger_7 |

## 파일과 책임

- Client/Public/Level_KakulSaydonArena.h: 발사 지점별 bounded 재생 slot과 전용 CWorldSequencePlayer 소유권 선언.
- Client/Private/Level_KakulSaydonArena_WorldObjects.cpp: published marker와 기존 광대 얼굴 공 show resource/motion을 검증한 뒤 소규모 메모리 재생 문서로 투영한다. 기존 WorldSequenceObject -> CModel/CMaterial 경로를 재사용한다. 원본 문서와 기존 시퀀스 player는 수정하지 않는다.
- Client/Private/Level_KakulSaydonArena.cpp: 기존 WorldSequence update 뒤 발사 시계를 샘플하고 Level 종료 시 모델/레이어를 먼저 해제한다.
- .md/TEAM/AREA_DATA_LAYER_GUIDE.md: marker 역할, 위치 저장/publish, 주기/속도/높이와 피해 없음 기록.

## 수치와 수명

기본 4000ms 간격, 3m/s, stable marker 이름의 FNV hash를 4000으로 나눈 나머지를 지점별 위상으로 사용한다. 발사마다 Server tick 기반 deterministic hash로 낮음/높음을 고른다. 기존 BottomCenter 키에 +0.05m 또는 +0.90m를 더해 1.5m 광대의 발/머리 부근에 맞춘다. 이는 실제 본 추적이나 gameplay 충돌 판정이 아니다. 같은 출생 tick의 폭탄은 동일 높이를 유지하며 카메라/현재 플레이어 점프 높이를 추적하지 않는다.

2026-09-09 사용자 피드백으로 기존 2초/6m/s에서 위 수치로 낮췄다.
기존 코드는 비행 위치만 바꾸고 show의 identity 회전을 복사해 모델 얼굴이 +X에 고정됐다.
실제 cooked ClownFaceBall의 코가 향한 축은 +X다. `Ready_MarioBombPresentation`에서
비행 XZ 차이로 `atan2(-deltaZ, deltaX)` yaw를 계산해 두 끝 key에 같은 quaternion을 쓴다.
회전하지 않는 world-up으로 얼굴이 비행 방향을 보고 마리오의 옆 카메라에 옆모습을 보인다.
show의 비대칭 모델 BottomCenter 위치 보정도 같은 quaternion으로 회전해 발사 중심을 유지한다.
자유 카메라 각도까지 고정하는 billboard가 아니며 카메라/비행 경로 자체를 수정하지 않는다.

한 지점 최대 32 slot, 비행 길이 최대 200m로 제한하고 실제 필요한 slot만 생성한다. 마리오 진입 이전 출생은 표시하지 않으며 stage 전환/사망/퇴장 시 Stop_All로 제거한다. 종점 도착은 slot Stop_Instance로 제거한다. 샘플 오류는 해당 발사 지점만 중지·로그를 남기고 나머지 표현/이동을 유지한다. 리소스/문서 초기화는 stage -> validate -> prepare -> commit으로 처리한다.

## 검증과 등록

기존 CPP/H만 수정하므로 vcxproj/filters에 새 항목은 없다. 관련 Client Debug 컴파일/링크, 기존 WorldGameplay publisher의 validate/publish, published marker 일치, 경로 길이·slot 상한·두 높이·시각별 endpoint/소멸 수치와 git diff --check를 확인한다. Client/UI 실행과 최종 화면 판정은 사용자가 한다. 전체 반영 코드와 독립 검토 결과는 아래에 갱신한다.

## G01 전체 반영 코드

### C:/Users/USER/source/졸업팀폴/LostArk/Client/Public/Level_KakulSaydonArena.h
...
### C:/Users/USER/source/졸업팀폴/LostArk/Client/Private/Level_KakulSaydonArena_WorldObjects.cpp
