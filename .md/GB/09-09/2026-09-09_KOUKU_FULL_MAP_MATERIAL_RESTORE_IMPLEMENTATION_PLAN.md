...
기준일: 2026-09-09. 입력 재실측: 2026-09-11. 현재 제품의 CModel → CMaterial 경로를 확장한다. 기존 바닥 복원 결과와 원본 shader cache의 실제 Base/Directional/Baked 분기를 근거로 삼는다. 사용자 placement ID·Transform·visibility와 기존 승인 바닥 2행을 보존한다. 09-11 사용자가 승인한 원본 RNM·환경·광원 연결은 같은 Area 조명 작업에서 진행한다.

## G00. 현재 실측과 입력 정본

- 09-11 적용 전 LV_LUT_MIDNIGHTC_ED catalog 330개 중 사용 asset 310개, 사용 material slot 384개, placement 3,368개(visible 2,917개). RNM별 catalog variant를 추가해도 placement ID·Transform·visibility는 유지한다.
- 기존 mapmaterials는 floor 2행과 diffuse sampler 3행이다.
- exact native material map 216/216개, DXBC 142개, Base+Light 조합 49개를 확보했다. texture가 없는 숨김 navigation cul01/02의 7개 slot 결과(6개 asset)는 별도 미지원 경계다.
...
비트는 normal=1, bump=2, specular=4, separate specular texture=8, reflection=16, world reflection=32, masked=64, COLOR0.a normal strength=128, simple=256, seamless=512이다. 원본 texture address의 diffuse mirror V=1024, normal mirror U=2048, normal mirror V=4096도 저장하며 diffuse mirror U는 기존 named sampler 상태를 재사용한다. 알 수 없는 비트와 specular 없이 specular texture, reflection 없이 world reflection은 거부한다. UV 회전은 원본 uv_rotate × 2.000000476837158의 sin/cos이며 중심 0.5를 유지한다. UV 이동은 원본의 고정 uv_move_x/y 값이며 시간 속도로 바꾸지 않는다. 원본 UV [1,-0.5]처럼 유한한 signed tiling을 허용하며 다른 기존 family의 양수 검증을 완화하지 않는다.
...
linear flicker는 `(2*(frac(time*speed)-0.5))²+minimum`, nested flicker는 원본 sin/cos 식을 유지한다. 원본 engine origin에 의존하는 phase/reflection offset은 명시적인 별도 입력이며 해결되지 않은 값을 복구 완료로 쓰지 않는다. 원본 texture sampler address와 sRGB는 source properties로 확인해 연결한다. flicker minimum은 원본 실측 2.0까지 있으므로 BG8에 기존 [0,1] 제한을 이식하지 않는다.
...
- Client/Private/MapAssetCatalog.cpp: formatVersion 2 mapmaterials의 `bg-source-opaque-masked`를 strict parse하고 기존 staged commit에 연결한다. `source.*` 원본 character/forward 행은 기존 SourceCharacterMaterial 파서·선택된 uniform packing과 texture mask를 재사용한다.
- Client/Private/MapPlacementRuntime.cpp: source-character family를 static instance batch에서 제외하여 Mario와 source forward가 실제 CMapAssetObject → CModel 경로를 사용하게 한다. Source forward 원본 BlendMode는 catalog의 Alpha/Sky/Additive로 연결한다.
...
216 native material 각각의 MIC 상속·default uniform·static switch·실제 texture register를 결합한다. 동일 body의 parameter만 다른 재질은 공통 표면 함수를 재사용한다. simple/seamless는 네이티브 분기를 비교해 BG8의 독립 bit로 선택한다. overlay는 기존 family7에 원본 UV tiling과 별도 S 입력을 연결한다. translucent/spotlight/sky는 선택한 nativePS33~37의 원본 별도 shader를 재사용하며 classic BG에 넣지 않는다.
