...
목표 선언: 게임 피직스의 본질을 실측 근거로 정리하고, 현재 프레임워크에 PhysX 5를 **어디에, 어떤 계약으로, 무엇을 위해** 세팅하는지 확정한다. 발탄 파괴 트리거 애니메이션에는 PhysX가 필수가 아님을 명시하고, PhysX의 실제 소비자(본체인 secondary motion, 이후 debris 연출)를 G 단위 로드맵으로 닫는다.
...
2026-08-05 기준 branch `codex/effect-tool-reboot` 실측이다. PhysX 참조는 저장소 전체에 0건이다.

| 항목 | 상태 | 근거 |
|---|---|---|
| 애니메이션 → 본 행렬 파이프라인 | 있음 | `CModel::Play_Animation()`이 채널 로컬 갱신 → 블렌드 → `m_Bones` 순회로 Combined 행렬 확정 (`Engine/Private/Model.cpp:301-323`) |
...
| 단순 충돌 질의 | 있음 | `CCollider`(AABB/OBB/Sphere) `Intersect()` 질의 + `_DEBUG` 와이어 렌더 (`Engine/Public/Collider.h`) — **Client 소비자는 현재 0건** |
...
- 프레임 루프: `CGameInstance::Update_Engine()`의 갱신 순서는 Picking → Input → Sound → Priority_Update → PipeLine → Frustum → Update → Late_Update → Level_Manager다 (`Engine/Private/GameInstance.cpp:104-124`).
...
단, 이 경로는 현재 **휴면 상태**다. 실측: `Load_DeployProps()` 호출자 0건, `Prototype_GameObject_DeployProp` 등록 0건, `Set_DeployPhase`의 유일 호출자는 로드 직후 INTACT 일괄 설정이며 FRACTURED를 만드는 UI/게임플레이 경로가 없고, MapTool 툴바가 "DeployProp authoring is excluded until its source/stage contract is complete"로 명시 제외 중이다. 발탄 파괴를 실제로 닫으려면 PhysX가 아니라 **이 재배선(prototype 등록 → 로드 호출자 → Server 페이즈 소비자)**이 선행 수직 슬라이스다.

PhysX는 그 뒤 "파편이 튕기고 굴러가고 잠드는" **optional 연출(G03)**에서만 쓴다. one-shot 파편 스폰 지점은 `Set_State`의 `m_State = state;` 직전, 이전 상태 INTACT → 새 상태 FRACTURED 가드다 (`Set_DeployPhase`가 상태를 일괄 재설정하므로 old-state 가드는 필수).
...
- 지금 세팅할 것: PhysX SDK 빌드/배치/링크/배포 계약(G00)과 고정 스텝 물리 서브시스템 + 최소 검증 소비자(G01). 이것이 "프레임워크가 피직스를 세팅해줘야 하나"의 답이며, 이후 캐릭터(G02)·맵(G03) 담당 슬라이스가 같은 서브시스템을 소비한다.
...
| # | 결정 | 이유 |
|---|---|---|
| S1 | PhysX SDK **5.6.1** (tag `107.3-physx-5.6.1`), `vc17win64` preset | VS2022 공식 preset 존재. 5.6.1은 공개 매뉴얼이 완비된 최신 버전(최신 태그 5.9.0은 문서 미공개) |
...
| S11 | 고정 스텝 펌프는 `Update_Engine`의 `Late_Update` 뒤, `Level_Manager->Update` 앞 | 오브젝트 Update에서 애니메이션이 확정된 뒤에 시뮬해야 G02(앵커 주입→시뮬→역주입)가 같은 자리에서 닫힌다. accumulator + 1/60 고정 스텝 + 프레임당 최대 스텝 상한(초과분 폐기 카운트) — `Timer_60` delta가 1/60 고정이 아니라는 실측 때문에 필수 |
...
| S14 | G01의 소비자는 F1 Developer Tools의 **Physics Tool**(drop test + 통계 + 와이어프레임) | 소비자 없는 서브시스템은 AGENTS.md 위반. drop test는 simulate/fetchResults/고정 스텝/레벨 정리의 실행 증거이고, 패널은 G02 branch 튜닝 UI로 이어진다. 디버그 드로우는 `CCollider`와 동일한 DirectXTK `DebugDraw`(`DX::Draw`) 경로를 재사용 — 두 번째 시각화 런타임을 만들지 않는다 |
...
| G02 | 본체인 secondary motion: `CModel` 훅 + branch solver + `physics.json` 로더 + Animation Tool branch 저작 | 별도 코드 PLAN | 4~5일 | 미착수 — 이 문서 §3.2가 설계 정본 |
| G03 | 발탄 debris 연출: DeployProp 재배선 슬라이스(선행, 물리 무관) 뒤 `Set_State` one-shot 파편 스폰 | 별도 코드 PLAN | 1~2일 | 미착수 — 선행 슬라이스 필요 |

합계는 08-03 문서 §4.4 견적(PhysX 통합 2~3일 + 저작 툴 2~3일 + 튜닝 2일 = 6~8일, P6 stretch/C컷)과 정합한다. G00-G01은 그 중 "통합" 절반의 선행 완료분이다.
