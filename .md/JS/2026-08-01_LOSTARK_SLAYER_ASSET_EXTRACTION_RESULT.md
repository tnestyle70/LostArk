...
> **2026-08-02 정정:** 여기 있던 "창술사 창만 x100이 필요하다"는 더 이상 맞지 않는다.
> 그 창 하나가 옛 추출 경로에서 나와 2.3 단위였던 것이고, 나머지 무기와 같은 방식으로
> 다시 뽑으니 233 유닛으로 나왔다. **이제 모든 무기가 예외 없이 `XMMatrixIdentity`**다.
> 판정 기준이 "바디와 같은 단위인지"라는 결론 자체는 그대로 유효하다.
> `2026-08-02_LOSTARK_CLASS_LOGIC_SKILLDATA_STANCE_RESULT.md` 참고.
