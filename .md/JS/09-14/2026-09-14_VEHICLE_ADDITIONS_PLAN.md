...
## G09. 2026-09-19 차원술사 탑승자 import 배율 교정

현재 본체 `pc_sp_m_00_sk`의 rest 및 155개 clip 6,244개 scale key는 1이다. 기존 Rider donor 6개는 같은 236개 bone 이름·부모를 가지지만 해당 armature rest와 39개 clip 3,352개 scale key에 import 배율 100이 남아 있다. `CModel::Attach_AnimationSet`은 donor rest/mesh/preScale을 복사하지 않고 animation만 target 본체에 붙이므로 탑승 시 100배 확대된다.

`Tools/VehiclePipeline/repair_dimensionmaster_rider_scale.py`는 본체의 unit basis와 등록된 6개 donor의 topology·고정 root scale을 검사한 뒤 해당 key의 XYZ만 100으로 나눈다. 현재 등록된 Horse/Swing/Hoverboard/HeavywalkerBm9/Tube/Dragon2만 대상으로 하며 미참조 Raptor는 변경하지 않는다. 도구는 원본을 out에 백업하고 후보를 검증한 후 교체 직전 hash에 해당하는 바이트 비교와 원자적 교체를 수행한다. 중간 실패 시 자신의 변경만 복원한다. C++/project/filter/서버 계약 변경은 없다.

실제 본체 palette에 교정 전후 donor clip을 적용해 skin bounds와 root basis를 비교한다. 위치·회전·시간·다른 bone·mesh·material·skeleton의 바이트 보존과 invalid/dynamic scale 거부, 재실행 무변경을 검사한다. 실행 화면은 사용자가 확인한다. Resources는 Runtime ZIP에 포함하지 않고 기존 경로의 6개 파일을 별도 Drive 전달한다.

```powershell
python Tools/VehiclePipeline/repair_dimensionmaster_rider_scale.py --out out/DimensionMasterRiderScale20260919 --install
python -m unittest discover -s Tools/VehiclePipeline -p test_repair_dimensionmaster_rider_scale.py
```

### repair_dimensionmaster_rider_scale.py 전체 코드
