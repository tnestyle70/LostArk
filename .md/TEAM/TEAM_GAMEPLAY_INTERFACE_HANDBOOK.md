...
`0.0.0.0`은 Server의 수신 주소일 뿐 Client 접속 주소로 사용하지 않는다. 현재 Client 기본값은 `192.168.0.14`다. 주소를 바꾸면 `Tools/Network/TeamLanEndpoint.json`, Server/Client 코드 기본값, 공유 debugger 설정과 이 사용서를 같은 변경 단위에서 갱신하고 `Sync-TeamLanEndpoint.ps1`, NetworkProtocolHarness, Server contract test로 검증한다.

#### pull 후 공유 Server에 들어가는 순서

Server PC와 Client PC는 먼저 같은 commit과 생성 데이터를 맞춘다. 기능 브랜치를 검증할 때도 양쪽이 같은 변경을 사용해야 한다. `pull`만 하고 예전 실행 파일을 쓰면 protocol v46 또는 Debug gameplay revision이 달라 Server가 연결을 종료할 수 있다. v45 이하 빌드는 v46과 호환되지 않는다.
...
포함된다. active roster의 각 `peer`를 Client Lobby의 local endpoint와 대조해 실제 사용자에게 없는
endpoint가 있을 때만 stale/ghost 후보로 판정한다. `lastInboundAgeMs`는 heartbeat가 없는 계약에서 건강한
idle Client도 커질 수 있으므로 단독 ghost 증거가 아니다. Bern은 4개, Valtan은 8개 active seat가 모두
실제 접속자와 일치하는지 확인한 뒤에만 다음 entry를 정상 capacity rejection으로 판정한다.
...
F1 진입 이름은 `Action Presentation Workbench`다. 기존 Character clip/event/skill binding 편집을 유지하면서 Valtan의 Server stage, animation occurrence, Product Effect, combat-object hit clock, Sound event와 실제 WAV variant를 stable ID로 join해 한 화면에 표시한다. `Clip Preview`는 기존 model preview, `Pattern Offline`은 Product animation의 로컬 sample, `Server Replay / Live`는 `CBossTool` typed API를 통한 실제 Valtan Arena authority 실행이다. stage의 빈 구간은 `patternId/stageId`로 편집한다. 사용자에게 보이는 저장 명령은 `Save` 하나이며 내부에서 strict join 검증, authoring 저장, Product 생성, 가능한 live revision 적용을 순서대로 실행한다. Sound binding draft가 남아 있으면 reload를 막고 먼저 같은 Save를 요구한다. 내부 단계 이름을 별도 authoring 버튼으로 노출하지 않는다.

Workbench의 `Animation Sequence Intake`는 review source일 뿐 Product pattern이나 Server 실행 정본이 아니다. KakulSaydon도 Product world/encounter/pattern/Server runtime admission이 끝나기 전에는 Workbench 실행 대상으로 위장하지 않는다.
