...
Server PC와 Client PC는 먼저 같은 `origin/main` commit과 생성 데이터를 맞춘다. `pull`만 하고 예전 실행 파일을 쓰면 protocol v39 또는 Debug gameplay revision이 달라 Server가 연결을 종료할 수 있다.
