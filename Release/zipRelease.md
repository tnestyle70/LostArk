...
2026-09-19 v6 배포 기준. **사용자의 VS Release 빌드를 기준으로 v6 ZIP 생성·파일 검증을 완료했다.**
실제 4인 화면·음향·접속 안정성은 이 배포본으로 사용자 재검증한다.

v6는 압축을 푼 폴더의 EXE·DLL·컴파일된 shader와 Data/DataFiles를 사용한다.
...
| 항목 | 값 |
|---|---|
| ZIP | `LostArk-Release-20260919-v6-192.168.0.14.zip` |
| 위치 | 저장소의 `Release` 폴더 |
| 상태 | 사용자 VS Release 산출물 pin·ZIP CRC/전체 파일 hash·사전검사 PASS |
| ZIP 크기 | **101,441,652 bytes** (약 101.4 MB) |
| ZIP SHA256 | `bb0ec4fec11845ea3ebd13aa21a14d7d54fcfe7a43e373edc5f8598f490e3f54` |
| 구성 | Release x64 |
| 서버 접속 주소 | `192.168.0.14:7777` |
| 서버 bind | `0.0.0.0:7777` |
| 현재 게시 데이터 기준 | Action revision **1753**, Sequence revision **65**, protocol **93** |

[v6 ZIP 열기](C:/Users/user/Desktop/LostArk/Release/LostArk-Release-20260919-v6-192.168.0.14.zip)
...
사용자의 VS Release 빌드에서 캡처한 Client/Server/Engine hash와 ZIP이 일치한다. Server는 기존 v5와 다른 바이너리다.
새 Product pipeline 실행 결과로 기록하지 않으며, 빌드 출처는 아래 검증 기록에서 구분한다.
...
`ChangedData` 폴더, 중첩 Runtime ZIP, 기존 저장소를 변경하는 설치기는 v6에 넣지 않는다.
...
- v6 폴더에 `Framework.sln`, Git checkout, Visual Studio 빌드는 필요하지 않다.
- 공유 서버 PC도 같은 v6의 새 Server와 게시 데이터를 사용해야 한다.
...
1. 압축을 푼 v6 폴더의 `LostArk.exe`를 실행한다.
2. Resources가 있는 기존 LostArk 폴더를 선택한다. `Client/Bin/Resources`를 자동으로 찾으며, Resources 폴더 자체를 선택해도 된다.
3. 패키지 검증이 끝나면 **v6 폴더 안의 `Client/Bin/Release/Client.exe`**가 시작된다.
4. 서버 PC에서는 기존 Server를 종료한 뒤 v6 폴더의 `ServerHost.cmd`를 한 번 실행한다. 다른 PC에서는 Client만 실행한다.

선택한 기존 폴더의 EXE·DLL·Data는 사용하거나 교체하지 않는다.
반복 검증도 v6의 `LostArk.exe`로 실행한다. v4의 설치 바로가기는 기존 저장소 바이너리를 실행하므로 v6 실행 방법과 구분한다.
서버 메모리의 데이터는 파일 게시만으로 갱신되지 않으므로 새 Server로 시작해야 한다.

| 대상 | 사용 경로·설정 |
|---|---|
| Client EXE / 작업 폴더 | v6의 `Client/Bin/Release/Client.exe` / `Client/Default` |
| 직접 소비 Data | `LOSTARK_PROJECT_DATA_ROOT` = v6의 `Data` |
| 외부 Resources | `LOSTARK_RESOURCE_ROOT` = 선택한 Resources |
| Client 서버 주소 | `LOSTARK_SERVER_HOST` = `192.168.0.14` |
| Server EXE / 작업 폴더 | v6의 `Server/Bin/Release/Server.exe` / `Server/Default` |
...
Client/Server를 시작하지 않고 경로와 파일만 확인하려면, v6를 푼 폴더에서 다음을 실행한다.
...
`P`는 패턴, `B`는 여러 패턴을 묶은 번들이다. v6에 포함된 현재 게시 데이터 기준이다.
...
문제가 난 Client PC와 Server PC 각각에서 **실제로 실행한 v6 폴더**를 대상으로 수집한다.
아래 명령은 v6를 푼 폴더에서 실행한다.

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\Collect-RuntimeDiagnostics.ps1 -RuntimeRoot .
```

결과는 해당 v6 폴더의 `out/DiagnosticBundles/<시각>-<ID>.zip`과 같은 이름의 폴더에 생성된다.
...
v6 실행기 자체의 실패는 오류창 또는 `--check` receipt에 표시하며 별도 LauncherLogs 파일은 만들지 않는다.

## 6. 검증 상태와 이전 배포본

v6에는 Server의 terminal 송신 종료 처리와 Client 레이드 준비·Server heartbeat 진단을 반영했다.
준비 단계의 1인·4인 CPU 검사 14개는 PASS지만, 실제 PREPARING 직후 중단 원인은 아직 확정하지 않았다.
화면 증상이 해결됐다는 판정은 새 진단 기록과 사용자 실행 결과로 확인한다.

사용자의 VS Release 산출물과 Client/Server/Engine hash 일치, ZIP CRC·전체 manifest 파일 hash,
Resources/PNG/ChangedData 0개, V2 JSON 324개와 초기 필수 UI JSON 포함 검증을 통과했다.
완성된 v6 폴더의 `--check`도 실제 외부 Resources 경로와 bundle EXE·Data·작업 폴더를 확인했다.
...
| 근거 | 파일 |
|---|---|
| 사용자 VS Release 빌드 캡처 | `out/KoukuRaidPreparing20260919/visual-studio-build-capture.json` |
| 최종 EXE/DLL pin | `out/KoukuRaidPreparing20260919/release-ready.receipt.json` |
| ZIP 생성 결과 | `out/KoukuRaidPreparing20260919/portable-delivery.receipt.json` |
| 독립 ZIP 재검증 | `out/KoukuRaidPreparing20260919/portable-final-verification.receipt.json` |
| 실제 bundle 사전검사 | `out/KoukuRaidPreparing20260919/v6-portable.preflight.json` |

기존 v5 ZIP도 그대로 보존한다: `LostArk-Release-20260919-v5-192.168.0.14.zip`, 101,394,117 bytes,
SHA256 `b6e956b7c106843e34313efcb9ee61780133b68f7439c44c332f9e5b24d04f36`.

기존 v4는 저장소에 설치하는 별도 배포본으로 보존한다. v4에는 G 키캡 PNG 1개가 포함됐으나 v6는 모든 PNG를 제외한다.
v4에서 완료한 Release 빌드·Sequence 65 게시·미로 대기·이펙트 예산·Billboard·진단 검증을 v6 최종 검증으로 대신 기록하지 않는다.
...
이 문서 갱신은 기존 v4/v5 ZIP 내용과 SHA256을 변경하지 않는다.
