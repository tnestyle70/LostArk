...
Git 제외 `Client.vcxproj.user`를 `LOSTARK_SERVER_HOST=127.0.0.1`로 갱신한다.
...
- 공유 LAN Server는 2026-08-17 팀장 결정으로 폐지했다. 각자 자기 PC에서 Server를 띄우며 Server listener 기본 bind와 Client 기본 endpoint는 모두 `127.0.0.1:7777`이다. endpoint 정본은 `Tools/Network/TeamLanEndpoint.json`이고 `Framework.slnLaunch`의 `Server + Client` profile과 공유 x64 debugger 설정도 같은 계약을 사용한다. `LOSTARK_SERVER_HOST`로 Client endpoint를 명시하면 그 값을 우선하며 `0.0.0.0`은 Client 접속 주소로 사용하지 않는다. loopback endpoint는 만료 검사와 방화벽 규칙을 요구하지 않는다. 다시 공유 Server를 쓰게 되면 endpoint 문서에 concrete IPv4와 bind `0.0.0.0`을 넣고 Server/Client 기본값, 공유 debugger 설정, 팀 사용서와 Network/Server 실행 검증을 같은 변경 단위에서 갱신한다.
