...
Server는 fixed 30 Hz에서 world entity의 transform/action/pattern state를 소유하고 현재 Shared protocol v99 snapshot으로 보낸다. Debug Next Pattern을 live Product/같은 owner Flow/idle에서 채택하는 typed command와 기존 예약·취소 CAS identity/lifecycle을 유지한다. Complete Play와 Restart는 현재 Server-active gameplay definition revision을 wire에 포함해 exact CAS하며, 다른 protocol version의 Server/Client를 섞어 실행하지 않는다. Client의 `CClientReplication`과 `CValtan`은 표현만 담당한다. UI·MapTool·Client GameObject가 제품 보스 판정을 직접 결정하지 않는다.

Kouku의 `Book1_Monsters`/`Book2_Monsters`와 Valtan의 `Stage_1`/`Stage_2` 웨이브 트리거(각각 `spawn.kouku.book1`/`spawn.kouku.book2`, `spawn.valtan.stage01`/`spawn.valtan.stage03`)는 Release Server에서 지금처럼 밟으면 그룹이 시작된다. Debug Server는 이 네 상자를 밟아도, G를 눌러도 실행하지 않고 안내도 보내지 않으며, F1 `KoukuSaydon Arena`의 `Bingo Board` 아래 `Normal Monster 1/2`와 발탄 아레나 안에서만 보이는 `Valtan Arena`의 같은 이름 버튼이 typed 명령 `C2S_DEBUG_RESUMMON_WAVE_MONSTERS`(protocol 99)를 보낸다. Server는 `CServerTriggerSystem::Find_WaveMonsterButton`의 고정 표로만 (world, button)을 그룹에 연결하고, 그 그룹의 살아 있는 monster를 제거한 뒤 그룹을 초기화해 첫 wave부터 다시 시작한다. monster는 그룹의 저작 anchor에 나타나며 플레이어 위치와 무관하다. 결과 메시지는 없고 monster는 world snapshot으로 온다. Release는 이 명령을 무시한다. `Stage_MiniBoss_Spawn`, `Stage_3`, `Stage_Boss`와 다른 월드의 트리거는 Debug에서도 그대로 동작한다. Debug/Release 구분은 컴파일 시 `_DEBUG`이고 Server/Client는 같은 protocol 99로 함께 빌드·재시작한다. 이 브랜치의 protocol 99는 origin/main의 99(`f291f886`, 무적 구역 연출 펄스)와 다른 wire다. main을 병합할 때 `PacketType.h`의 버전 줄이 충돌하며 병합 결과의 올바른 번호는 100이고, 이 문서의 "protocol 99"와 새 contract test도 그때 함께 맞춘다.
...
탈것은 `Data/Actors/VehicleCatalog.json`(Client 표현: 모델·좌석 본·탈것 idle/run·직업별 탑승자 클립·원작 재질)과 `Data/Vehicles/VehicleProfiles.json`(Server 이동 속도, `EFTable_Vehicle.MoveSpeed` cm/s ÷ 100)이 정본이다. `Tools/GameplayPipeline/Publish-VehicleProfiles.ps1 -Mode Publish`가 `Server/Bin/DataFiles/Vehicles/Vehicles.bootstrap`을 만들고 `CGameRoom`은 이 파일 없이 준비되지 않는다(`Invoke-BuildDomainOwner.ps1 -Owner Server`의 `vehicles.profiles`). H 키가 protocol 82의 `C2S_SET_VEHICLE_RIDING`으로 탑승/하차를 요청하고, Server는 Bern·Character Select에서만 허용하며 사망·강제 행동·광대·마리오·패턴 속박 등에서는 매 tick 강제 하차시킨다. 탑승 중 캐릭터 스킬·에스더 요청은 거부되고 다른 월드 입장은 도보로 시작한다. 탑승 중 R은 하차, Space/Q/W/E는 `EFTable_Vehicle.MovingSkill/SkillId0~2`의 탈것 스킬을 기존 `C2S_USE_SKILL`로 요청한다. 스킬 정의(slot·`EFTable_Skill.Cooltime`·행동 길이·PSA `b_root` 루트 이동 샘플)는 `Tools/VehiclePipeline/build_vehicle_skills.py`가 `VehicleProfiles.json`/`VehicleCatalog.json` formatVersion 2에 기록하고 bootstrap v2의 `VEHICLESKILL` 행으로 publish한다. Server는 protocol 85 `PLAYER_ACTION_STATE::VEHICLE_SKILL`로 쿨타임·이동 잠금·루트 이동(navigation clamp·collision)을 소유하고, Client는 action age로 탑승자·탈것 클립 체인을 seek한다. 파티클·사운드·버프·HUD는 없다. Client `CCharacter`는 snapshot의 `iVehicleId`로 `CPart_Vehicle`을 붙이고 좌석 본(`b_cockpit`) 위치를 표현 루트에만 더하며, 창술사·워로드·도화가·차원술사는 `Character/<Class>/AnimSets/<Class>_RideHorseAnimSet.wmodel`의 `ride_horse_*` 클립을 사용한다. 황금 테르페이온(6705) 몸체는 SourceCharacter program 85, 장식은 24, 갈기는 18이다. 고요한 별빛의 가호(9370, 몸체 86·반투명 외피 88), 레인보우 모코보드(7209, 87), 아우프슈텐-R(8302, 85), 바다 유니콘 튜브(8906, 24), 고대의 신화(9524, 89)가 추가돼 6종이며 직업별 `…_Ride{Horse,Swing,Hoverboard,HeavywalkerBm9,Tube,Dragon2}AnimSet.wmodel`을 쓴다. 은색 전투 랩터(7104)는 외형 문제로 제외했다. SourceCharacter program을 새로 설치할 때 CSO 변형 그룹 경계(`Engine/Private/Shader.cpp`, `Tools/EffectPipeline/native_shader_dispatch.py`, 현재 마지막 그룹 84~89)와 `Model.cpp` 상한을 함께 늘린다. 탑승 자세는 `EFTable_Vehicle.RidingMode`를 원본 `ActionConditionRidingModeOutput` enum 순서로 읽는다. 탈것 파츠의 반투명 원작 재질(program 18 양면·88 단면)은 BLEND 그룹에서 forward로 그린다. H가 탈 탈것은 Debug F1 `Vehicle Riding (Debug)`에서 고르며, 선택이 없거나 직업 탑승 자세가 없으면 catalog 첫 탈것을 쓴다. Debug는 Bern·Character Select Loader에서 탈것 모델을 미리 읽지 않고, Server snapshot의 첫 탑승 ID에 대해 기존 `CVehiclePresentationAssetService`로 필요한 모델·재질만 동기 준비한다. 같은 Level의 재탑승은 prototype을 재사용하고 Level 재진입 시 캐시를 초기화한다. 최초 탑승은 모델 로딩 시간을 부담하며 탑승자 animation set은 기존 캐릭터 bundle에 유지한다. Release는 기존 진입 선로딩을 유지한다.
...
`HAMMER_HIT_TICK_OFFSET`이며 실측 스윙에 맞춘 30틱(1.0초)이다. 이 배치는 현재 protocol 85를 쓰므로
...
사용하려면 gameplay bootstrap v26과 현재 protocol v99의 Server/Client를 함께 빌드·배포해야 한다.
