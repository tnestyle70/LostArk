...
F1의 기존 Animation Tool 진입은 `Action Presentation Workbench`로 확장됐다. Character animation/event/skill binding 기능은 그대로 유지하며, Valtan 화면은 최소 `1180x760`의 Master/Preview/Persistent Detail, full-width Sequencer, Data Files 작업공간에서 같은 `CBalanceTool` draft와 `CBossTool` Server audition controller를 조합한다. `VALTAN_HIGH_JUMP/AIRBORNE`를 포함한 stage의 `Server wall / blank timeline ms`, `RELEASE_GRABBED_PLAYERS`의 speed/duration/yaw, paired Counter/Groggy edge는 stable `patternId/stageId/actionId`로 편집한다. Counter enable은 counterable ENTER/EXIT, `COUNTER_HIT` branch, 같은 pattern의 GROGGY stage와 groggy ENTER/EXIT가 모두 유효한 Server-authority 단위일 때만 저장한다. Animation row는 `Valtan.patternbindings.json`, Pattern Sound row는 `Valtan.patternsoundcues.json`의 실제 clip occurrence owner를 사용한다. Pattern Sound는 exact clip occurrence와 검증된 event로 row를 추가하고 exact `bindingId + occurrenceId`로 제거하며 deterministic stable ID와 원자 CAS Save를 사용한다. Effect/Camera/World는 각 owner Tool의 stable deep-link를 사용한다. Animation/Pattern Sound 저장은 Effect/Sound/Shake/Encounter/Animation dependency bytes와 실제 model clip duration을 함께 검증하고, Development preview가 아니라 active Server-replicated primary Valtan의 joined presentation cache를 transactionally reload한다. reload 실패는 ordinary despawn 뒤에도 freshness gate에 남고, 성공한 reload/spawn 또는 world reset 전에는 Complete Play를 차단한다. Gameplay Save 뒤에도 최신 Product candidate가 실제 Server-active revision이 되기 전까지 Complete Play를 차단한다. `Pattern Offline`은 animation만 로컬 sample하고 Complete Play는 실제 Arena authority에 pattern ID를 제출한다. `Animation Sequence Intake` 저장은 `Valtan.presentation.debug.json` 원본만 갱신하며 reviewed promotion 전에는 Product pattern이 아니다.
