...
탈것은 `Data/Actors/VehicleCatalog.json`(Client 표현: 모델·좌석 본·탈것 idle/run·직업별 탑승자 클립·원작 재질)과 `Data/Vehicles/VehicleProfiles.json`(Server 이동 속도, `EFTable_Vehicle.MoveSpeed` cm/s ÷ 100)이 정본이다. `Tools/GameplayPipeline/Publish-VehicleProfiles.ps1 -Mode Publish`가 `Server/Bin/DataFiles/Vehicles/Vehicles.bootstrap`을 만들고 `CGameRoom`은 이 파일 없이 준비되지 않는다(`Invoke-BuildDomainOwner.ps1 -Owner Server`의 `vehicles.profiles`). H 키가 protocol 82의 `C2S_SET_VEHICLE_RIDING`으로 탑승/하차를 요청하고, Server는 Bern·Character Select에서만 허용하며 사망·강제 행동·광대·마리오·패턴 속박 등에서는 매 tick 강제 하차시킨다. 탑승 중 스킬·에스더 요청은 거부되고 다른 월드 입장은 도보로 시작한다. Client `CCharacter`는 snapshot의 `iVehicleId`로 `CPart_Vehicle`을 붙이고 좌석 본(`b_cockpit`) 위치를 표현 루트에만 더하며, 창술사·워로드·도화가·차원술사는 `Character/<Class>/AnimSets/<Class>_RideHorseAnimSet.wmodel`의 `ride_horse_*` 클립을 사용한다. 황금 테르페이온(6705) 몸체는 SourceCharacter program 85, 장식은 24, 갈기는 18이다. 은색 전투 랩터(7104, program 23/25), 고요한 별빛의 가호(9370, 몸체 86·반투명 외피 88), 레인보우 모코보드(7209, 87)가 추가돼 4종이며 직업별 `…_Ride{Horse,Raptor,Swing,Hoverboard}AnimSet.wmodel`을 쓴다. 탈것 파츠의 반투명 원작 재질(program 18 양면·88 단면)은 BLEND 그룹에서 forward로 그린다. H가 탈 탈것은 Debug F1 `Vehicle Riding (Debug)`에서 고르며, 선택이 없거나 직업 탑승 자세가 없으면 catalog 첫 탈것을 쓴다. Debug는 Bern·Character Select Loader에서 탈것 모델을 미리 읽지 않고, Server snapshot의 첫 탑승 ID에 대해 기존 `CVehiclePresentationAssetService`로 필요한 모델·재질만 동기 준비한다. 같은 Level의 재탑승은 prototype을 재사용하고 Level 재진입 시 캐시를 초기화한다. 최초 탑승은 모델 로딩 시간을 부담하며 탑승자 animation set은 기존 캐릭터 bundle에 유지한다. Release는 기존 진입 선로딩을 유지한다.
