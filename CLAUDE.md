...
cleanup으로 처리해 queue 포화와 close/entry race가 player slot을 남기지 않게 한다. Bern은 4개,
Valtan은 8개의 network player seat를 사용하고 party roster 상한은 두 world 모두 4다. capture 위치와
`ROOM_FULL` 판정 절차는 팀 handbook의 `Lobby fallback 진단과 room capacity 대조`를 따른다.
