...
public:
	...
	/* Opens the one large read-only Valtan Logic Pattern window.  It reuses this Boss
	...
	/* Integrated Workbench route.  The caller supplies only the stable Product
	   pattern identity; Valtan Boss Tool re-resolves its current Server-audition
	...
	/* Read-only projection of the process-wide Server verdict/lifecycle for the
	   exact Valtan Boss Tool pattern request.  A different consumer or Pattern ID is
