// Original Kouku material programs 2560..2623; native IDs and expressions are unchanged.
...
// fx_k_pa_ninjaflow_01_03_tr: 28c8301200413e449da688a3c218b53c; selected map 62be4dcdda7937113edbd3b48a4322f45053dc21e06b9e86b6d749eee5f67d95.
float4 ArtistNative2560(ARTIST_NATIVE_INPUT input)
{
...
float4 ArtistNative2561(ARTIST_NATIVE_INPUT input)
{
    float4 source[9]; [unroll] for (uint i=0u; i<9u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
float4 ArtistNative2562(ARTIST_NATIVE_INPUT input)
{
...
float4 ArtistNative2563(ARTIST_NATIVE_INPUT input)
{
...
float4 ArtistNative2564(ARTIST_NATIVE_INPUT input)
{
...
float4 ArtistNative2565(ARTIST_NATIVE_INPUT input)
{
...
float4 ArtistNative2566(ARTIST_NATIVE_INPUT input)
{
...
float4 ArtistNative2567(ARTIST_NATIVE_INPUT input)
{
...
float4 ArtistNative2568(ARTIST_NATIVE_INPUT input)
{
    float4 source[4]; [unroll] for (uint i=0u; i<4u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
float4 ArtistNative2569(ARTIST_NATIVE_INPUT input)
{
    float4 source[3]; [unroll] for (uint i=0u; i<3u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
#ifndef ARTIST_NATIVE_MODEL_ONLY
// fx_e_pa_fi_05_1_tr: 125c17fdb0c9054ba5e06631ff28236a; selected map 0294b748630ab75cf1b73903839a03fe02a7b6a955fba31808c71eb70346be87.
float4 ArtistNative2570(ARTIST_NATIVE_INPUT input)
{
    float4 source[3]; [unroll] for (uint i=0u; i<3u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
#ifndef ARTIST_NATIVE_MODEL_ONLY
// fx_i_fire_02_ad: fbd35674b7d7244cbb5a4921df27bf95; selected map f015544c73bbc4ac4a91bb5b070544a27b75f690d3b746cf064e1ca67b0b0039.
float4 ArtistNative2571(ARTIST_NATIVE_INPUT input)
{
    float4 source[3]; [unroll] for (uint i=0u; i<3u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
    ...
    // 12: div_sat r0.x, r0.x, r0.y
    r0.x = (saturate((r0.xxxx)/(r0.yyyy))).x;
    // 13: sample_b_indexable(texture2d)(float,float,float,float) r1.xyzw, v2.zwzz, t0.xyzw, s1, l(0.000000)
    r1.xyzw = (ArtistNativeSample0((v2.zwzz).xy, (float4(0.000000,0.000000,0.000000,0.000000)).x, false).xyzw).xyzw;
    // 14: sample_b_indexable(texture2d)(float,float,float,float) r2.xyzw, v2.xyxx, t0.xyzw, s1, l(0.000000)
    r2.xyzw = (ArtistNativeSample0((v2.xyxx).xy, (float4(0.000000,0.000000,0.000000,0.000000)).x, false).xyzw).xyzw;
    // 15: add r1.xyzw, r1.wxyz, -r2.wxyz
    r1.xyzw = ((r1.wxyz)+(-(r2.wxyz))).xyzw;
    // 16: mad r1.xyzw, v0.wwww, r1.xyzw, r2.wxyz
    ...
    // 30: mul r0.yzw, r1.yyzw, cb0[2].xxxx
    r0.yzw = ((r1.yyzw)*(source[2].xxxx)).yzw;
    ...
    // 32: mad r1.xyz, -cb0[2].xxxx, r1.yzwy, r1.xxxx
    r1.xyz = ((-(source[2].xxxx))*(r1.yzwy)+(r1.xxxx)).xyz;
    // 33: mad r0.yzw, cb0[2].yyyy, r1.xxyz, r0.yyzw
    r0.yzw = ((source[2].yyyy)*(r1.xxyz)+(r0.yyzw)).yzw;
    // 34: mad r0.yzw, r0.yyzw, v3.xxyz, cb0[1].xxyz
    r0.yzw = ((r0.yyzw)*(v3.xxyz)+(source[1].xxyz)).yzw;
...
float4 ArtistNative2572(ARTIST_NATIVE_INPUT input)
{
    float4 source[3]; [unroll] for (uint i=0u; i<3u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
#ifndef ARTIST_NATIVE_MODEL_ONLY
// fx_f_pa_wind_06_tr: 152c850a31af0046b921a13c9e7817b7; selected map 5cfc24af8ed6d907b8de227576bd01a21c475e1b2fdcda2af1235b20c41cfdd7.
float4 ArtistNative2573(ARTIST_NATIVE_INPUT input)
{
    float4 source[4]; [unroll] for (uint i=0u; i<4u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
    source[1] = g_ArtistSourceMaterialParameters[0u];
    source[2] = ArtistNativeAppend(cos((g_ArtistSourceMaterialTime.xxxx*float4(0.25, 0.0, 0.0, 0.0))),(float4(-1.0, 0.0, 0.0, 0.0)*sin((g_ArtistSourceMaterialTime.xxxx*float4(0.25, 0.0, 0.0, 0.0)))),1u);
    source[3] = ArtistNativeAppend(sin((g_ArtistSourceMaterialTime.xxxx*float4(0.25, 0.0, 0.0, 0.0))),cos((g_ArtistSourceMaterialTime.xxxx*float4(0.25, 0.0, 0.0, 0.0))),1u);
    float4 passValues[4]={float4(.5f,-.5f,.5f,.5f),float4(0.f,0.f,0.f,0.f),float4(0.f,0.f,0.f,0.f),float4(0.f,0.f,0.f,0.f)};
    float4 v0 = float4(input.sourceBasisX,0.f); // native texcoord10
    float4 v1 = float4(input.sourceBasisZ,input.handedness); // native texcoord11
    float4 v2 = float4(input.uv,float2(0.f,0.f)); // native texcoord0
    ...
    // 6: mul r1.xyzw, v2.xyxy, l(0.500000, 0.500000, 0.300000, 0.300000)
    r1.xyzw = ((v2.xyxy)*(float4(0.500000,0.500000,0.300000,0.300000))).xyzw;
    // 7: mad r1.xyzw, v4.yyyy, l(0.100000, -0.100000, -0.070000, 0.100000), r1.xyzw
    r1.xyzw = ((v4.yyyy)*(float4(0.100000,-0.100000,-0.070000,0.100000))+(r1.xyzw)).xyzw;
    // 8: sample_b_indexable(texture2d)(float,float,float,float) r0.y, r1.zwzz, t0.xyzw, s0, l(0.000000)
    r0.y = (ArtistNativeSample0((r1.zwzz).xy, (float4(0.000000,0.000000,0.000000,0.000000)).x, false).xyzw).y;
    // 9: sample_b_indexable(texture2d)(float,float,float,float) r0.z, r1.xyxx, t0.yzxw, s0, l(0.000000)
    r0.z = (ArtistNativeSample0((r1.xyxx).xy, (float4(0.000000,0.000000,0.000000,0.000000)).x, false).yzxw).z;
    // 10: add r0.y, -r0.z, r0.y
    r0.y = ((-(r0.zzzz))+(r0.yyyy)).y;
    // 11: mad r0.x, r0.x, r0.y, r0.z
    r0.x = ((r0.xxxx)*(r0.yyyy)+(r0.zzzz)).x;
    // 12: mul r1.xyzw, v2.xyxy, l(0.700000, 0.700000, 0.950000, 0.950000)
    r1.xyzw = ((v2.xyxy)*(float4(0.700000,0.700000,0.950000,0.950000))).xyzw;
    // 13: mad r1.xyzw, r0.xxxx, l(0.400000, 0.400000, 0.200000, 0.200000), r1.xyzw
    r1.xyzw = ((r0.xxxx)*(float4(0.400000,0.400000,0.200000,0.200000))+(r1.xyzw)).xyzw;
    // 14: mad r0.xy, r0.xxxx, v4.xxxx, v2.xyxx
    r0.xy = ((r0.xxxx)*(v4.xxxx)+(v2.xyxx)).xy;
    // 15: sample_b_indexable(texture2d)(float,float,float,float) r0.x, r0.xyxx, t5.xyzw, s3, l(0.000000)
    r0.x = (ArtistNativeSample3((r0.xyxx).xy, (float4(0.000000,0.000000,0.000000,0.000000)).x, false).xyzw).x;
    // 16: sample_b_indexable(texture2d)(float,float,float,float) r0.y, r1.xyxx, t2.yxzw, s4, l(0.000000)
    ...
    // 18: sample_b_indexable(texture2d)(float,float,float,float) r0.z, r1.zwzz, t3.yzxw, s5, l(0.000000)
    r0.z = (ArtistNativeSample5((r1.zwzz).xy, (float4(0.000000,0.000000,0.000000,0.000000)).x, false).yzxw).z;
    // 19: sample_b_indexable(texture2d)(float,float,float,float) r0.w, r1.xyxx, t4.yzwx, s2, l(0.000000)
    r0.w = (ArtistNativeSample2((r1.xyxx).xy, (float4(0.000000,0.000000,0.000000,0.000000)).x, false).yzwx).w;
    // 20: add r0.x, r0.x, r0.w
    r0.x = ((r0.xxxx)+(r0.wwww)).x;
    // 21: mad r1.xyz, r0.xxxx, v3.xyzx, cb0[1].xyzx
    r1.xyz = ((r0.xxxx)*(v3.xyzx)+(source[1].xyzx)).xyz;
    // 22: mul r1.xyz, r1.xyzx, v5.wwww
    r1.xyz = ((r1.xyzx)*(v5.wwww)).xyz;
    // 23: mul r0.x, r0.z, r0.y
...
float4 ArtistNative2574(ARTIST_NATIVE_INPUT input)
{
    float4 source[7]; [unroll] for (uint i=0u; i<7u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
float4 ArtistNative2575(ARTIST_NATIVE_INPUT input)
{
...
float4 ArtistNative2576(ARTIST_NATIVE_INPUT input)
{
    float4 source[7]; [unroll] for (uint i=0u; i<7u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
float4 ArtistNative2577(ARTIST_NATIVE_INPUT input)
{
    float4 source[4]; [unroll] for (uint i=0u; i<4u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
float4 ArtistNative2578(ARTIST_NATIVE_INPUT input)
{
    float4 source[7]; [unroll] for (uint i=0u; i<7u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
float4 ArtistNative2579(ARTIST_NATIVE_INPUT input)
{
    float4 source[2]; [unroll] for (uint i=0u; i<2u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
float4 ArtistNative2580(ARTIST_NATIVE_INPUT input)
{
    float4 source[7]; [unroll] for (uint i=0u; i<7u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
float4 ArtistNative2581(ARTIST_NATIVE_INPUT input)
{
    float4 source[4]; [unroll] for (uint i=0u; i<4u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
float4 ArtistNative2582(ARTIST_NATIVE_INPUT input)
{
    float4 source[4]; [unroll] for (uint i=0u; i<4u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
float4 ArtistNative2583(ARTIST_NATIVE_INPUT input)
{
    float4 source[2]; [unroll] for (uint i=0u; i<2u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
#ifndef ARTIST_NATIVE_MODEL_ONLY
// fx_m_pa_spritewave_01_35_tr: 1b614321d13a674ba81d24973bb6b184; selected map 14be505a134258ff472c4ecfa430347520d50d720964438b3b672c805ef17b2f.
float4 ArtistNative2584(ARTIST_NATIVE_INPUT input)
{
...
float4 ArtistNative2585(ARTIST_NATIVE_INPUT input)
{
    float4 source[4]; [unroll] for (uint i=0u; i<4u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
#ifndef ARTIST_NATIVE_MODEL_ONLY
// fx_k_pa_rampshape_01_12_ad: 103a9b035e2cdf478664fac24e797f6f; selected map 1d2fc584c5e9ed63771635d086940889d38c063a1693af4d6e59d037bcbe3ed9.
float4 ArtistNative2586(ARTIST_NATIVE_INPUT input)
{
    float4 source[7]; [unroll] for (uint i=0u; i<7u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
#ifndef ARTIST_NATIVE_MODEL_ONLY
// fx_o_pa_waterflow_01_04_tr: d5264a6e7ea1394690402346afbc0de6; selected map 7f81ef1d8d362e88193717f346d5f3bc9143e81b03f25b065f212b6f5346b98d.
float4 ArtistNative2587(ARTIST_NATIVE_INPUT input)
{
...
float4 ArtistNative2588(ARTIST_NATIVE_INPUT input)
{
    float4 source[4]; [unroll] for (uint i=0u; i<4u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#ifndef ARTIST_NATIVE_MODEL_ONLY
// fx_k_me_makeflow_02_01_tr3653: 9765660da7e1414994a02fe197e8e364; selected map 918ae65d939d6b75a6cc152d5ac743e52b01221f2cff31ca56faeeb01242cf25.
float4 ArtistNative2589(ARTIST_NATIVE_INPUT input)
{
...
#if !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
#ifndef ARTIST_NATIVE_MODEL_ONLY
// fx_j_pa_rampshape_01_05_ad: 103a9b035e2cdf478664fac24e797f6f; selected map 1d2fc584c5e9ed63771635d086940889d38c063a1693af4d6e59d037bcbe3ed9.
float4 ArtistNative2590(ARTIST_NATIVE_INPUT input)
{
    float4 source[7]; [unroll] for (uint i=0u; i<7u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(EFFECT_NATIVE_PARTICLE_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
#ifndef ARTIST_NATIVE_MODEL_ONLY
// fx_d_me_panwithouttex_01_01_ad: e3c2bd3d9b303d42872b701115c7d529; selected map 088be13f95f09eac78f679f7096c4f81d469b973112022792e28782e16f3b846.
float4 ArtistNative2591(ARTIST_NATIVE_INPUT input)
{
    float4 source[5]; [unroll] for (uint i=0u; i<5u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
    ...
    source[3].x = (g_ArtistSourceMaterialParameters[0u].xxxx).x;
    source[3].y = (g_ArtistSourceMaterialParameters[0u].yyyy).x;
    source[3].z = (max(float4(9.99999975e-06, 0.0, 0.0, 0.0),g_ArtistSourceMaterialParameters[0u].yyyy)).x;
    source[3].w = ((float4(1.0, 0.0, 0.0, 0.0)/max(float4(9.99999975e-06, 0.0, 0.0, 0.0),g_ArtistSourceMaterialParameters[0u].yyyy))).x;
    source[4].x = (g_ArtistSourceMaterialParameters[0u].zzzz).x;
    source[4].y = ((float4(1.0, 0.0, 0.0, 0.0)-g_ArtistSourceMaterialParameters[0u].zzzz)).x;
    source[4].z = (max((float4(1.0, 0.0, 0.0, 0.0)-g_ArtistSourceMaterialParameters[0u].zzzz),float4(9.99999975e-06, 0.0, 0.0, 0.0))).x;
    source[4].w = ((float4(1.0, 0.0, 0.0, 0.0)/max((float4(1.0, 0.0, 0.0, 0.0)-g_ArtistSourceMaterialParameters[0u].zzzz),float4(9.99999975e-06, 0.0, 0.0, 0.0)))).x;
    float4 passValues[4]={float4(.5f,-.5f,.5f,.5f),float4(0.f,0.f,0.f,0.f),float4(0.f,0.f,0.f,0.f),float4(0.f,0.f,0.f,0.f)};
    float4 v0 = float4(input.sourceBasisX,0.f); // native texcoord10
    float4 v1 = float4(input.sourceBasisZ,input.handedness); // native texcoord11
    float4 v2 = input.vertexColor; // native color0
    ...
    float4 v8 = asfloat(uint4(input.frontFace ? 0xffffffffu : 0u,0u,0u,0u)); // native sv_isfrontface0
    float4 r0=0.f, r1=0.f;
    // 1: add r0.x, cb0[1].w, l(-1.000000)
    r0.x = ((source[1].wwww)+(float4(-1.000000,-1.000000,-1.000000,-1.000000))).x;
    // 2: mul r0.x, r0.x, cb0[3].x
    r0.x = ((r0.xxxx)*(source[3].xxxx)).x;
    // 3: add r0.y, v4.x, l(-0.500000)
    r0.y = ((v4.xxxx)+(float4(-0.500000,-0.500000,-0.500000,-0.500000))).y;
    // 4: mad r1.x, r0.y, cb0[3].x, r0.x
    r1.x = ((r0.yyyy)*(source[3].xxxx)+(r0.xxxx)).x;
    // 5: add r0.x, r0.y, r0.y
    r0.x = ((r0.yyyy)+(r0.yyyy)).x;
    // 6: add r0.x, -|r0.x|, l(1.000000)
    r0.x = ((-(abs(r0.xxxx)))+(float4(1.000000,1.000000,1.000000,1.000000))).x;
    // 7: mul_sat r0.x, r0.x, l(30.000000)
    r0.x = (saturate((r0.xxxx)*(float4(30.000000,30.000000,30.000000,30.000000)))).x;
    // 8: mov r1.y, v4.y
    r1.y = (v4.yyyy).y;
    // 9: add r0.yz, r1.xxyx, l(0.000000, 0.000000, -0.500000, 0.000000)
    r0.yz = ((r1.xxyx)+(float4(0.000000,0.000000,-0.500000,0.000000))).yz;
    // 10: dp2 r0.y, r0.yzyy, r0.yzyy
    r0.y = (dot((r0.yzyy).xy,(r0.yzyy).xy).xxxx).y;
    // 11: sqrt r0.y, r0.y
    r0.y = (sqrt(r0.yyyy)).y;
    // 12: mad r0.y, -r0.y, cb0[3].w, l(1.000000)
    r0.y = ((-(r0.yyyy))*(source[3].wwww)+(float4(1.000000,1.000000,1.000000,1.000000))).y;
    // 13: mul_sat r0.y, r0.y, cb0[4].w
    r0.y = (saturate((r0.yyyy)*(source[4].wwww))).y;
    // 14: mul r0.x, r0.x, r0.y
    r0.x = ((r0.xxxx)*(r0.yyyy)).x;
    // 15: mul r0.x, r0.x, cb0[0].x
    r0.x = ((r0.xxxx)*(source[0].xxxx)).x;
    // 16: add r0.yzw, cb0[1].xxyz, cb0[2].xxyz
...
#if !defined(EFFECT_NATIVE_PARTICLE_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
#ifndef ARTIST_NATIVE_MODEL_ONLY
// fx_d_me_master_01_047_ts_ad: 4a87a66ebd21644990e71ba230428f3a; selected map b85b1d3895932d43a6c7d0f81541abe2531953d79049126d88dee03788749360.
float4 ArtistNative2592(ARTIST_NATIVE_INPUT input)
{
    float4 source[8]; [unroll] for (uint i=0u; i<8u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
    ...
    float4 v8 = asfloat(uint4(input.frontFace ? 0xffffffffu : 0u,0u,0u,0u)); // native sv_isfrontface0
    float4 r0=0.f, r1=0.f;
    // 1: mul r0.x, v4.x, cb0[5].w
    r0.x = ((v4.xxxx)*(source[5].wwww)).x;
    // 2: mul r0.y, cb0[5].x, cb0[5].y
    r0.y = ((source[5].xxxx)*(source[5].yyyy)).y;
    // 3: mad r1.x, r0.y, cb0[5].z, r0.x
    r1.x = ((r0.yyyy)*(source[5].zzzz)+(r0.xxxx)).x;
    // 4: mul r0.xz, v4.yyxy, cb0[6].xxwx
    r0.xz = ((v4.yyxy)*(source[6].xxwx)).xz;
    // 5: mad r1.yz, r0.yyyy, cb0[6].yyzy, r0.xxzx
    r1.yz = ((r0.yyyy)*(source[6].yyzy)+(r0.xxzx)).yz;
    // 6: sample_b_indexable(texture2d)(float,float,float,float) r0.x, r1.xyxx, t0.xyzw, s0, l(0.000000)
    r0.x = (ArtistNativeSample0((r1.xyxx).xy, (float4(0.000000,0.000000,0.000000,0.000000)).x, false).xyzw).x;
    // 7: mul r0.z, v4.y, cb0[7].x
    r0.z = ((v4.yyyy)*(source[7].xxxx)).z;
    // 8: mad r1.w, r0.y, cb0[7].y, r0.z
    r1.w = ((r0.yyyy)*(source[7].yyyy)+(r0.zzzz)).w;
    // 9: sample_b_indexable(texture2d)(float,float,float,float) r0.y, r1.zwzz, t1.yxzw, s1, l(0.000000)
    r0.y = (ArtistNativeSample1((r1.zwzz).xy, (float4(0.000000,0.000000,0.000000,0.000000)).x, false).yxzw).y;
    // 10: add r0.z, -cb0[4].x, l(1.000000)
    r0.z = ((-(source[4].xxxx))+(float4(1.000000,1.000000,1.000000,1.000000))).z;
    // 11: mad r0.x, r0.x, r0.y, -r0.z
    r0.x = ((r0.xxxx)*(r0.yyyy)+(-(r0.zzzz))).x;
    // 12: mul_sat r0.x, r0.x, cb0[7].z
    r0.x = (saturate((r0.xxxx)*(source[7].zzzz))).x;
    // 13: log r0.y, r0.x
    r0.y = (log2(r0.xxxx)).y;
    // 14: lt r0.x, r0.x, l(0.000001)
    r0.x = (asfloat((uint4)((r0.xxxx)<(float4(0.000001,0.000001,0.000001,0.000001))) * 0xffffffffu)).x;
    // 15: mul r0.y, r0.y, cb0[7].w
    r0.y = ((r0.yyyy)*(source[7].wwww)).y;
    // 16: exp r0.y, r0.y
    r0.y = (exp2(r0.yyyy)).y;
    // 17: mul_sat r0.y, r0.y, cb0[1].w
    r0.y = (saturate((r0.yyyy)*(source[1].wwww))).y;
    // 18: mul r0.y, r0.y, cb0[0].x
    r0.y = ((r0.yyyy)*(source[0].xxxx)).y;
    // 19: movc r0.x, r0.x, l(0), r0.y
    r0.x = ((asuint(r0.xxxx) != 0u) ? (float4(asfloat(0u),asfloat(0u),asfloat(0u),asfloat(0u))) : (r0.yyyy)).x;
    // 20: mul r0.yzw, cb0[3].xxyz, cb0[3].wwww
    r0.yzw = ((source[3].xxyz)*(source[3].wwww)).yzw;
    // 21: mad r0.yzw, r0.yyzw, cb0[1].xxyz, cb0[2].xxyz
...
float4 ArtistNative2566Distortion(ARTIST_NATIVE_INPUT input)
{
    float4 source[3]; [unroll] for (uint i=0u; i<3u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2570Distortion(ARTIST_NATIVE_INPUT input)
{
    float4 source[3]; [unroll] for (uint i=0u; i<3u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2571Distortion(ARTIST_NATIVE_INPUT input)
{
    float4 source[3]; [unroll] for (uint i=0u; i<3u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
float4 ArtistNative2574Distortion(ARTIST_NATIVE_INPUT input)
{
    float4 source[3]; [unroll] for (uint i=0u; i<3u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_PARTICLE_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2575Distortion(ARTIST_NATIVE_INPUT input)
{
    float4 source[3]; [unroll] for (uint i=0u; i<3u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_PARTICLE_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2576Distortion(ARTIST_NATIVE_INPUT input)
{
    float4 source[3]; [unroll] for (uint i=0u; i<3u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2577Distortion(ARTIST_NATIVE_INPUT input)
{
    float4 source[3]; [unroll] for (uint i=0u; i<3u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
    ...
    source[0].z = (g_ArtistSourceMaterialParameters[1u].yyyy).x;
    ...
    source[1].z = (g_ArtistSourceMaterialParameters[0u].xxxx).x;
    ...
    // 26: add r1.y, -cb0[1].z, l(1.000000)
    r1.y = ((-(source[1].zzzz))+(float4(1.000000,1.000000,1.000000,1.000000))).y;
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2587Distortion(ARTIST_NATIVE_INPUT input)
{
    float4 source[7]; [unroll] for (uint i=0u; i<7u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
float4 ArtistNative2589Distortion(ARTIST_NATIVE_INPUT input)
{
    float4 source[3]; [unroll] for (uint i=0u; i<3u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_PARTICLE_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2592Distortion(ARTIST_NATIVE_INPUT input)
{
    float4 source[3]; [unroll] for (uint i=0u; i<3u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2593(ARTIST_NATIVE_INPUT input)
{
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2594(ARTIST_NATIVE_INPUT input)
{
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_PARTICLE_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2595(ARTIST_NATIVE_INPUT input)
{
    ...
    // 18: sample_b_indexable(texture2d)(float,float,float,float) r1.xyzw, r0.yzyy, t0.xyzw, s0, l(0.000000)
    r1.xyzw = (ArtistNativeSample0((r0.yzyy).xy, (float4(0.000000,0.000000,0.000000,0.000000)).x, false).xyzw).xyzw;
    // 19: log r0.y, |r1.w|
    r0.y = (log2(abs(r1.wwww))).y;
    // 20: mul r0.y, r0.y, cb0[5].z
    r0.y = ((r0.yyyy)*(source[5].zzzz)).y;
    // 21: exp r0.y, r0.y
    r0.y = (exp2(r0.yyyy)).y;
    // 22: mul r0.y, r0.y, cb0[5].w
    r0.y = ((r0.yyyy)*(source[5].wwww)).y;
    // 23: mul_sat r0.y, r0.y, cb0[1].w
    r0.y = (saturate((r0.yyyy)*(source[1].wwww))).y;
    // 24: mul r0.y, r0.y, cb0[2].x
    r0.y = ((r0.yyyy)*(source[2].xxxx)).y;
    // 25: mul r0.x, r0.x, r0.y
    r0.x = ((r0.xxxx)*(r0.yyyy)).x;
    // 26: lt r0.y, |r1.w|, l(0.000001)
    r0.y = (asfloat((uint4)((abs(r1.wwww))<(float4(0.000001,0.000001,0.000001,0.000001))) * 0xffffffffu)).y;
    // 27: movc o0.w, r0.y, l(0), r0.x
    output.w = ((asuint(r0.yyyy) != 0u) ? (float4(asfloat(0u),asfloat(0u),asfloat(0u),asfloat(0u))) : (r0.xxxx)).w;
    // 28: dp3 r0.x, r1.xyzx, l(0.300000, 0.590000, 0.110000, 0.000000)
    r0.x = (dot((r1.xyzx).xyz,(float4(0.300000,0.590000,0.110000,0.000000)).xyz).xxxx).x;
    // 29: add r0.xyz, -r1.xyzx, r0.xxxx
    r0.xyz = ((-(r1.xyzx))+(r0.xxxx)).xyz;
    // 30: mad r0.xyz, cb0[5].yyyy, r0.xyzx, r1.xyzx
    r0.xyz = ((source[5].yyyy)*(r0.xyzx)+(r1.xyzx)).xyz;
    // 31: mul r0.xyz, r0.xyzx, cb0[1].xyzx
    r0.xyz = ((r0.xyzx)*(source[1].xyzx)).xyz;
    // 32: mul r1.xyz, cb0[4].xyzx, cb0[4].wwww
    r1.xyz = ((source[4].xyzx)*(source[4].wwww)).xyz;
    // 33: mad r0.xyz, r1.xyzx, r0.xyzx, cb0[3].xyzx
    r0.xyz = ((r1.xyzx)*(r0.xyzx)+(source[3].xyzx)).xyz;
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_PARTICLE_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2595Distortion(ARTIST_NATIVE_INPUT input)
{
    float4 source[3]; [unroll] for (uint i=0u; i<3u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2597(ARTIST_NATIVE_INPUT input)
{
    float4 source[18]; [unroll] for (uint i=0u; i<18u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2597Distortion(ARTIST_NATIVE_INPUT input)
{
    float4 source[3]; [unroll] for (uint i=0u; i<3u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2598(ARTIST_NATIVE_INPUT input)
{
    float4 source[5]; [unroll] for (uint i=0u; i<5u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
    source[1] = g_ArtistSourceMaterialParameters[1u];
    source[2] = ArtistNativeAppend(ArtistNativePeriodic(((g_ArtistSourceMaterialParameters[0u].zzzz*g_ArtistSourceMaterialTime.xxxx)*float4(-0.0500000007, 0.0, 0.0, 0.0))),ArtistNativePeriodic(((g_ArtistSourceMaterialParameters[0u].zzzz*g_ArtistSourceMaterialTime.xxxx)*float4(0.100000001, 0.0, 0.0, 0.0))),1u);
    source[3] = ArtistNativeAppend(float4(0.0, 0.0, 0.0, 0.0),g_ArtistSourceMaterialParameters[0u].wwww,1u);
    source[4].x = (g_ArtistSourceMaterialParameters[0u].xxxx).x;
    source[4].y = ((float4(1.0, 0.0, 0.0, 0.0)-g_ArtistSourceMaterialParameters[0u].xxxx)).x;
    ...
    // 3: mad r0.x, r0.x, l(2.000000), l(-1.000000)
    r0.x = ((r0.xxxx)*(float4(2.000000,2.000000,2.000000,2.000000))+(float4(-1.000000,-1.000000,-1.000000,-1.000000))).x;
    // 4: mad r0.xy, r0.xxxx, cb0[3].xyxx, v2.xyxx
    r0.xy = ((r0.xxxx)*(source[3].xyxx)+(v2.xyxx)).xy;
    // 5: add r0.xy, r0.xyxx, l(-0.500000, -0.500000, 0.000000, 0.000000)
    r0.xy = ((r0.xyxx)+(float4(-0.500000,-0.500000,0.000000,0.000000))).xy;
    // 6: max r0.z, |r0.y|, |r0.x|
    r0.z = (max(abs(r0.yyyy),abs(r0.xxxx))).z;
    // 7: div r0.z, l(1.000000, 1.000000, 1.000000, 1.000000), r0.z
    r0.z = ((float4(1.000000,1.000000,1.000000,1.000000))/(r0.zzzz)).z;
    // 8: min r0.w, |r0.y|, |r0.x|
    r0.w = (min(abs(r0.yyyy),abs(r0.xxxx))).w;
    // 9: mul r0.z, r0.z, r0.w
    r0.z = ((r0.zzzz)*(r0.wwww)).z;
    // 10: mul r0.w, r0.z, r0.z
    r0.w = ((r0.zzzz)*(r0.zzzz)).w;
    // 11: mad r1.x, r0.w, l(0.020835), l(-0.085133)
    r1.x = ((r0.wwww)*(float4(0.020835,0.020835,0.020835,0.020835))+(float4(-0.085133,-0.085133,-0.085133,-0.085133))).x;
    // 12: mad r1.x, r0.w, r1.x, l(0.180141)
    r1.x = ((r0.wwww)*(r1.xxxx)+(float4(0.180141,0.180141,0.180141,0.180141))).x;
    // 13: mad r1.x, r0.w, r1.x, l(-0.330299)
    r1.x = ((r0.wwww)*(r1.xxxx)+(float4(-0.330299,-0.330299,-0.330299,-0.330299))).x;
    // 14: mad r0.w, r0.w, r1.x, l(0.999866)
    r0.w = ((r0.wwww)*(r1.xxxx)+(float4(0.999866,0.999866,0.999866,0.999866))).w;
    // 15: mul r1.x, r0.w, r0.z
    r1.x = ((r0.wwww)*(r0.zzzz)).x;
    // 16: mad r1.x, r1.x, l(-2.000000), l(1.570796)
    r1.x = ((r1.xxxx)*(float4(-2.000000,-2.000000,-2.000000,-2.000000))+(float4(1.570796,1.570796,1.570796,1.570796))).x;
    // 17: lt r1.y, |r0.y|, |r0.x|
    r1.y = (asfloat((uint4)((abs(r0.yyyy))<(abs(r0.xxxx))) * 0xffffffffu)).y;
    // 18: and r1.x, r1.y, r1.x
    r1.x = (asfloat(asuint(r1.yyyy) & asuint(r1.xxxx))).x;
    // 19: mad r0.z, r0.z, r0.w, r1.x
    r0.z = ((r0.zzzz)*(r0.wwww)+(r1.xxxx)).z;
    // 20: lt r0.w, r0.y, -r0.y
    r0.w = (asfloat((uint4)((r0.yyyy)<(-(r0.yyyy))) * 0xffffffffu)).w;
    // 21: and r0.w, r0.w, l(0xc0490fdb)
    r0.w = (asfloat(asuint(r0.wwww) & uint4(0xc0490fdbu,0xc0490fdbu,0xc0490fdbu,0xc0490fdbu))).w;
    // 22: add r0.z, r0.w, r0.z
    r0.z = ((r0.wwww)+(r0.zzzz)).z;
    // 23: min r0.w, r0.y, r0.x
    r0.w = (min(r0.yyyy,r0.xxxx)).w;
    // 24: max r0.x, r0.y, r0.x
    r0.x = (max(r0.yyyy,r0.xxxx)).x;
    // 25: ge r0.x, r0.x, -r0.x
    r0.x = (asfloat((uint4)((r0.xxxx)>=(-(r0.xxxx))) * 0xffffffffu)).x;
    // 26: lt r0.y, r0.w, -r0.w
    r0.y = (asfloat((uint4)((r0.wwww)<(-(r0.wwww))) * 0xffffffffu)).y;
    // 27: and r0.x, r0.x, r0.y
    r0.x = (asfloat(asuint(r0.xxxx) & asuint(r0.yyyy))).x;
    // 28: movc r0.x, r0.x, -r0.z, r0.z
    r0.x = ((asuint(r0.xxxx) != 0u) ? (-(r0.zzzz)) : (r0.zzzz)).x;
    // 29: mul r0.y, r0.x, l(0.159155)
    r0.y = ((r0.xxxx)*(float4(0.159155,0.159155,0.159155,0.159155))).y;
    // 30: mad r0.x, r0.x, l(0.159155), l(0.500000)
    r0.x = ((r0.xxxx)*(float4(0.159155,0.159155,0.159155,0.159155))+(float4(0.500000,0.500000,0.500000,0.500000))).x;
    // 31: add r0.x, -r0.x, l(0.500000)
    r0.x = ((-(r0.xxxx))+(float4(0.500000,0.500000,0.500000,0.500000))).x;
    // 32: ge r0.z, r0.x, l(0.000000)
    r0.z = (asfloat((uint4)((r0.xxxx)>=(float4(0.000000,0.000000,0.000000,0.000000))) * 0xffffffffu)).z;
    // 33: max r0.x, r0.x, l(0.000000)
    r0.x = (max(r0.xxxx,float4(0.000000,0.000000,0.000000,0.000000))).x;
    // 34: movc r0.x, r0.z, r0.x, r0.y
    r0.x = ((asuint(r0.zzzz) != 0u) ? (r0.xxxx) : (r0.yyyy)).x;
    // 35: mad_sat r0.x, -r0.x, l(2.000000), l(1.000000)
    r0.x = (saturate((-(r0.xxxx))*(float4(2.000000,2.000000,2.000000,2.000000))+(float4(1.000000,1.000000,1.000000,1.000000)))).x;
    // 36: add r0.x, r0.x, -cb0[4].y
    r0.x = ((r0.xxxx)+(-(source[4].yyyy))).x;
    // 37: mul_sat r0.x, r0.x, cb0[4].z
    r0.x = (saturate((r0.xxxx)*(source[4].zzzz))).x;
    // 38: mul_sat r0.x, r0.x, v3.w
    r0.x = (saturate((r0.xxxx)*(v3.wwww))).x;
    // 39: mul o0.w, r0.x, cb0[0].x
    output.w = ((r0.xxxx)*(source[0].xxxx)).w;
    // 40: div r0.xy, v7.xyxx, v7.wwww
    r0.xy = ((v7.xyxx)/(v7.wwww)).xy;
    // 41: mad r0.xy, r0.xyxx, cb2[0].xyxx, cb2[0].wzww
    r0.xy = ((r0.xyxx)*(passValues[0].xyxx)+(passValues[0].wzww)).xy;
    // 42: sample_indexable(texture2d)(float,float,float,float) r0.xyz, r0.xyxx, t1.xyzw, s0 (project resolved HDR SceneColor snapshot adapter)
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_PARTICLE_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2599(ARTIST_NATIVE_INPUT input)
{
    float4 source[18]; [unroll] for (uint i=0u; i<18u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
    ...
    source[4] = ArtistNativeAppend(g_ArtistSourceMaterialParameters[0u].wwww,g_ArtistSourceMaterialParameters[1u].yyyy,1u);
    ...
    source[8] = ArtistNativeAppend(ArtistNativeAppend(sin((((clamp(g_ArtistSourceMaterialParameters[0u].xxxx,float4(1.0, 0.0, 0.0, 0.0),float4(12.0, 0.0, 0.0, 0.0))-float4(1.0, 0.0, 0.0, 0.0))*float4(0.0909999982, 0.0, 0.0, 0.0))*float4(6.28318548, 0.0, 0.0, 0.0))),(cos((((clamp(g_ArtistSourceMaterialParameters[0u].xxxx,float4(1.0, 0.0, 0.0, 0.0),float4(12.0, 0.0, 0.0, 0.0))-float4(1.0, 0.0, 0.0, 0.0))*float4(0.0909999982, 0.0, 0.0, 0.0))*float4(6.28318548, 0.0, 0.0, 0.0)))*float4(-1.0, 0.0, 0.0, 0.0)),1u),float4(0.75, 0.0, 0.0, 0.0),2u);
    ...
    source[10].x = (((g_ArtistSourceMaterialTime.xxxx*g_ArtistSourceMaterialParameters[2u].zzzz)*float4(0.0, 0.0, 0.0, 0.0))).x;
    ...
    source[11].y = (ArtistNativePeriodic(((g_ArtistSourceMaterialTime.xxxx*g_ArtistSourceMaterialParameters[2u].zzzz)*float4(0.0, 0.0, 0.0, 0.0)))).x;
    source[11].z = (ArtistNativePeriodic(((g_ArtistSourceMaterialTime.xxxx*g_ArtistSourceMaterialParameters[2u].zzzz)*float4(1.0, 0.0, 0.0, 0.0)))).x;
    ...
    // 10: discard_nz r0.x
    if ((asuint(r0.xxxx)).x != 0u) clip(-1.f);
    // 11: sample_b_indexable(texture2d)(float,float,float,float) r0.x, v4.xyxx, t1.zxyw, s1, l(0.000000)
    r0.x = (ArtistNativeSample1((v4.xyxx).xy, (float4(0.000000,0.000000,0.000000,0.000000)).x, false).zxyw).x;
    // 12: add r0.x, -r0.x, l(1.000000)
    r0.x = ((-(r0.xxxx))+(float4(1.000000,1.000000,1.000000,1.000000))).x;
    // 13: max r0.y, cb0[1].y, l(0.000010)
    r0.y = (max(source[1].yyyy,float4(0.000010,0.000010,0.000010,0.000010))).y;
    ...
    // 17: sqrt r1.x, r1.x
    r1.x = (sqrt(r1.xxxx)).x;
    // 18: mad r0.y, -r1.x, r0.y, l(1.000000)
    r0.y = ((-(r1.xxxx))*(r0.yyyy)+(float4(1.000000,1.000000,1.000000,1.000000))).y;
    // 19: max r0.y, r0.y, l(0.000000)
    r0.y = (max(r0.yyyy,float4(0.000000,0.000000,0.000000,0.000000))).y;
    // 20: mul r0.x, r0.x, r0.y
    r0.x = ((r0.xxxx)*(r0.yyyy)).x;
    // 21: sample_b_indexable(texture2d)(float,float,float,float) r1.x, v4.xyxx, t0.zxyw, s0, l(0.000000)
    r1.x = (ArtistNativeSample0((v4.xyxx).xy, (float4(0.000000,0.000000,0.000000,0.000000)).x, false).zxyw).x;
    // 22: mad r1.x, -r0.x, cb0[9].z, r1.x
    r1.x = ((-(r0.xxxx))*(source[9].zzzz)+(r1.xxxx)).x;
    // 23: mul r0.xy, r0.xyxx, cb0[9].zzzz
    r0.xy = ((r0.xyxx)*(source[9].zzzz)).xy;
    // 24: mul r1.x, r1.x, cb0[9].w
    r1.x = ((r1.xxxx)*(source[9].wwww)).x;
    // 25: mad r1.x, r1.x, l(0.050000), l(-0.025000)
    r1.x = ((r1.xxxx)*(float4(0.050000,0.050000,0.050000,0.050000))+(float4(-0.025000,-0.025000,-0.025000,-0.025000))).x;
    // 26: dp3 r1.y, v6.xyzx, v6.xyzx
    r1.y = (dot((v6.xyzx).xyz,(v6.xyzx).xyz).xxxx).y;
    // 27: rsq r1.y, r1.y
    r1.y = (rsqrt(r1.yyyy)).y;
    // 28: mul r1.yz, r1.yyyy, v6.xxyx
    r1.yz = ((r1.yyyy)*(v6.xxyx)).yz;
    // 29: mul r2.xy, r1.yzyy, r1.xxxx
    r2.xy = ((r1.yzyy)*(r1.xxxx)).xy;
    // 30: mad r1.xy, r1.xxxx, r1.yzyy, v4.xyxx
    r1.xy = ((r1.xxxx)*(r1.yzyy)+(v4.xyxx)).xy;
    // 31: mad r1.zw, cb0[10].wwww, v4.xxxy, r2.xxxy
    r1.zw = ((source[10].wwww)*(v4.xxxy)+(r2.xxxy)).zw;
    // 32: sample_b_indexable(texture2d)(float,float,float,float) r1.z, r1.zwzz, t6.yzxw, s5, l(0.000000)
    r1.z = (ArtistNativeSample5((r1.zwzz).xy, (float4(0.000000,0.000000,0.000000,0.000000)).x, false).yzxw).z;
    // 33: log r1.w, |r1.z|
    ...
    // 35: mul r1.w, r1.w, cb0[11].x
    r1.w = ((r1.wwww)*(source[11].xxxx)).w;
    // 36: exp r1.w, r1.w
    r1.w = (exp2(r1.wwww)).w;
    // 37: mul_sat r1.w, r1.w, cb0[11].x
    r1.w = (saturate((r1.wwww)*(source[11].xxxx))).w;
    // 38: movc r1.z, r1.z, l(0), r1.w
    r1.z = ((asuint(r1.zzzz) != 0u) ? (float4(asfloat(0u),asfloat(0u),asfloat(0u),asfloat(0u))) : (r1.wwww)).z;
    // 39: mad r3.xyzw, cb0[10].yyzz, v4.xyxy, r2.xyxy
    r3.xyzw = ((source[10].yyzz)*(v4.xyxy)+(r2.xyxy)).xyzw;
    // 40: mad r0.zw, r0.zzzw, cb0[4].xxxy, r2.xxxy
    r0.zw = ((r0.zzzw)*(source[4].xxxy)+(r2.xxxy)).zw;
    // 41: add r0.zw, r0.zzzw, l(0.000000, 0.000000, 0.500000, 0.500000)
    r0.zw = ((r0.zzzw)+(float4(0.000000,0.000000,0.500000,0.500000))).zw;
    // 42: sample_b_indexable(texture2d)(float,float,float,float) r0.z, r0.zwzz, t8.xzyw, s2, l(0.000000)
    r0.z = (ArtistNativeSample2((r0.zwzz).xy, (float4(0.000000,0.000000,0.000000,0.000000)).x, false).xzyw).z;
    // 43: sample_b_indexable(texture2d)(float,float,float,float) r2.xyz, r3.zwzz, t5.xyzw, s4, l(0.000000)
    r2.xyz = (ArtistNativeSample4((r3.zwzz).xy, (float4(0.000000,0.000000,0.000000,0.000000)).x, false).xyzw).xyz;
    // 44: sample_b_indexable(texture2d)(float,float,float,float) r3.xyz, r3.xyxx, t4.xyzw, s3, l(0.000000)
    r3.xyz = (ArtistNativeSample3((r3.xyxx).xy, (float4(0.000000,0.000000,0.000000,0.000000)).x, false).xyzw).xyz;
    // 45: add r2.xyz, r2.xyzx, -r3.xyzx
    r2.xyz = ((r2.xyzx)+(-(r3.xyzx))).xyz;
    // 46: mad r2.xyz, r1.zzzz, r2.xyzx, r3.xyzx
    r2.xyz = ((r1.zzzz)*(r2.xyzx)+(r3.xyzx)).xyz;
    // 47: dp3 r0.w, r2.xyzx, l(0.300000, 0.590000, 0.110000, 0.000000)
    r0.w = (dot((r2.xyzx).xyz,(float4(0.300000,0.590000,0.110000,0.000000)).xyz).xxxx).w;
    // 48: add r3.xyz, -r2.xyzx, r0.wwww
    r3.xyz = ((-(r2.xyzx))+(r0.wwww)).xyz;
    // 49: mad r3.xyz, cb0[12].wwww, r3.xyzx, r2.xyzx
    r3.xyz = ((source[12].wwww)*(r3.xyzx)+(r2.xyzx)).xyz;
    // 50: mad r1.zw, r2.xxxy, l(0.000000, 0.000000, 0.200000, 0.200000), v4.xxxy
    r1.zw = ((r2.xxxy)*(float4(0.000000,0.000000,0.200000,0.200000))+(v4.xxxy)).zw;
    // 51: add r1.zw, r1.zzzw, cb0[5].xxxy
    r1.zw = ((r1.zzzw)+(source[5].xxxy)).zw;
    // 52: sample_b_indexable(texture2d)(float,float,float,float) r0.w, r1.zwzz, t4.yzwx, s3, l(0.000000)
    r0.w = (ArtistNativeSample3((r1.zwzz).xy, (float4(0.000000,0.000000,0.000000,0.000000)).x, false).yzwx).w;
    // 53: mul r2.xyz, cb0[7].xyzx, cb0[7].wwww
    r2.xyz = ((source[7].xyzx)*(source[7].wwww)).xyz;
    // 54: mul r2.xyz, r2.xyzx, r3.xyzx
    r2.xyz = ((r2.xyzx)*(r3.xyzx)).xyz;
    // 55: sample_b_indexable(texture2d)(float,float,float,float) r1.zw, r1.xyxx, t7.zwxy, s8, l(0.000000)
    r1.zw = (ArtistNativeSample8((r1.xyxx).xy, (float4(0.000000,0.000000,0.000000,0.000000)).x, false).zwxy).zw;
    // 56: mov_sat r1.xy, r1.xyxx
    r1.xy = (saturate(r1.xyxx)).xy;
    // 57: sample_b_indexable(texture2d)(float,float,float,float) r1.x, r1.xyxx, t2.xyzw, s6, l(0.000000)
    r1.x = (ArtistNativeSample6((r1.xyxx).xy, (float4(0.000000,0.000000,0.000000,0.000000)).x, false).xyzw).x;
    // 58: mul r1.x, r1.x, cb0[12].x
    r1.x = ((r1.xxxx)*(source[12].xxxx)).x;
    // 59: mad r3.xy, r1.zwzz, l(2.000000, 2.000000, 0.000000, 0.000000), l(-1.000000, -1.000000, 0.000000, 0.000000)
    r3.xy = ((r1.zwzz)*(float4(2.000000,2.000000,0.000000,0.000000))+(float4(-1.000000,-1.000000,0.000000,0.000000))).xy;
    // 60: dp2 r1.y, r3.xyxx, r3.xyxx
    r1.y = (dot((r3.xyxx).xy,(r3.xyxx).xy).xxxx).y;
    // 61: add r1.y, -r1.y, l(1.000000)
    r1.y = ((-(r1.yyyy))+(float4(1.000000,1.000000,1.000000,1.000000))).y;
    // 62: max r1.y, r1.y, l(0.000000)
    r1.y = (max(r1.yyyy,float4(0.000000,0.000000,0.000000,0.000000))).y;
    // 63: sqrt r1.y, r1.y
    r1.y = (sqrt(r1.yyyy)).y;
    // 64: add r3.z, r1.y, l(0.000010)
    r3.z = ((r1.yyyy)+(float4(0.000010,0.000010,0.000010,0.000010))).z;
    // 65: dp3 r1.y, r3.xyzx, r3.xyzx
    r1.y = (dot((r3.xyzx).xyz,(r3.xyzx).xyz).xxxx).y;
    // 66: sqrt r1.y, r1.y
    r1.y = (sqrt(r1.yyyy)).y;
    // 67: div r1.yzw, r3.xxyz, r1.yyyy
    r1.yzw = ((r3.xxyz)/(r1.yyyy)).yzw;
    // 68: dp3 r1.y, cb0[8].xyzx, r1.yzwy
    r1.y = (dot((source[8].xyzx).xyz,(r1.yzwy).xyz).xxxx).y;
    // 69: max r1.y, r1.y, l(0.150000)
    r1.y = (max(r1.yyyy,float4(0.150000,0.150000,0.150000,0.150000))).y;
    // 70: min r1.y, r1.y, l(1.000000)
    r1.y = (min(r1.yyyy,float4(1.000000,1.000000,1.000000,1.000000))).y;
    // 71: mul r1.yzw, r2.xxyz, r1.yyyy
    r1.yzw = ((r2.xxyz)*(r1.yyyy)).yzw;
    // 72: add r2.x, -r0.z, l(1.000000)
    r2.x = ((-(r0.zzzz))+(float4(1.000000,1.000000,1.000000,1.000000))).x;
    // 73: mul r2.x, r2.x, r2.x
    r2.x = ((r2.xxxx)*(r2.xxxx)).x;
    // 74: mul r2.x, r2.x, r2.x
    r2.x = ((r2.xxxx)*(r2.xxxx)).x;
    // 75: mul r1.yzw, r1.yyzw, r2.xxxx
    r1.yzw = ((r1.yyzw)*(r2.xxxx)).yzw;
    // 76: mad r1.yzw, r1.yyzw, cb2[3].wwww, cb2[3].xxyz
    r1.yzw = ((r1.yyzw)*(passValues[3].wwww)+(passValues[3].xxyz)).yzw;
    // 77: add r2.x, r0.w, r0.z
    r2.x = ((r0.wwww)+(r0.zzzz)).x;
    // 78: mul r0.z, r0.z, r2.x
    r0.z = ((r0.zzzz)*(r2.xxxx)).z;
    // 79: mul r0.z, r0.w, r0.z
    r0.z = ((r0.wwww)*(r0.zzzz)).z;
    // 80: lt r0.w, |r0.z|, l(0.000001)
    r0.w = (asfloat((uint4)((abs(r0.zzzz))<(float4(0.000001,0.000001,0.000001,0.000001))) * 0xffffffffu)).w;
    // 81: log r0.z, |r0.z|
    r0.z = (log2(abs(r0.zzzz))).z;
    // 82: mul r0.z, r0.z, cb0[11].w
    r0.z = ((r0.zzzz)*(source[11].wwww)).z;
    // 83: exp r0.z, r0.z
    r0.z = (exp2(r0.zzzz)).z;
    // 84: mul r2.xyz, cb0[6].xyzx, cb0[6].wwww
    r2.xyz = ((source[6].xyzx)*(source[6].wwww)).xyz;
    // 85: mul r2.xyz, r0.zzzz, r2.xyzx
    r2.xyz = ((r0.zzzz)*(r2.xyzx)).xyz;
    // 86: movc r2.xyz, r0.wwww, l(0,0,0,0), r2.xyzx
    r2.xyz = ((asuint(r0.wwww) != 0u) ? (float4(asfloat(0u),asfloat(0u),asfloat(0u),asfloat(0u))) : (r2.xyzx)).xyz;
    // 87: add r2.xyz, r2.xyzx, cb0[3].xyzx
    r2.xyz = ((r2.xyzx)+(source[3].xyzx)).xyz;
    // 88: dp3 r0.z, v7.xyzx, v7.xyzx
    r0.z = (dot((v7.xyzx).xyz,(v7.xyzx).xyz).xxxx).z;
    // 89: rsq r0.z, r0.z
    r0.z = (rsqrt(r0.zzzz)).z;
    // 90: mul r0.z, r0.z, v7.z
    r0.z = ((r0.zzzz)*(v7.zzzz)).z;
    // 91: mad r0.zw, r0.zzzz, l(0.000000, 0.000000, 0.500000, -0.500000), l(0.000000, 0.000000, 0.500000, 0.500000)
    r0.zw = ((r0.zzzz)*(float4(0.000000,0.000000,0.500000,-0.500000))+(float4(0.000000,0.000000,0.500000,0.500000))).zw;
    // 92: mul r0.zw, r0.zzzw, r0.zzzw
    r0.zw = ((r0.zzzw)*(r0.zzzw)).zw;
    // 93: mul r3.xyz, r0.wwww, cb0[16].xyzx
    r3.xyz = ((r0.wwww)*(source[16].xyzx)).xyz;
    // 94: mad r3.xyz, r0.zzzz, cb0[15].xyzx, r3.xyzx
    r3.xyz = ((r0.zzzz)*(source[15].xyzx)+(r3.xyzx)).xyz;
    // 95: mul r3.xyz, r3.xyzx, cb0[17].wwww
    r3.xyz = ((r3.xyzx)*(source[17].wwww)).xyz;
    // 96: mad r2.xyz, r3.xyzx, r1.yzwy, r2.xyzx
    r2.xyz = ((r3.xyzx)*(r1.yzwy)+(r2.xyzx)).xyz;
    // 97: mul r3.xyz, r1.yzwy, r3.xyzx
    r3.xyz = ((r1.yzwy)*(r3.xyzx)).xyz;
    // 99: mad r2.xyz, r1.yzwy, cb0[17].xyzx, r2.xyzx
    r2.xyz = ((r1.yzwy)*(source[17].xyzx)+(r2.xyzx)).xyz;
    // 101: mad o0.xyz, r2.xyzx, v5.wwww, v5.xyzx
    output.xyz = ((r2.xyzx)*(v5.wwww)+(v5.xyzx)).xyz;
    // 102: log r0.z, |r1.x|
    r0.z = (log2(abs(r1.xxxx))).z;
    // 103: lt r0.w, |r1.x|, l(0.000001)
    r0.w = (asfloat((uint4)((abs(r1.xxxx))<(float4(0.000001,0.000001,0.000001,0.000001))) * 0xffffffffu)).w;
    // 104: mul r0.z, r0.z, cb0[12].y
    r0.z = ((r0.zzzz)*(source[12].yyyy)).z;
    // 105: exp r0.z, r0.z
    r0.z = (exp2(r0.zzzz)).z;
    // 106: mul r0.y, r0.y, r0.z
    r0.y = ((r0.yyyy)*(r0.zzzz)).y;
    // 107: movc r0.y, r0.w, l(0), r0.y
    r0.y = ((asuint(r0.wwww) != 0u) ? (float4(asfloat(0u),asfloat(0u),asfloat(0u),asfloat(0u))) : (r0.yyyy)).y;
    // 108: mul r0.zw, v4.xxxy, cb0[12].zzzz
    r0.zw = ((v4.xxxy)*(source[12].zzzz)).zw;
    // 109: sample_b_indexable(texture2d)(float,float,float,float) r0.z, r0.zwzz, t3.yzxw, s7, l(0.000000)
    r0.z = (ArtistNativeSample7((r0.zwzz).xy, (float4(0.000000,0.000000,0.000000,0.000000)).x, false).yzxw).z;
    // 110: mov_sat r0.w, cb0[1].w
    r0.w = (saturate(source[1].wwww)).w;
    // 111: add r0.w, -r0.w, l(1.000000)
    r0.w = ((-(r0.wwww))+(float4(1.000000,1.000000,1.000000,1.000000))).w;
    // 112: mul r0.z, r0.z, r0.w
    r0.z = ((r0.zzzz)*(r0.wwww)).z;
    // 113: mul r0.x, r0.z, r0.x
    r0.x = ((r0.zzzz)*(r0.xxxx)).x;
    // 114: mad r0.x, -r0.x, l(10.000000), r0.y
    r0.x = ((-(r0.xxxx))*(float4(10.000000,10.000000,10.000000,10.000000))+(r0.yyyy)).x;
    // 115: mul_sat r0.x, r0.x, cb0[1].w
    r0.x = (saturate((r0.xxxx)*(source[1].wwww))).x;
    // 116: mul r0.x, r0.x, cb0[2].x
    r0.x = ((r0.xxxx)*(source[2].xxxx)).x;
    // 117: add r0.y, -|v4.w|, cb0[0].y
    r0.y = ((-(abs(v4.wwww)))+(source[0].yyyy)).y;
    // 118: mul r0.y, r0.y, l(5.000000)
    r0.y = ((r0.yyyy)*(float4(5.000000,5.000000,5.000000,5.000000))).y;
    // 119: div_sat r0.y, r0.y, cb0[0].y
    r0.y = (saturate((r0.yyyy)/(source[0].yyyy))).y;
    // 120: mul r0.y, r0.y, v4.z
    r0.y = ((r0.yyyy)*(v4.zzzz)).y;
    // 121: mul o0.w, r0.y, r0.x
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2600(ARTIST_NATIVE_INPUT input)
{
    float4 source[16]; [unroll] for (uint i=0u; i<16u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_PARTICLE_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2601(ARTIST_NATIVE_INPUT input)
{
    float4 source[10]; [unroll] for (uint i=0u; i<10u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_PARTICLE_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2601Distortion(ARTIST_NATIVE_INPUT input)
{
    float4 source[3]; [unroll] for (uint i=0u; i<3u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_PARTICLE_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2602(ARTIST_NATIVE_INPUT input)
{
    float4 source[18]; [unroll] for (uint i=0u; i<18u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_PARTICLE_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2603(ARTIST_NATIVE_INPUT input)
{
    float4 source[7]; [unroll] for (uint i=0u; i<7u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2604(ARTIST_NATIVE_INPUT input)
{
    float4 source[10]; [unroll] for (uint i=0u; i<10u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_PARTICLE_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2605(ARTIST_NATIVE_INPUT input)
{
    float4 source[18]; [unroll] for (uint i=0u; i<18u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2606(ARTIST_NATIVE_INPUT input)
{
    float4 source[14]; [unroll] for (uint i=0u; i<14u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
    ...
    source[3].y = (g_ArtistSourceMaterialParameters[4u].zzzz).x;
    source[3].z = (g_ArtistSourceMaterialParameters[4u].wwww).x;
    source[3].w = (g_ArtistSourceMaterialParameters[5u].zzzz).x;
    source[4].x = ((g_ArtistSourceMaterialParameters[4u].zzzz*g_ArtistSourceMaterialTime.xxxx)).x;
    source[4].y = (g_ArtistSourceMaterialParameters[3u].xxxx).x;
    source[4].z = (g_ArtistSourceMaterialParameters[5u].xxxx).x;
    source[4].w = (g_ArtistSourceMaterialParameters[5u].yyyy).x;
    source[5].x = ((g_ArtistSourceMaterialParameters[5u].yyyy*g_ArtistSourceMaterialTime.xxxx)).x;
    ...
    source[6].x = ((g_ArtistSourceMaterialParameters[2u].wwww*g_ArtistSourceMaterialTime.xxxx)).x;
    source[6].y = (g_ArtistSourceMaterialParameters[6u].zzzz).x;
    source[6].z = (g_ArtistSourceMaterialParameters[6u].wwww).x;
    source[6].w = (g_ArtistSourceMaterialParameters[5u].wwww).x;
    source[7].x = ((g_ArtistSourceMaterialParameters[5u].wwww*g_ArtistSourceMaterialTime.xxxx)).x;
    source[7].y = (((g_ArtistSourceMaterialParameters[5u].wwww*g_ArtistSourceMaterialTime.xxxx)*g_ArtistSourceMaterialParameters[5u].xxxx)).x;
    source[7].z = (g_ArtistSourceMaterialParameters[6u].xxxx).x;
    source[7].w = (g_ArtistSourceMaterialParameters[6u].yyyy).x;
    source[8].x = (g_ArtistSourceMaterialParameters[1u].wwww).x;
    source[8].y = (g_ArtistSourceMaterialParameters[2u].xxxx).x;
    ...
    source[10].y = (max(float4(9.99999975e-06, 0.0, 0.0, 0.0),g_ArtistSourceMaterialParameters[0u].zzzz)).x;
    source[10].z = ((float4(1.0, 0.0, 0.0, 0.0)/max(float4(9.99999975e-06, 0.0, 0.0, 0.0),g_ArtistSourceMaterialParameters[0u].zzzz))).x;
    source[10].w = (g_ArtistSourceMaterialParameters[0u].wwww).x;
    source[11].x = ((float4(1.0, 0.0, 0.0, 0.0)-g_ArtistSourceMaterialParameters[0u].wwww)).x;
    source[11].y = (max((float4(1.0, 0.0, 0.0, 0.0)-g_ArtistSourceMaterialParameters[0u].wwww),float4(9.99999975e-06, 0.0, 0.0, 0.0))).x;
    source[11].z = ((float4(1.0, 0.0, 0.0, 0.0)/max((float4(1.0, 0.0, 0.0, 0.0)-g_ArtistSourceMaterialParameters[0u].wwww),float4(9.99999975e-06, 0.0, 0.0, 0.0)))).x;
    source[11].w = (g_ArtistSourceMaterialParameters[1u].xxxx).x;
    source[12].x = (g_ArtistSourceMaterialParameters[0u].xxxx).x;
    source[12].y = (max(float4(9.99999975e-06, 0.0, 0.0, 0.0),g_ArtistSourceMaterialParameters[0u].xxxx)).x;
    source[12].z = ((float4(1.0, 0.0, 0.0, 0.0)/max(float4(9.99999975e-06, 0.0, 0.0, 0.0),g_ArtistSourceMaterialParameters[0u].xxxx))).x;
    source[12].w = (g_ArtistSourceMaterialParameters[0u].yyyy).x;
    source[13].x = ((float4(1.0, 0.0, 0.0, 0.0)-g_ArtistSourceMaterialParameters[0u].yyyy)).x;
    source[13].y = (max((float4(1.0, 0.0, 0.0, 0.0)-g_ArtistSourceMaterialParameters[0u].yyyy),float4(9.99999975e-06, 0.0, 0.0, 0.0))).x;
    source[13].z = ((float4(1.0, 0.0, 0.0, 0.0)/max((float4(1.0, 0.0, 0.0, 0.0)-g_ArtistSourceMaterialParameters[0u].yyyy),float4(9.99999975e-06, 0.0, 0.0, 0.0)))).x;
    source[13].w = (g_ArtistSourceMaterialParameters[1u].yyyy).x;
    float4 passValues[4]; [unroll] for(uint passIndex=0u;passIndex<4u;++passIndex) passValues[passIndex]=0.f;
    passValues[0]=float4(.5f,-.5f,.5f,.5f);
    float4 v0 = float4(input.sourceBasisX,0.f); // native texcoord10
    float4 v1 = float4(input.sourceBasisZ,input.handedness); // native texcoord11
    ...
    // 32: mul r1.x, r0.y, cb0[6].y
    r1.x = ((r0.yyyy)*(source[6].yyyy)).x;
    // 33: mul r2.x, r0.y, cb0[5].y
    r2.x = ((r0.yyyy)*(source[5].yyyy)).x;
    // 34: add r0.y, r0.x, r0.x
    r0.y = ((r0.xxxx)+(r0.xxxx)).y;
    // 35: log r0.y, r0.y
    r0.y = (log2(r0.yyyy)).y;
    // 36: mul r0.y, r0.y, cb0[4].y
    r0.y = ((r0.yyyy)*(source[4].yyyy)).y;
    // 37: exp r0.y, r0.y
    r0.y = (exp2(r0.yyyy)).y;
    // 38: mul r0.y, r0.y, cb0[4].z
    r0.y = ((r0.yyyy)*(source[4].zzzz)).y;
    // 39: lt r0.z, r0.x, l(0.000000)
    r0.z = (asfloat((uint4)((r0.xxxx)<(float4(0.000000,0.000000,0.000000,0.000000))) * 0xffffffffu)).z;
    // 40: movc r0.y, r0.z, l(0), r0.y
    r0.y = ((asuint(r0.zzzz) != 0u) ? (float4(asfloat(0u),asfloat(0u),asfloat(0u),asfloat(0u))) : (r0.yyyy)).y;
    // 41: add r0.z, v4.x, cb0[5].x
    ...
    // 43: mad r1.y, r0.y, cb0[6].z, cb0[7].y
    r1.y = ((r0.yyyy)*(source[6].zzzz)+(source[7].yyyy)).y;
    // 44: mad r2.y, r0.y, cb0[5].z, cb0[6].x
    r2.y = ((r0.yyyy)*(source[5].zzzz)+(source[6].xxxx)).y;
    // 45: sample_l_indexable(texture2d)(float,float,float,float) r0.yz, r1.xyxx, t0.zxyw, s1, l(0.000000)
    r0.yz = (ArtistNativeSample1((r1.xyxx).xy, (float4(0.000000,0.000000,0.000000,0.000000)).x, true).zxyw).yz;
    // 46: max r0.yz, |r0.yyzy|, l(0.000000, 0.000001, 0.000001, 0.000000)
    r0.yz = (max(abs(r0.yyzy),float4(0.000000,0.000001,0.000001,0.000000))).yz;
    // 47: log r0.yz, r0.yyzy
    r0.yz = (log2(r0.yyzy)).yz;
    // 48: mul r0.yz, r0.yyzy, cb0[7].zzzz
    r0.yz = ((r0.yyzy)*(source[7].zzzz)).yz;
    // 49: exp r0.yz, r0.yyzy
    r0.yz = (exp2(r0.yyzy)).yz;
    // 50: mad r1.xy, cb0[7].wwww, r0.yzyy, r2.xyxx
    r1.xy = ((source[7].wwww)*(r0.yzyy)+(r2.xyxx)).xy;
    // 51: mad r0.yz, cb0[7].wwww, r0.yyzy, v2.xxyx
    r0.yz = ((source[7].wwww)*(r0.yyzy)+(v2.xxyx)).yz;
    // 52: sample_b_indexable(texture2d)(float,float,float,float) r0.yzw, r0.yzyy, t2.wxyz, s2, l(0.000000)
    r0.yzw = (ArtistNativeSample2((r0.yzyy).xy, (float4(0.000000,0.000000,0.000000,0.000000)).x, false).wxyz).yzw;
    // 53: sample_l_indexable(texture2d)(float,float,float,float) r1.xyz, r1.xyxx, t1.xyzw, s0, l(0.000000)
    ...
    r1.xyz = (log2(r1.xyzx)).xyz;
    // 56: mul r1.xyz, r1.xyzx, cb0[8].xxxx
    r1.xyz = ((r1.xyzx)*(source[8].xxxx)).xyz;
    // 57: exp r1.xyz, r1.xyzx
    r1.xyz = (exp2(r1.xyzx)).xyz;
    // 58: mul r2.xyz, r1.xyzx, cb0[8].yyyy
    r2.xyz = ((r1.xyzx)*(source[8].yyyy)).xyz;
    // 59: dp3 r1.w, r2.xyzx, l(0.300000, 0.590000, 0.110000, 0.000000)
    r1.w = (dot((r2.xyzx).xyz,(float4(0.300000,0.590000,0.110000,0.000000)).xyz).xxxx).w;
    // 60: mad r1.xyz, -cb0[8].yyyy, r1.xyzx, r1.wwww
    r1.xyz = ((-(source[8].yyyy))*(r1.xyzx)+(r1.wwww)).xyz;
    // 61: mad r1.xyz, cb0[8].zzzz, r1.xyzx, r2.xyzx
    r1.xyz = ((source[8].zzzz)*(r1.xyzx)+(r2.xyzx)).xyz;
    // 62: mul r1.xyz, r1.xyzx, cb0[2].xyzx
    r1.xyz = ((r1.xyzx)*(source[2].xyzx)).xyz;
    // 63: dp3 r1.w, r0.yzwy, l(0.300000, 0.590000, 0.110000, 0.000000)
    r1.w = (dot((r0.yzwy).xyz,(float4(0.300000,0.590000,0.110000,0.000000)).xyz).xxxx).w;
    // 64: add r2.xyz, -r0.yzwy, r1.wwww
    r2.xyz = ((-(r0.yzwy))+(r1.wwww)).xyz;
    // 65: mad r0.yzw, cb0[8].wwww, r2.xxyz, r0.yyzw
    r0.yzw = ((source[8].wwww)*(r2.xxyz)+(r0.yyzw)).yzw;
    // 66: max r0.yzw, |r0.yyzw|, l(0.000000, 0.000001, 0.000001, 0.000001)
    r0.yzw = (max(abs(r0.yyzw),float4(0.000000,0.000001,0.000001,0.000001))).yzw;
    // 67: log r0.yzw, r0.yyzw
    r0.yzw = (log2(r0.yyzw)).yzw;
    // 68: mul r0.yzw, r0.yyzw, cb0[9].xxxx
    r0.yzw = ((r0.yyzw)*(source[9].xxxx)).yzw;
    // 69: exp r0.yzw, r0.yyzw
    r0.yzw = (exp2(r0.yyzw)).yzw;
    // 70: mul r0.yzw, r0.yyzw, cb0[9].yyyy
    r0.yzw = ((r0.yyzw)*(source[9].yyyy)).yzw;
    // 71: mul r0.yzw, r0.yyzw, r1.xxyz
    r0.yzw = ((r0.yyzw)*(r1.xxyz)).yzw;
    // 72: mul r0.yzw, r0.yyzw, cb0[9].zzzz
    r0.yzw = ((r0.yyzw)*(source[9].zzzz)).yzw;
    // 73: max r0.yzw, |r0.yyzw|, l(0.000000, 0.000001, 0.000001, 0.000001)
    r0.yzw = (max(abs(r0.yyzw),float4(0.000000,0.000001,0.000001,0.000001))).yzw;
    // 74: log r0.yzw, r0.yyzw
    r0.yzw = (log2(r0.yyzw)).yzw;
    // 75: mul r0.yzw, r0.yyzw, cb0[9].wwww
    ...
    r0.yzw = ((r0.yyzw)*(v5.wwww)).yzw;
    // 79: mad r1.x, -r0.x, cb0[12].z, l(1.000000)
    r1.x = ((-(r0.xxxx))*(source[12].zzzz)+(float4(1.000000,1.000000,1.000000,1.000000))).x;
    // 80: mad r0.x, -r0.x, cb0[10].z, l(1.000000)
    r0.x = ((-(r0.xxxx))*(source[10].zzzz)+(float4(1.000000,1.000000,1.000000,1.000000))).x;
    // 81: mul_sat r0.x, r0.x, cb0[11].z
    r0.x = (saturate((r0.xxxx)*(source[11].zzzz))).x;
    // 82: add r0.x, -r0.x, l(1.000000)
    r0.x = ((-(r0.xxxx))+(float4(1.000000,1.000000,1.000000,1.000000))).x;
    // 83: mul_sat r1.x, r1.x, cb0[13].z
    r1.x = (saturate((r1.xxxx)*(source[13].zzzz))).x;
    // 84: log r1.y, r0.x
    r1.y = (log2(r0.xxxx)).y;
    // 85: lt r0.x, r0.x, l(0.000001)
    r0.x = (asfloat((uint4)((r0.xxxx)<(float4(0.000001,0.000001,0.000001,0.000001))) * 0xffffffffu)).x;
    // 86: mul r1.y, r1.y, cb0[11].w
    r1.y = ((r1.yyyy)*(source[11].wwww)).y;
    // 87: exp r1.y, r1.y
    r1.y = (exp2(r1.yyyy)).y;
    // 88: mul r1.x, r1.x, r1.y
    r1.x = ((r1.xxxx)*(r1.yyyy)).x;
    // 89: mul r1.x, r1.x, cb0[13].w
    r1.x = ((r1.xxxx)*(source[13].wwww)).x;
    // 90: mul r1.x, r1.x, v3.w
    r1.x = ((r1.xxxx)*(v3.wwww)).x;
    // 91: mul r1.x, r1.x, cb0[0].x
    r1.x = ((r1.xxxx)*(source[0].xxxx)).x;
    // 92: movc r0.x, r0.x, l(0), r1.x
    r0.x = ((asuint(r0.xxxx) != 0u) ? (float4(asfloat(0u),asfloat(0u),asfloat(0u),asfloat(0u))) : (r1.xxxx)).x;
    // 93: mul o0.xyz, r0.xxxx, r0.yzwy
    output.xyz = ((r0.xxxx)*(r0.yzwy)).xyz;
    // 94: mov o0.w, l(0)
    output.w = (float4(asfloat(0u),asfloat(0u),asfloat(0u),asfloat(0u))).w;
    return output;
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2606Distortion(ARTIST_NATIVE_INPUT input)
{
    float4 source[9]; [unroll] for (uint i=0u; i<9u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
    ...
    source[1].y = (g_ArtistSourceMaterialParameters[4u].zzzz).x;
    source[1].z = (g_ArtistSourceMaterialParameters[4u].wwww).x;
    source[1].w = (g_ArtistSourceMaterialParameters[5u].zzzz).x;
    source[2].x = ((g_ArtistSourceMaterialParameters[4u].zzzz*g_ArtistSourceMaterialTime.xxxx)).x;
    source[2].y = (g_ArtistSourceMaterialParameters[3u].xxxx).x;
    source[2].z = (g_ArtistSourceMaterialParameters[5u].xxxx).x;
    source[2].w = (g_ArtistSourceMaterialParameters[5u].yyyy).x;
    source[3].x = ((g_ArtistSourceMaterialParameters[5u].yyyy*g_ArtistSourceMaterialTime.xxxx)).x;
    ...
    source[4].x = ((g_ArtistSourceMaterialParameters[2u].wwww*g_ArtistSourceMaterialTime.xxxx)).x;
    source[4].y = (g_ArtistSourceMaterialParameters[6u].zzzz).x;
    source[4].z = (g_ArtistSourceMaterialParameters[6u].wwww).x;
    source[4].w = (g_ArtistSourceMaterialParameters[5u].wwww).x;
    source[5].x = ((g_ArtistSourceMaterialParameters[5u].wwww*g_ArtistSourceMaterialTime.xxxx)).x;
    source[5].y = (((g_ArtistSourceMaterialParameters[5u].wwww*g_ArtistSourceMaterialTime.xxxx)*g_ArtistSourceMaterialParameters[5u].xxxx)).x;
    ...
    source[8].y = ((float4(0.0, 0.0, 0.0, 0.0)*g_ArtistSourceMaterialParameters[3u].yyyy)).x;
    float4 passValues[4]; [unroll] for(uint passIndex=0u;passIndex<4u;++passIndex) passValues[passIndex]=0.f;
    passValues[0]=float4(.5f,-.5f,.5f,.5f);
    float4 v0 = float4(input.sourceBasisX,0.f); // native texcoord10
    float4 v1 = float4(input.uv,float2(0.f,0.f)); // native texcoord0
    ...
    r0.z = ((r0.zzzz)+(r0.zzzz)).z;
    // 29: mul r0.yz, r0.yyzy, cb0[1].zzwz
    r0.yz = ((r0.yyzy)*(source[1].zzwz)).yz;
    // 30: mad r0.y, r0.y, l(0.318310), r0.z
    r0.y = ((r0.yyyy)*(float4(0.318310,0.318310,0.318310,0.318310))+(r0.zzzz)).y;
    // 31: add r0.y, r0.y, cb0[2].x
    r0.y = ((r0.yyyy)+(source[2].xxxx)).y;
    // 32: mul r1.x, r0.y, cb0[4].y
    r1.x = ((r0.yyyy)*(source[4].yyyy)).x;
    // 33: mul r2.x, r0.y, cb0[3].y
    r2.x = ((r0.yyyy)*(source[3].yyyy)).x;
    // 34: add r0.y, r0.x, r0.x
    r0.y = ((r0.xxxx)+(r0.xxxx)).y;
    ...
    // 36: log r0.y, r0.y
    r0.y = (log2(r0.yyyy)).y;
    // 37: mul r0.y, r0.y, cb0[2].y
    r0.y = ((r0.yyyy)*(source[2].yyyy)).y;
    // 38: exp r0.y, r0.y
    r0.y = (exp2(r0.yyyy)).y;
    // 39: mul r0.y, r0.y, cb0[2].z
    r0.y = ((r0.yyyy)*(source[2].zzzz)).y;
    // 40: movc r0.x, r0.x, l(0), r0.y
    r0.x = ((asuint(r0.xxxx) != 0u) ? (float4(asfloat(0u),asfloat(0u),asfloat(0u),asfloat(0u))) : (r0.yyyy)).x;
    // 41: add r0.y, v3.x, cb0[3].x
    r0.y = ((v3.xxxx)+(source[3].xxxx)).y;
    // 42: mad r0.x, r0.y, l(-0.400000), r0.x
    r0.x = ((r0.yyyy)*(float4(-0.400000,-0.400000,-0.400000,-0.400000))+(r0.xxxx)).x;
    // 43: mad r1.y, r0.x, cb0[4].z, cb0[5].y
    r1.y = ((r0.xxxx)*(source[4].zzzz)+(source[5].yyyy)).y;
    // 44: mad r2.y, r0.x, cb0[3].z, cb0[4].x
    r2.y = ((r0.xxxx)*(source[3].zzzz)+(source[4].xxxx)).y;
    // 45: sample_l_indexable(texture2d)(float,float,float,float) r0.xy, r1.xyxx, t0.xyzw, s2, l(0.000000)
    r0.xy = (ArtistNativeSample1((r1.xyxx).xy, (float4(0.000000,0.000000,0.000000,0.000000)).x, true).xyzw).xy;
    // 46: max r0.xy, |r0.xyxx|, l(0.000001, 0.000001, 0.000000, 0.000000)
    r0.xy = (max(abs(r0.xyxx),float4(0.000001,0.000001,0.000000,0.000000))).xy;
    // 47: log r0.xy, r0.xyxx
    r0.xy = (log2(r0.xyxx)).xy;
    // 48: mul r0.xy, r0.xyxx, cb0[5].zzzz
    r0.xy = ((r0.xyxx)*(source[5].zzzz)).xy;
    // 49: exp r0.xy, r0.xyxx
    r0.xy = (exp2(r0.xyxx)).xy;
    // 50: mad r0.zw, cb0[5].wwww, r0.xxxy, r2.xxxy
    r0.zw = ((source[5].wwww)*(r0.xxxy)+(r2.xxxy)).zw;
    // 51: mad r0.xy, cb0[5].wwww, r0.xyxx, v1.xyxx
    r0.xy = ((source[5].wwww)*(r0.xyxx)+(v1.xyxx)).xy;
    // 52: sample_b_indexable(texture2d)(float,float,float,float) r1.xyz, r0.xyxx, t2.xyzw, s3, l(0.000000)
    r1.xyz = (ArtistNativeSample2((r0.xyxx).xy, (float4(0.000000,0.000000,0.000000,0.000000)).x, false).xyzw).xyz;
    // 53: sample_l_indexable(texture2d)(float,float,float,float) r0.xyz, r0.zwzz, t1.xyzw, s1, l(0.000000)
    r0.xyz = (ArtistNativeSample0((r0.zwzz).xy, (float4(0.000000,0.000000,0.000000,0.000000)).x, true).xyzw).xyz;
    // 54: max r0.xyz, |r0.xyzx|, l(0.000001, 0.000001, 0.000001, 0.000000)
    r0.xyz = (max(abs(r0.xyzx),float4(0.000001,0.000001,0.000001,0.000000))).xyz;
    // 55: log r0.xyz, r0.xyzx
    r0.xyz = (log2(r0.xyzx)).xyz;
    // 56: mul r0.xyz, r0.xyzx, cb0[6].xxxx
    r0.xyz = ((r0.xyzx)*(source[6].xxxx)).xyz;
    // 57: exp r0.xyz, r0.xyzx
    r0.xyz = (exp2(r0.xyzx)).xyz;
    // 58: mul r2.xyz, r0.xyzx, cb0[6].yyyy
    r2.xyz = ((r0.xyzx)*(source[6].yyyy)).xyz;
    // 59: dp3 r0.z, r2.xyzx, l(0.300000, 0.590000, 0.110000, 0.000000)
    r0.z = (dot((r2.xyzx).xyz,(float4(0.300000,0.590000,0.110000,0.000000)).xyz).xxxx).z;
    // 60: mad r0.xyzw, -cb0[6].yyyy, r0.xyxy, r0.zzzz
    r0.xyzw = ((-(source[6].yyyy))*(r0.xyxy)+(r0.zzzz)).xyzw;
    // 61: mad r0.xyzw, cb0[6].zzzz, r0.xyzw, r2.xyxy
    r0.xyzw = ((source[6].zzzz)*(r0.xyzw)+(r2.xyxy)).xyzw;
    // 62: mul r0.xyzw, r0.xyzw, cb0[0].xyxy
    r0.xyzw = ((r0.xyzw)*(source[0].xyxy)).xyzw;
    // 63: dp3 r1.z, r1.xyzx, l(0.300000, 0.590000, 0.110000, 0.000000)
    r1.z = (dot((r1.xyzx).xyz,(float4(0.300000,0.590000,0.110000,0.000000)).xyz).xxxx).z;
    // 64: add r2.xyzw, -r1.xyxy, r1.zzzz
    r2.xyzw = ((-(r1.xyxy))+(r1.zzzz)).xyzw;
    // 65: mad r1.xyzw, cb0[6].wwww, r2.xyzw, r1.xyxy
    r1.xyzw = ((source[6].wwww)*(r2.xyzw)+(r1.xyxy)).xyzw;
    // 66: max r1.xyzw, |r1.xyzw|, l(0.000001, 0.000001, 0.000001, 0.000001)
    r1.xyzw = (max(abs(r1.xyzw),float4(0.000001,0.000001,0.000001,0.000001))).xyzw;
    // 67: log r1.xyzw, r1.xyzw
    r1.xyzw = (log2(r1.xyzw)).xyzw;
    // 68: mul r1.xyzw, r1.xyzw, cb0[7].xxxx
    r1.xyzw = ((r1.xyzw)*(source[7].xxxx)).xyzw;
    // 69: exp r1.xyzw, r1.xyzw
    r1.xyzw = (exp2(r1.xyzw)).xyzw;
    // 70: mul r1.xyzw, r1.xyzw, cb0[7].yyyy
    r1.xyzw = ((r1.xyzw)*(source[7].yyyy)).xyzw;
    // 71: mul r0.xyzw, r0.xyzw, r1.xyzw
    r0.xyzw = ((r0.xyzw)*(r1.xyzw)).xyzw;
    // 72: mul r0.xyzw, r0.xyzw, cb0[7].zzzz
    r0.xyzw = ((r0.xyzw)*(source[7].zzzz)).xyzw;
    // 73: max r0.xyzw, |r0.xyzw|, l(0.000001, 0.000001, 0.000001, 0.000001)
    r0.xyzw = (max(abs(r0.xyzw),float4(0.000001,0.000001,0.000001,0.000001))).xyzw;
    // 74: log r0.xyzw, r0.xyzw
    r0.xyzw = (log2(r0.xyzw)).xyzw;
    // 75: mul r0.xyzw, r0.xyzw, cb0[7].wwww
    r0.xyzw = ((r0.xyzw)*(source[7].wwww)).xyzw;
    // 76: exp r0.xyzw, r0.xyzw
    r0.xyzw = (exp2(r0.xyzw)).xyzw;
    // 77: mul r0.xyzw, r0.xyzw, cb0[8].yyyy
    r0.xyzw = ((r0.xyzw)*(source[8].yyyy)).xyzw;
    // 78: mad r0.xyzw, r0.xyzw, l(2.000000, -2.000000, 2.000000, -2.000000), l(-1.000000, 1.000000, -1.000000, 1.000000)
    r0.xyzw = ((r0.xyzw)*(float4(2.000000,-2.000000,2.000000,-2.000000))+(float4(-1.000000,1.000000,-1.000000,1.000000))).xyzw;
    // 79: mad r0.xyzw, r0.xyzw, cb2[0].xyxy, cb2[0].wzwz
    r0.xyzw = ((r0.xyzw)*(passValues[0].xyxy)+(passValues[0].wzwz)).xyzw;
    // 80: dp2 r1.x, r0.zwzz, r0.zwzz
    r1.x = (dot((r0.zwzz).xy,(r0.zwzz).xy).xxxx).x;
    // 81: add r1.x, r1.x, l(-0.100000)
    r1.x = ((r1.xxxx)+(float4(-0.100000,-0.100000,-0.100000,-0.100000))).x;
    // 82: lt r1.x, r1.x, l(0.000000)
    r1.x = (asfloat((uint4)((r1.xxxx)<(float4(0.000000,0.000000,0.000000,0.000000))) * 0xffffffffu)).x;
    // 83: discard_nz r1.x
    if ((asuint(r1.xxxx)).x != 0u) return 0.f;
    // 84: div r1.xy, v4.xyxx, v4.wwww
    r1.xy = ((v4.xyxx)/(v4.wwww)).xy;
    // 85: mad r1.xy, r1.xyxx, cb2[0].xyxx, cb2[0].wzww
    r1.xy = ((r1.xyxx)*(passValues[0].xyxx)+(passValues[0].wzww)).xy;
    // 86: mad r0.xy, r0.xyxx, l(0.003922, -0.003922, 0.000000, 0.000000), r1.xyxx
    r0.xy = ((r0.xyxx)*(float4(0.003922,-0.003922,0.000000,0.000000))+(r1.xyxx)).xy;
    // 87: mul r0.zw, r0.zzzw, l(0.000000, 0.000000, 4.000000, 4.000000)
    r0.zw = ((r0.zzzw)*(float4(0.000000,0.000000,4.000000,4.000000))).zw;
    // 88: max r0.zw, r0.zzzw, l(0.000000, 0.000000, -255.000000, -255.000000)
    r0.zw = (max(r0.zzzw,float4(0.000000,0.000000,-255.000000,-255.000000))).zw;
    // 89: min r0.zw, r0.zzzw, l(0.000000, 0.000000, 255.000000, 255.000000)
    r0.zw = (min(r0.zzzw,float4(0.000000,0.000000,255.000000,255.000000))).zw;
    // 90: mul r0.zw, r0.zzzw, l(0.000000, 0.000000, 0.003922, 0.003922)
    r0.zw = ((r0.zzzw)*(float4(0.000000,0.000000,0.003922,0.003922))).zw;
    // Native 91: source device depth mapped to centimetre view depth; reconstruction at 93.
    r0.x = g_EffectSceneDepthTexture.SampleLevel(EffectSliceDepthSampler, (r0.xyxx).xy, 0.f).y * 100000.f;
    // Native 93-96: reconstructed view depth is supplied by the runtime adapter.
    r0.x = r0.x;
    // 97: ge r0.x, v4.w, r0.x
    r0.x = (asfloat((uint4)((v4.wwww)>=(r0.xxxx)) * 0xffffffffu)).x;
    // 98: movc r0.xy, r0.xxxx, l(0,0,0,0), r0.zwzz
    r0.xy = ((asuint(r0.xxxx) != 0u) ? (float4(asfloat(0u),asfloat(0u),asfloat(0u),asfloat(0u))) : (r0.zwzz)).xy;
    // 99: max o0.xy, r0.xyxx, l(0.000000, 0.000000, 0.000000, 0.000000)
    output.xy = (max(r0.xyxx,float4(0.000000,0.000000,0.000000,0.000000))).xy;
    // 100: min r0.xy, r0.xyxx, l(0.000000, 0.000000, 0.000000, 0.000000)
    r0.xy = (min(r0.xyxx,float4(0.000000,0.000000,0.000000,0.000000))).xy;
    // 101: mov o0.zw, -r0.xxxy
    output.zw = (-(r0.xxxy)).zw;
    return output;
}
#endif
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_PARTICLE_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2607(ARTIST_NATIVE_INPUT input)
{
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_PARTICLE_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2607Distortion(ARTIST_NATIVE_INPUT input)
{
    float4 source[8]; [unroll] for (uint i=0u; i<8u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2608(ARTIST_NATIVE_INPUT input)
{
    float4 source[5]; [unroll] for (uint i=0u; i<5u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2608Distortion(ARTIST_NATIVE_INPUT input)
{
    float4 source[3]; [unroll] for (uint i=0u; i<3u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2609(ARTIST_NATIVE_INPUT input)
{
    float4 source[3]; [unroll] for (uint i=0u; i<3u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2609Distortion(ARTIST_NATIVE_INPUT input)
{
    float4 source[3]; [unroll] for (uint i=0u; i<3u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2611(ARTIST_NATIVE_INPUT input)
{
    float4 source[18]; [unroll] for (uint i=0u; i<18u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_PARTICLE_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2612(ARTIST_NATIVE_INPUT input)
{
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_PARTICLE_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2612Distortion(ARTIST_NATIVE_INPUT input)
{
    float4 source[3]; [unroll] for (uint i=0u; i<3u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2613(ARTIST_NATIVE_INPUT input)
{
    float4 source[8]; [unroll] for (uint i=0u; i<8u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2613Distortion(ARTIST_NATIVE_INPUT input)
{
    float4 source[3]; [unroll] for (uint i=0u; i<3u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
float4 ArtistNative2614(ARTIST_NATIVE_INPUT input)
{
    float4 source[8]; [unroll] for (uint i=0u; i<8u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_PARTICLE_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
#ifndef ARTIST_NATIVE_MODEL_ONLY
// fx_o_de_behitmonfan_02_01_tr: c390f9f83fce2b4791421e47db4cdcc6; selected map fb632bff3d574a802c5e7ef44c24a8514c6e5327192d19db02e0f94785d9398e.
float4 ArtistNative2615(ARTIST_NATIVE_INPUT input)
{
    ...
    source[6] = g_ArtistSourceMaterialParameters[1u];
    source[7] = ArtistNativeAppend(sin((g_ArtistSourceMaterialParameters[0u].xxxx*float4(3.1400001, 0.0, 0.0, 0.0))),cos((g_ArtistSourceMaterialParameters[0u].xxxx*float4(3.1400001, 0.0, 0.0, 0.0))),1u);
    source[8] = ArtistNativeAppend(cos(((float4(0.5, 0.0, 0.0, 0.0)+g_ArtistSourceMaterialParameters[0u].xxxx)*float4(3.1400001, 0.0, 0.0, 0.0))),(float4(-1.0, 0.0, 0.0, 0.0)*sin(((float4(0.5, 0.0, 0.0, 0.0)+g_ArtistSourceMaterialParameters[0u].xxxx)*float4(3.1400001, 0.0, 0.0, 0.0)))),1u);
    source[9] = ArtistNativeAppend(sin(((float4(0.5, 0.0, 0.0, 0.0)+g_ArtistSourceMaterialParameters[0u].xxxx)*float4(3.1400001, 0.0, 0.0, 0.0))),cos(((float4(0.5, 0.0, 0.0, 0.0)+g_ArtistSourceMaterialParameters[0u].xxxx)*float4(3.1400001, 0.0, 0.0, 0.0))),1u);
    source[10] = ArtistNativeAppend(cos(((float4(0.5, 0.0, 0.0, 0.0)+g_ArtistSourceMaterialParameters[0u].xxxx)*float4(-3.1400001, 0.0, 0.0, 0.0))),(float4(-1.0, 0.0, 0.0, 0.0)*sin(((float4(0.5, 0.0, 0.0, 0.0)+g_ArtistSourceMaterialParameters[0u].xxxx)*float4(-3.1400001, 0.0, 0.0, 0.0)))),1u);
    source[11] = ArtistNativeAppend(sin(((float4(0.5, 0.0, 0.0, 0.0)+g_ArtistSourceMaterialParameters[0u].xxxx)*float4(-3.1400001, 0.0, 0.0, 0.0))),cos(((float4(0.5, 0.0, 0.0, 0.0)+g_ArtistSourceMaterialParameters[0u].xxxx)*float4(-3.1400001, 0.0, 0.0, 0.0))),1u);
    source[12] = ArtistNativeAppend(sin((g_ArtistSourceMaterialParameters[0u].xxxx*float4(-3.1400001, 0.0, 0.0, 0.0))),cos((g_ArtistSourceMaterialParameters[0u].xxxx*float4(-3.1400001, 0.0, 0.0, 0.0))),1u);
    source[13].x = ((float4(1.0, 0.0, 0.0, 0.0)/max(float4(9.99999975e-06, 0.0, 0.0, 0.0),(g_ArtistSourceMaterialParameters[0u].zzzz*float4(0.5, 0.0, 0.0, 0.0))))).x;
    source[13].y = (g_ArtistSourceMaterialParameters[0u].yyyy).x;
    source[13].z = (g_ArtistSourceMaterialParameters[0u].xxxx).x;
    source[13].w = ((float4(1.0, 0.0, 0.0, 0.0)-g_ArtistSourceMaterialParameters[0u].xxxx)).x;
    source[14].x = (cos((g_ArtistSourceMaterialParameters[0u].xxxx*float4(-3.1400001, 0.0, 0.0, 0.0)))).x;
    source[14].y = (g_ArtistSourceMaterialParameters[0u].wwww).x;
    float4 passValues[4]; [unroll] for(uint passIndex=0u;passIndex<4u;++passIndex) passValues[passIndex]=0.f;
    passValues[0]=float4(.5f,-.5f,.5f,.5f);
    passValues[3]=float4(0.f,0.f,0.f,1.f); // Neutral source diffuse override: preserve material color.
    ...
    // 10: discard_nz r0.x
    if ((asuint(r0.xxxx)).x != 0u) clip(-1.f);
    // 11: add r0.xy, v7.xyxx, cb0[0].xyxx
    r0.xy = ((v7.xyxx)+(source[0].xyxx)).xy;
    // 12: mul r0.xy, r0.xyxx, l(0.015000, 0.017250, 0.000000, 0.000000)
    r0.xy = ((r0.xyxx)*(float4(0.015000,0.017250,0.000000,0.000000))).xy;
    // 13: sample_b_indexable(texture2d)(float,float,float,float) r0.xyz, r0.xyxx, t1.xyzw, s0, l(0.000000)
    r0.xyz = (ArtistNativeSample0((r0.xyxx).xy, (float4(0.000000,0.000000,0.000000,0.000000)).x, false).xyzw).xyz;
    // 14: add r1.xyzw, r0.xxyz, l(-0.125000, -0.125000, -0.125000, -0.125000)
    r1.xyzw = ((r0.xxyz)+(float4(-0.125000,-0.125000,-0.125000,-0.125000))).xyzw;
    // 15: mul_sat r1.xyzw, r1.xyzw, l(5.000000, 5.000000, 5.000000, 5.000000)
    r1.xyzw = (saturate((r1.xyzw)*(float4(5.000000,5.000000,5.000000,5.000000)))).xyzw;
    // 16: add r1.xyzw, -r1.xyzw, l(1.000000, 1.000000, 1.000000, 1.000000)
    r1.xyzw = ((-(r1.xyzw))+(float4(1.000000,1.000000,1.000000,1.000000))).xyzw;
    // 17: max r1.xyzw, r1.xyzw, l(0.060000, 0.060000, 0.060000, 0.060000)
    r1.xyzw = (max(r1.xyzw,float4(0.060000,0.060000,0.060000,0.060000))).xyzw;
    // 18: add r2.xy, v4.xyxx, l(-0.500000, -0.500000, 0.000000, 0.000000)
    r2.xy = ((v4.xyxx)+(float4(-0.500000,-0.500000,0.000000,0.000000))).xy;
    // 19: max r0.w, |r2.y|, |r2.x|
    r0.w = (max(abs(r2.yyyy),abs(r2.xxxx))).w;
    // 20: div r0.w, l(1.000000, 1.000000, 1.000000, 1.000000), r0.w
    r0.w = ((float4(1.000000,1.000000,1.000000,1.000000))/(r0.wwww)).w;
    // 21: min r2.z, |r2.y|, |r2.x|
    r2.z = (min(abs(r2.yyyy),abs(r2.xxxx))).z;
    // 22: mul r0.w, r0.w, r2.z
    r0.w = ((r0.wwww)*(r2.zzzz)).w;
    // 23: mul r2.z, r0.w, r0.w
    r2.z = ((r0.wwww)*(r0.wwww)).z;
    // 24: mad r2.w, r2.z, l(0.020835), l(-0.085133)
    r2.w = ((r2.zzzz)*(float4(0.020835,0.020835,0.020835,0.020835))+(float4(-0.085133,-0.085133,-0.085133,-0.085133))).w;
    // 25: mad r2.w, r2.z, r2.w, l(0.180141)
    r2.w = ((r2.zzzz)*(r2.wwww)+(float4(0.180141,0.180141,0.180141,0.180141))).w;
    // 26: mad r2.w, r2.z, r2.w, l(-0.330299)
    r2.w = ((r2.zzzz)*(r2.wwww)+(float4(-0.330299,-0.330299,-0.330299,-0.330299))).w;
    // 27: mad r2.z, r2.z, r2.w, l(0.999866)
    r2.z = ((r2.zzzz)*(r2.wwww)+(float4(0.999866,0.999866,0.999866,0.999866))).z;
    // 28: mul r2.w, r0.w, r2.z
    r2.w = ((r0.wwww)*(r2.zzzz)).w;
    // 29: mad r2.w, r2.w, l(-2.000000), l(1.570796)
    r2.w = ((r2.wwww)*(float4(-2.000000,-2.000000,-2.000000,-2.000000))+(float4(1.570796,1.570796,1.570796,1.570796))).w;
    // 30: lt r3.x, |r2.y|, |r2.x|
    r3.x = (asfloat((uint4)((abs(r2.yyyy))<(abs(r2.xxxx))) * 0xffffffffu)).x;
    // 31: and r2.w, r2.w, r3.x
    r2.w = (asfloat(asuint(r2.wwww) & asuint(r3.xxxx))).w;
    // 32: mad r0.w, r0.w, r2.z, r2.w
    r0.w = ((r0.wwww)*(r2.zzzz)+(r2.wwww)).w;
    // 33: lt r2.z, r2.y, -r2.y
    r2.z = (asfloat((uint4)((r2.yyyy)<(-(r2.yyyy))) * 0xffffffffu)).z;
    // 34: and r2.z, r2.z, l(0xc0490fdb)
    r2.z = (asfloat(asuint(r2.zzzz) & uint4(0xc0490fdbu,0xc0490fdbu,0xc0490fdbu,0xc0490fdbu))).z;
    // 35: add r0.w, r0.w, r2.z
    r0.w = ((r0.wwww)+(r2.zzzz)).w;
    // 36: min r2.z, r2.y, r2.x
    r2.z = (min(r2.yyyy,r2.xxxx)).z;
    // 37: lt r2.z, r2.z, -r2.z
    r2.z = (asfloat((uint4)((r2.zzzz)<(-(r2.zzzz))) * 0xffffffffu)).z;
    // 38: max r2.w, r2.y, r2.x
    r2.w = (max(r2.yyyy,r2.xxxx)).w;
    // 39: ge r2.w, r2.w, -r2.w
    r2.w = (asfloat((uint4)((r2.wwww)>=(-(r2.wwww))) * 0xffffffffu)).w;
    // 40: and r2.z, r2.w, r2.z
    r2.z = (asfloat(asuint(r2.wwww) & asuint(r2.zzzz))).z;
    // 41: movc r0.w, r2.z, -r0.w, r0.w
    r0.w = ((asuint(r2.zzzz) != 0u) ? (-(r0.wwww)) : (r0.wwww)).w;
    // 42: mad r3.x, r0.w, l(0.159155), l(0.500000)
    r3.x = ((r0.wwww)*(float4(0.159155,0.159155,0.159155,0.159155))+(float4(0.500000,0.500000,0.500000,0.500000))).x;
    // 43: dp2 r2.z, r2.xyxx, r2.xyxx
    r2.z = (dot((r2.xyxx).xy,(r2.xyxx).xy).xxxx).z;
    // 44: sqrt r2.z, r2.z
    r2.z = (sqrt(r2.zzzz)).z;
    // 45: mad r2.w, -r2.z, l(2.000000), l(1.000000)
    r2.w = ((-(r2.zzzz))*(float4(2.000000,2.000000,2.000000,2.000000))+(float4(1.000000,1.000000,1.000000,1.000000))).w;
    // 46: max r3.z, r2.w, l(0.000000)
    r3.z = (max(r2.wwww,float4(0.000000,0.000000,0.000000,0.000000))).z;
    // 47: mul_sat r2.w, r2.w, l(100000.000000)
    r2.w = (saturate((r2.wwww)*(float4(100000.000000,100000.000000,100000.000000,100000.000000)))).w;
    // 48: add r3.y, -r3.z, l(1.000000)
    r3.y = ((-(r3.zzzz))+(float4(1.000000,1.000000,1.000000,1.000000))).y;
    // 49: mul r3.z, r3.z, l(300.000000)
    r3.z = ((r3.zzzz)*(float4(300.000000,300.000000,300.000000,300.000000))).z;
    // 50: min r3.z, r3.z, l(1.000000)
    r3.z = (min(r3.zzzz,float4(1.000000,1.000000,1.000000,1.000000))).z;
    // 51: add r4.xy, r3.xyxx, cb0[5].xyxx
    r4.xy = ((r3.xyxx)+(source[5].xyxx)).xy;
    // 52: add r3.x, -r3.x, l(0.500000)
    r3.x = ((-(r3.xxxx))+(float4(0.500000,0.500000,0.500000,0.500000))).x;
    // 53: sample_l_indexable(texture2d)(float,float,float,float) r4.xyz, r4.xyxx, t2.xyzw, s1, l(0.000000)
    r4.xyz = (ArtistNativeSample1((r4.xyxx).xy, (float4(0.000000,0.000000,0.000000,0.000000)).x, true).xyzw).xyz;
    // 54: add r4.xyz, -r4.xyzx, l(1.000000, 1.000000, 1.000000, 0.000000)
    r4.xyz = ((-(r4.xyzx))+(float4(1.000000,1.000000,1.000000,0.000000))).xyz;
    // 55: add r0.xyz, r0.xyzx, -r4.xyzx
    r0.xyz = ((r0.xyzx)+(-(r4.xyzx))).xyz;
    // 56: mul_sat r0.xyz, r0.xyzx, l(10.000000, 10.000000, 10.000000, 0.000000)
    r0.xyz = (saturate((r0.xyzx)*(float4(10.000000,10.000000,10.000000,0.000000)))).xyz;
    // 57: add r0.x, r0.y, r0.x
    r0.x = ((r0.yyyy)+(r0.xxxx)).x;
    // 58: add r0.x, r0.z, r0.x
    r0.x = ((r0.zzzz)+(r0.xxxx)).x;
    // 59: mul r0.xw, r0.xxxw, l(0.333330, 0.000000, 0.000000, 0.159155)
    r0.xw = ((r0.xxxw)*(float4(0.333330,0.000000,0.000000,0.159155))).xw;
    // 60: mul r1.xyzw, r1.xyzw, r0.xxxx
    r1.xyzw = ((r1.xyzw)*(r0.xxxx)).xyzw;
    // 61: mad r0.x, -r2.z, cb0[13].x, l(1.000000)
    r0.x = ((-(r2.zzzz))*(source[13].xxxx)+(float4(1.000000,1.000000,1.000000,1.000000))).x;
    // 62: mul_sat r0.x, r0.x, l(100000.000000)
    r0.x = (saturate((r0.xxxx)*(float4(100000.000000,100000.000000,100000.000000,100000.000000)))).x;
    // 63: mul r1.xyzw, r1.xyzw, r0.xxxx
    r1.xyzw = ((r1.xyzw)*(r0.xxxx)).xyzw;
    // 64: mul r0.x, r0.x, l(0.125000)
    r0.x = ((r0.xxxx)*(float4(0.125000,0.125000,0.125000,0.125000))).x;
    // 65: mad r1.xyzw, r1.xyzw, l(0.800000, 0.800000, 0.800000, 0.800000), r0.xxxx
    r1.xyzw = ((r1.xyzw)*(float4(0.800000,0.800000,0.800000,0.800000))+(r0.xxxx)).xyzw;
    // 66: dp2 r0.x, cb0[10].xyxx, r2.xyxx
    r0.x = (dot((source[10].xyxx).xy,(r2.xyxx).xy).xxxx).x;
    // 67: dp2 r0.y, cb0[11].xyxx, r2.xyxx
    r0.y = (dot((source[11].xyxx).xy,(r2.xyxx).xy).xxxx).y;
    // 68: add r0.xy, r0.xyxx, l(0.500000, 0.500000, 0.000000, 0.000000)
    r0.xy = ((r0.xyxx)+(float4(0.500000,0.500000,0.000000,0.000000))).xy;
    // 69: sample_b_indexable(texture2d)(float,float,float,float) r0.x, r0.xyxx, t0.xyzw, s2, l(0.000000)
    r0.x = (ArtistNativeSample2((r0.xyxx).xy, (float4(0.000000,0.000000,0.000000,0.000000)).x, false).xyzw).x;
    // 70: dp2 r0.y, cb0[12].xyxx, r2.xyxx
    r0.y = (dot((source[12].xyxx).xy,(r2.xyxx).xy).xxxx).y;
    // 71: mul_sat r0.y, r0.y, l(200.000000)
    r0.y = (saturate((r0.yyyy)*(float4(200.000000,200.000000,200.000000,200.000000)))).y;
    // 72: mul r0.x, r0.y, r0.x
    r0.x = ((r0.yyyy)*(r0.xxxx)).x;
    // 73: dp2 r4.x, cb0[8].xyxx, r2.xyxx
    r4.x = (dot((source[8].xyxx).xy,(r2.xyxx).xy).xxxx).x;
    // 74: dp2 r4.y, cb0[9].xyxx, r2.xyxx
    r4.y = (dot((source[9].xyxx).xy,(r2.xyxx).xy).xxxx).y;
    // 75: dp2 r0.y, cb0[7].xyxx, r2.xyxx
    r0.y = (dot((source[7].xyxx).xy,(r2.xyxx).xy).xxxx).y;
    // 76: mul_sat r0.y, r0.y, l(200.000000)
    r0.y = (saturate((r0.yyyy)*(float4(200.000000,200.000000,200.000000,200.000000)))).y;
    // 77: add r2.xy, r4.xyxx, l(0.500000, 0.500000, 0.000000, 0.000000)
    r2.xy = ((r4.xyxx)+(float4(0.500000,0.500000,0.000000,0.000000))).xy;
    // 78: sample_b_indexable(texture2d)(float,float,float,float) r0.z, r2.xyxx, t0.yzxw, s2, l(0.000000)
    r0.z = (ArtistNativeSample2((r2.xyxx).xy, (float4(0.000000,0.000000,0.000000,0.000000)).x, false).yzxw).z;
    // 79: mad_sat r0.x, r0.y, r0.z, r0.x
    r0.x = (saturate((r0.yyyy)*(r0.zzzz)+(r0.xxxx))).x;
    // 80: mul r0.x, r0.x, r0.x
    r0.x = ((r0.xxxx)*(r0.xxxx)).x;
    // 81: mul r0.x, r0.x, r0.x
    r0.x = ((r0.xxxx)*(r0.xxxx)).x;
    // 82: log r0.y, r3.y
    r0.y = (log2(r3.yyyy)).y;
    // 83: mul r0.y, r0.y, l(30.000000)
    r0.y = ((r0.yyyy)*(float4(30.000000,30.000000,30.000000,30.000000))).y;
    // 84: exp r0.y, r0.y
    r0.y = (exp2(r0.yyyy)).y;
    // 85: lt r0.z, r3.y, l(0.000001)
    r0.z = (asfloat((uint4)((r3.yyyy)<(float4(0.000001,0.000001,0.000001,0.000001))) * 0xffffffffu)).z;
    // 86: movc r0.y, r0.z, l(0), r0.y
    r0.y = ((asuint(r0.zzzz) != 0u) ? (float4(asfloat(0u),asfloat(0u),asfloat(0u),asfloat(0u))) : (r0.yyyy)).y;
    // 87: mad r0.x, r3.y, r0.x, r0.y
    r0.x = ((r3.yyyy)*(r0.xxxx)+(r0.yyyy)).x;
    // 88: mul r0.x, r3.z, r0.x
    r0.x = ((r3.zzzz)*(r0.xxxx)).x;
    // 89: min r0.x, r0.x, l(1.000000)
    r0.x = (min(r0.xxxx,float4(1.000000,1.000000,1.000000,1.000000))).x;
    // 90: mad r0.x, r0.x, l(0.200000), r1.x
    r0.x = ((r0.xxxx)*(float4(0.200000,0.200000,0.200000,0.200000))+(r1.xxxx)).x;
    // 91: add r1.xyz, r1.yzwy, l(1.000000, 1.000000, 1.000000, 0.000000)
    r1.xyz = ((r1.yzwy)+(float4(1.000000,1.000000,1.000000,0.000000))).xyz;
    // 92: ge r0.y, r3.x, l(0.000000)
    r0.y = (asfloat((uint4)((r3.xxxx)>=(float4(0.000000,0.000000,0.000000,0.000000))) * 0xffffffffu)).y;
    // 93: max r0.z, r3.x, l(0.000000)
    r0.z = (max(r3.xxxx,float4(0.000000,0.000000,0.000000,0.000000))).z;
    // 94: movc r0.y, r0.y, r0.z, r0.w
    r0.y = ((asuint(r0.yyyy) != 0u) ? (r0.zzzz) : (r0.wwww)).y;
    // 95: mad_sat r0.y, -r0.y, l(2.000000), l(1.000000)
    r0.y = (saturate((-(r0.yyyy))*(float4(2.000000,2.000000,2.000000,2.000000))+(float4(1.000000,1.000000,1.000000,1.000000)))).y;
    // 96: add r0.z, r0.y, -cb0[13].w
    r0.z = ((r0.yyyy)+(-(source[13].wwww))).z;
    // 97: mul_sat r0.z, r0.z, l(2000.000000)
    r0.z = (saturate((r0.zzzz)*(float4(2000.000000,2000.000000,2000.000000,2000.000000)))).z;
    // 98: mul r0.x, r0.x, r0.z
    r0.x = ((r0.xxxx)*(r0.zzzz)).x;
    // 99: mul r0.x, r0.x, cb0[2].w
    r0.x = ((r0.xxxx)*(source[2].wwww)).x;
    // 100: mul_sat r0.x, r0.x, cb0[14].y
    r0.x = (saturate((r0.xxxx)*(source[14].yyyy))).x;
    // 101: mul r0.x, r0.x, cb0[3].x
    r0.x = ((r0.xxxx)*(source[3].xxxx)).x;
    // 102: add r0.w, -|v4.w|, cb0[1].y
    r0.w = ((-(abs(v4.wwww)))+(source[1].yyyy)).w;
    // 103: mul r0.w, r0.w, l(5.000000)
    r0.w = ((r0.wwww)*(float4(5.000000,5.000000,5.000000,5.000000))).w;
    // 104: div_sat r0.w, r0.w, cb0[1].y
    r0.w = (saturate((r0.wwww)/(source[1].yyyy))).w;
    // 105: mul r0.w, r0.w, v4.z
    r0.w = ((r0.wwww)*(v4.zzzz)).w;
    // 106: mul o0.w, r0.w, r0.x
    output.w = ((r0.wwww)*(r0.xxxx)).w;
    // 107: lt r0.x, l(0.000000), cb0[13].y
    r0.x = (asfloat((uint4)((float4(0.000000,0.000000,0.000000,0.000000))<(source[13].yyyy)) * 0xffffffffu)).x;
    // 108: movc r0.x, r0.x, cb0[13].y, l(1.000000)
    r0.x = ((asuint(r0.xxxx) != 0u) ? (source[13].yyyy) : (float4(1.000000,1.000000,1.000000,1.000000))).x;
    // 109: ge r0.w, cb0[13].y, l(0.000000)
    r0.w = (asfloat((uint4)((source[13].yyyy)>=(float4(0.000000,0.000000,0.000000,0.000000))) * 0xffffffffu)).w;
    // 110: movc r0.x, r0.w, r0.x, cb0[13].y
    r0.x = ((asuint(r0.wwww) != 0u) ? (r0.xxxx) : (source[13].yyyy)).x;
    // 111: div r0.xw, l(0.003000, 0.000000, 0.000000, 0.002000), r0.xxxx
    r0.xw = ((float4(0.003000,0.000000,0.000000,0.002000))/(r0.xxxx)).xw;
    // 112: add r0.w, -r0.w, cb0[13].z
    r0.w = ((-(r0.wwww))+(source[13].zzzz)).w;
    // 113: add r0.xw, -r0.xxxw, l(0.500000, 0.000000, 0.000000, 1.000000)
    r0.xw = ((-(r0.xxxw))+(float4(0.500000,0.000000,0.000000,1.000000))).xw;
    // 114: add r0.y, -r0.w, r0.y
    r0.y = ((-(r0.wwww))+(r0.yyyy)).y;
    // 115: mul_sat r0.y, r0.y, l(2000.000000)
    r0.y = (saturate((r0.yyyy)*(float4(2000.000000,2000.000000,2000.000000,2000.000000)))).y;
    // 116: add r0.y, -r0.y, r0.z
    r0.y = ((-(r0.yyyy))+(r0.zzzz)).y;
    // 117: mul r0.y, r0.y, l(5.000000)
    r0.y = ((r0.yyyy)*(float4(5.000000,5.000000,5.000000,5.000000))).y;
    // 118: max r0.x, r0.x, l(0.000010)
    r0.x = (max(r0.xxxx,float4(0.000010,0.000010,0.000010,0.000010))).x;
    // 119: div r0.x, l(1.000000, 1.000000, 1.000000, 1.000000), r0.x
    r0.x = ((float4(1.000000,1.000000,1.000000,1.000000))/(r0.xxxx)).x;
    // 120: mad r0.x, -r2.z, r0.x, l(1.000000)
    r0.x = ((-(r2.zzzz))*(r0.xxxx)+(float4(1.000000,1.000000,1.000000,1.000000))).x;
    // 121: mul_sat r0.x, r0.x, l(100000.000000)
    r0.x = (saturate((r0.xxxx)*(float4(100000.000000,100000.000000,100000.000000,100000.000000)))).x;
    // 122: add r0.x, -r0.x, l(1.000000)
    r0.x = ((-(r0.xxxx))+(float4(1.000000,1.000000,1.000000,1.000000))).x;
    // 123: mul r0.x, r0.x, r2.w
    r0.x = ((r0.xxxx)*(r2.wwww)).x;
    // 124: mad r0.x, r0.x, l(5.000000), r0.y
    r0.x = ((r0.xxxx)*(float4(5.000000,5.000000,5.000000,5.000000))+(r0.yyyy)).x;
    // 125: add r0.xyz, r0.xxxx, r1.xyzx
    r0.xyz = ((r0.xxxx)+(r1.xyzx)).xyz;
    // 126: mul r1.xyz, cb0[2].xyzx, cb0[6].xyzx
...
#if !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
#ifndef ARTIST_NATIVE_MODEL_ONLY
// fx_k_pa_ht_02_ad: ea3e8860d5fea3429ec05d193a929497; selected map 8f87d54985091d3ab901d1c208b7a70bee69a38f2a322027fb9b83f28b130576.
float4 ArtistNative2616(ARTIST_NATIVE_INPUT input)
{
    float4 source[5]; [unroll] for (uint i=0u; i<5u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
    ...
    r0.x = ((asuint(r0.yyyy) != 0u) ? (float4(asfloat(0u),asfloat(0u),asfloat(0u),asfloat(0u))) : (r0.xxxx)).x;
    // 37: mul r0.yzw, r1.xxyz, cb0[3].wwww
    r0.yzw = ((r1.xxyz)*(source[3].wwww)).yzw;
    // 38: dp3 r1.w, r0.yzwy, l(0.300000, 0.590000, 0.110000, 0.000000)
    r1.w = (dot((r0.yzwy).xyz,(float4(0.300000,0.590000,0.110000,0.000000)).xyz).xxxx).w;
    // 39: mad r1.xyz, -cb0[3].wwww, r1.xyzx, r1.wwww
    r1.xyz = ((-(source[3].wwww))*(r1.xyzx)+(r1.wwww)).xyz;
    // 40: mad r0.yzw, cb0[4].xxxx, r1.xxyz, r0.yyzw
    r0.yzw = ((source[4].xxxx)*(r1.xxyz)+(r0.yyzw)).yzw;
    // 41: mad r0.yzw, r0.yyzw, v3.xxyz, cb0[1].xxyz
    r0.yzw = ((r0.yyzw)*(v3.xxyz)+(source[1].xxyz)).yzw;
    // 42: mul r0.yzw, r0.yyzw, v5.wwww
    r0.yzw = ((r0.yyzw)*(v5.wwww)).yzw;
    // 43: mul o0.xyz, r0.xxxx, r0.yzwy
    output.xyz = ((r0.xxxx)*(r0.yzwy)).xyz;
    // 44: mov o0.w, l(0)
    output.w = (float4(asfloat(0u),asfloat(0u),asfloat(0u),asfloat(0u))).w;
    return output;
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2616Distortion(ARTIST_NATIVE_INPUT input)
{
    float4 source[3]; [unroll] for (uint i=0u; i<3u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
    source[0].x = (g_ArtistSourceMaterialParameters[1u].zzzz).x;
    source[0].y = (g_ArtistSourceMaterialTime.xxxx).x;
    source[0].z = (g_ArtistSourceMaterialParameters[2u].xxxx).x;
    source[0].w = (g_ArtistSourceMaterialParameters[2u].yyyy).x;
    source[1].x = (g_ArtistSourceMaterialParameters[1u].wwww).x;
    ...
    source[2].x = (g_ArtistSourceMaterialParameters[0u].wwww).x;
    source[2].y = (g_ArtistSourceMaterialParameters[1u].xxxx).x;
    ...
    // Native 26-29: reconstructed view depth is supplied by the runtime adapter.
    r1.x = r1.x;
    // 30: add r1.x, r1.x, -v4.w
    r1.x = ((r1.xxxx)+(-(v4.wwww))).x;
    // 31: add r1.y, -cb0[2].z, l(1.000000)
    r1.y = ((-(source[2].zzzz))+(float4(1.000000,1.000000,1.000000,1.000000))).y;
    ...
    r0.x = ((asuint(r0.xxxx) != 0u) ? (float4(asfloat(0u),asfloat(0u),asfloat(0u),asfloat(0u))) : (r0.yyyy)).x;
    // 36: mul r1.xz, r0.xxxx, l(2.000000, 0.000000, 2.000000, 0.000000)
    r1.xz = ((r0.xxxx)*(float4(2.000000,0.000000,2.000000,0.000000))).xz;
    // 37: mov r1.yw, l(0,-0.000000,0,-0.000000)
    r1.yw = (float4(asfloat(0u),-0.000000,asfloat(0u),-0.000000)).yw;
    // 38: add r1.xyzw, r1.xyzw, l(-1.000000, 1.000000, -1.000000, 1.000000)
    r1.xyzw = ((r1.xyzw)+(float4(-1.000000,1.000000,-1.000000,1.000000))).xyzw;
    // 39: mad r1.xyzw, r1.xyzw, cb2[0].xyxy, cb2[0].wzwz
    r1.xyzw = ((r1.xyzw)*(passValues[0].xyxy)+(passValues[0].wzwz)).xyzw;
    // 40: dp2 r0.x, r1.zwzz, r1.zwzz
    r0.x = (dot((r1.zwzz).xy,(r1.zwzz).xy).xxxx).x;
    // 41: add r0.x, r0.x, l(-0.100000)
    r0.x = ((r0.xxxx)+(float4(-0.100000,-0.100000,-0.100000,-0.100000))).x;
    // 42: lt r0.x, r0.x, l(0.000000)
    r0.x = (asfloat((uint4)((r0.xxxx)<(float4(0.000000,0.000000,0.000000,0.000000))) * 0xffffffffu)).x;
    // 43: discard_nz r0.x
    if ((asuint(r0.xxxx)).x != 0u) return 0.f;
    // 44: mad r0.xy, r1.xyxx, l(0.003922, -0.003922, 0.000000, 0.000000), r0.zwzz
    r0.xy = ((r1.xyxx)*(float4(0.003922,-0.003922,0.000000,0.000000))+(r0.zwzz)).xy;
    // 45: mul r0.zw, r1.zzzw, l(0.000000, 0.000000, 4.000000, 4.000000)
    r0.zw = ((r1.zzzw)*(float4(0.000000,0.000000,4.000000,4.000000))).zw;
    // 46: max r0.zw, r0.zzzw, l(0.000000, 0.000000, -255.000000, -255.000000)
    r0.zw = (max(r0.zzzw,float4(0.000000,0.000000,-255.000000,-255.000000))).zw;
    // 47: min r0.zw, r0.zzzw, l(0.000000, 0.000000, 255.000000, 255.000000)
    r0.zw = (min(r0.zzzw,float4(0.000000,0.000000,255.000000,255.000000))).zw;
    // 48: mul r0.zw, r0.zzzw, l(0.000000, 0.000000, 0.003922, 0.003922)
    r0.zw = ((r0.zzzw)*(float4(0.000000,0.000000,0.003922,0.003922))).zw;
    // Native 49: source device depth mapped to centimetre view depth; reconstruction at 51.
    r0.x = g_EffectSceneDepthTexture.SampleLevel(EffectSliceDepthSampler, (r0.xyxx).xy, 0.f).y * 100000.f;
    // Native 51-54: reconstructed view depth is supplied by the runtime adapter.
    r0.x = r0.x;
    // 55: ge r0.x, v4.w, r0.x
    r0.x = (asfloat((uint4)((v4.wwww)>=(r0.xxxx)) * 0xffffffffu)).x;
    // 56: movc r0.xy, r0.xxxx, l(0,0,0,0), r0.zwzz
    r0.xy = ((asuint(r0.xxxx) != 0u) ? (float4(asfloat(0u),asfloat(0u),asfloat(0u),asfloat(0u))) : (r0.zwzz)).xy;
    // 57: max o0.xy, r0.xyxx, l(0.000000, 0.000000, 0.000000, 0.000000)
    output.xy = (max(r0.xyxx,float4(0.000000,0.000000,0.000000,0.000000))).xy;
    // 58: min r0.xy, r0.xyxx, l(0.000000, 0.000000, 0.000000, 0.000000)
    r0.xy = (min(r0.xyxx,float4(0.000000,0.000000,0.000000,0.000000))).xy;
    // 59: mov o0.zw, -r0.xxxy
    output.zw = (-(r0.xxxy)).zw;
    return output;
}
#endif
...
float4 ArtistNative2617(ARTIST_NATIVE_INPUT input)
{
    float4 source[3]; [unroll] for (uint i=0u; i<3u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
float4 ArtistNative2618(ARTIST_NATIVE_INPUT input)
{
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_PARTICLE_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2618Distortion(ARTIST_NATIVE_INPUT input)
{
    float4 source[14]; [unroll] for (uint i=0u; i<14u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
float4 ArtistNative2619(ARTIST_NATIVE_INPUT input)
{
    float4 source[12]; [unroll] for (uint i=0u; i<12u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_PARTICLE_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2619Distortion(ARTIST_NATIVE_INPUT input)
{
    float4 source[3]; [unroll] for (uint i=0u; i<3u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
#ifndef ARTIST_NATIVE_MODEL_ONLY
// fx_d_pa_master_01_021_ad: c366b1641ee5d94cbf8efa77a4f85613; selected map 490f9c1cd855d3f6830a8ba55a99c7f27377b1fd2911947c67f799bbde2f6ed9.
float4 ArtistNative2620(ARTIST_NATIVE_INPUT input)
{
    float4 source[8]; [unroll] for (uint i=0u; i<8u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
    ...
    // 9: sample_b_indexable(texture2d)(float,float,float,float) r1.xyz, r1.zwzz, t2.xyzw, s1, l(0.000000)
    ...
    // 22: mul r1.xy, v2.xyxx, cb0[6].yzyy
    r1.xy = ((v2.xyxx)*(source[6].yzyy)).xy;
    // 23: mad r1.xy, r0.yyyy, cb0[6].xwxx, r1.xyxx
    r1.xy = ((r0.yyyy)*(source[6].xwxx)+(r1.xyxx)).xy;
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2620Distortion(ARTIST_NATIVE_INPUT input)
{
    float4 source[3]; [unroll] for (uint i=0u; i<3u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
#ifndef ARTIST_NATIVE_MODEL_ONLY
// fx_d_pa_master_01_056_dt_ad: c5c1a96e39a1934388b0685577d2ec7e; selected map c651e9654795da922f06bc44f42731c8b71d4c327925730476c024cc556a1f2c.
float4 ArtistNative2621(ARTIST_NATIVE_INPUT input)
{
    float4 source[8]; [unroll] for (uint i=0u; i<8u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
    source[1] = g_ArtistSourceMaterialParameters[6u];
    source[2] = g_ArtistSourceMaterialParameters[5u];
    source[3].x = (g_ArtistSourceMaterialParameters[4u].yyyy).x;
    source[3].y = (g_ArtistSourceMaterialTime.xxxx).x;
    source[3].z = (g_ArtistSourceMaterialParameters[1u].wwww).x;
    source[3].w = (g_ArtistSourceMaterialParameters[0u].wwww).x;
    ...
    // 31: exp r1.x, r1.x
    r1.x = (exp2(r1.xxxx)).x;
    // 32: movc r0.y, r0.y, l(0), r1.x
    r0.y = ((asuint(r0.yyyy) != 0u) ? (float4(asfloat(0u),asfloat(0u),asfloat(0u),asfloat(0u))) : (r1.xxxx)).y;
    // 33: div r1.xy, v7.xyxx, v7.wwww
    r1.xy = ((v7.xyxx)/(v7.wwww)).xy;
    // 34: mad r1.xy, r1.xyxx, cb2[0].xyxx, cb2[0].wzww
    r1.xy = ((r1.xyxx)*(passValues[0].xyxx)+(passValues[0].wzww)).xy;
    ...
    // 42: add r1.y, -cb0[7].z, l(1.000000)
    ...
    r0.y = ((r0.yyyy)*(r1.xxxx)).y;
    // 46: mul_sat r0.y, r0.y, v3.w
    r0.y = (saturate((r0.yyyy)*(v3.wwww))).y;
    // 47: mul r0.y, r0.y, cb0[0].x
    r0.y = ((r0.yyyy)*(source[0].xxxx)).y;
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2621Distortion(ARTIST_NATIVE_INPUT input)
{
    float4 source[3]; [unroll] for (uint i=0u; i<3u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
#ifndef ARTIST_NATIVE_MODEL_ONLY
// fx_d_pa_master_01_102_tr: 2b3cb2df48b4d744aa4c26cae927ef12; selected map 41368368e92fcfbc92a6fcb4735e7512bddffc69ffd63a12c8103c2543147a17.
float4 ArtistNative2622(ARTIST_NATIVE_INPUT input)
{
    float4 source[8]; [unroll] for (uint i=0u; i<8u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_MESH_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2622Distortion(ARTIST_NATIVE_INPUT input)
{
    float4 source[3]; [unroll] for (uint i=0u; i<3u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(EFFECT_NATIVE_PARTICLE_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
#ifndef ARTIST_NATIVE_MODEL_ONLY
// fx_d_me_master_01_012_ad: 2a0b72368f6bbd4caa6f07e6123410cf; selected map c4553d3fef038a97639aded8b12c725992ad2492f31060c12a5756db79311109.
float4 ArtistNative2623(ARTIST_NATIVE_INPUT input)
{
    float4 source[12]; [unroll] for (uint i=0u; i<12u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
    ...
    // 34: add r0.xy, r2.xzxx, l(0.500000, 0.500000, 0.000000, 0.000000)
    r0.xy = ((r2.xzxx)+(float4(0.500000,0.500000,0.000000,0.000000))).xy;
    // 35: sample_b_indexable(texture2d)(float,float,float,float) r0.x, r0.xyxx, t1.zxyw, s2, l(0.000000)
    r0.x = (ArtistNativeSample2((r0.xyxx).xy, (float4(0.000000,0.000000,0.000000,0.000000)).x, false).zxyw).x;
    // 36: add r0.y, -cb0[4].x, l(1.000000)
    r0.y = ((-(source[4].xxxx))+(float4(1.000000,1.000000,1.000000,1.000000))).y;
    // 37: add r0.x, -r0.y, r0.x
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_PARTICLE_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2623Distortion(ARTIST_NATIVE_INPUT input)
{
    float4 source[3]; [unroll] for (uint i=0u; i<3u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
float4 ArtistNative2596(ARTIST_NATIVE_INPUT input)
{
    float4 source[16]; [unroll] for (uint i=0u; i<16u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#if !defined(ARTIST_NATIVE_MODEL_ONLY) && !defined(EFFECT_NATIVE_PARTICLE_CARRIER) && !defined(EFFECT_NATIVE_DECAL_CARRIER) && !defined(EFFECT_NATIVE_TRAIL_CARRIER) && !defined(EFFECT_NATIVE_SCREEN_POST_CARRIER)
float4 ArtistNative2596Distortion(ARTIST_NATIVE_INPUT input)
{
    float4 source[3]; [unroll] for (uint i=0u; i<3u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
...
#ifndef ARTIST_NATIVE_MODEL_ONLY
// Installed source-identical native reuse
float4 ArtistNative2610(ARTIST_NATIVE_INPUT input)
{
    float4 source[6]; [unroll] for (uint i=0u; i<6u; ++i) source[i]=0.f;
    source[0].x=1.f; // Project engine opacity multiplier.
    float4 output=0.f;
