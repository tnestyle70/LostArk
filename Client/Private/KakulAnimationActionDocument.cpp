	...
	const std::string_view profileId)
{
	...
	return CProjectDataRoot::Resolve(
		std::filesystem::path(L"Animation/Reference/KakulSaydon") /
		std::filesystem::path(
...
bool_t Client::CKakulAnimationActionDocument::Parse_ReferenceText(
	...
	std::string& outStatus)
{
	...
	if (!CDataJson::Parse(text, root, parseError) ||
		!Has_ExactProperties(root,
			{ "schema", "formatVersion", "profileId", "modelAssetId",
			  ...
			  "actions" }))
	{
		outStatus = "Kakul action reference JSON is malformed: " + parseError;
		return false;
	}
	...
	{
		outStatus = "Kakul action reference header is invalid.";
		return false;
	}
	...
	for (const DATA_JSON_VALUE& actionValue : actions->Get_Array())
	{
		if (!Has_ExactProperties(actionValue,
				{ "sourceActionId", "displayName", "reviewStatus",
				  "authority", "stages" }))
		{
			outStatus = "Kakul action reference row has unexpected properties.";
			return false;
		}
		...
		KAKUL_ANIMATION_ACTION_REFERENCE stagedAction;
		if (nullptr == actionId ||
		...
		{
			outStatus = "Kakul action reference row is invalid.";
			return false;
		}
		...
		for (const DATA_JSON_VALUE& stageValue : stages->Get_Array())
		{
			...
			{
				outStatus = "Kakul action stage has unexpected properties.";
				return false;
			}
			...
			KAKUL_ANIMATION_ACTION_STAGE_REFERENCE stagedStage;
			...
			{
				outStatus = "Kakul action stage is invalid.";
				return false;
			}
			...
			for (const DATA_JSON_VALUE& holdout : holdouts->Get_Array())
			{
				...
				{
					outStatus = "Kakul action holdout clip name is invalid.";
					return false;
				}
			...
			for (const DATA_JSON_VALUE& slotValue : slots->Get_Array())
			{
				...
				{
					outStatus = "Kakul action slot has unexpected properties.";
					return false;
				}
				...
				KAKUL_ANIMATION_ACTION_SLOT_REFERENCE stagedSlot;
				...
				{
					outStatus = "Kakul action slot is invalid.";
					return false;
				}
	...
	outStatus = "Parsed Kakul animation action reference.";
	return true;
}

bool_t Client::CKakulAnimationActionDocument::Parse_AuthoredText(
	const std::string_view text,
	...
	std::string& outStatus)
{
	...
	{
		outStatus = "Kakul authored action JSON is malformed: " + parseError;
		return false;
	}
	...
	{
		outStatus = "Kakul authored action header is invalid.";
		return false;
	}

	KAKUL_ANIMATION_ACTION_AUTHORED_DOCUMENT staged;
	staged.iFormatVersion = parsedVersion;
	staged.strAuthority = authority->Get_String();
	staged.strProfileId = profile->Get_String();
	staged.strReferenceRevision = revision->Get_String();
	...
	for (const DATA_JSON_VALUE& bindingValue : bindings->Get_Array())
	{
		if (!Has_ExactProperties(bindingValue,
				{ "sourceActionId", "stageId", "slotId", "runtimeClip",
				  ...
				  "mappingBasis", "authority" }))
		{
			outStatus = "Kakul authored binding has unexpected properties.";
			return false;
		}
		...
		KAKUL_ANIMATION_ACTION_BINDING stagedBinding;
		if (nullptr == actionId ||
			!Try_ParseUnsigned(*actionId,
				(std::numeric_limits<std::uint32_t>::max)(),
		...
		{
			outStatus = "Kakul authored binding is invalid.";
			return false;
		}
	...
	outDocument = std::move(staged);
	outStatus = "Parsed Kakul animation action overrides.";
	return true;
}

bool_t Client::CKakulAnimationActionDocument::Validate_Reference(
	...
	std::string& outStatus)
{
	...
	{
		outStatus = "Kakul reference profile, model, revision, or authority is invalid.";
		return false;
	}
	...
	for (const KAKUL_ANIMATION_ACTION_REFERENCE& action : document.Actions)
	{
		...
		{
			outStatus = "Kakul reference has a duplicate or invalid action.";
			return false;
		}
			...
			stageIndex < action.Stages.size(); ++stageIndex)
		{
			...
			{
				outStatus = "Kakul reference has an invalid or duplicate stage.";
				return false;
			}
			...
			for (const std::string& holdout : stage.HoldoutClipNames)
			{
				...
				{
					outStatus = "Kakul reference has an invalid holdout clip.";
					return false;
				}
			}
			...
			for (const KAKUL_ANIMATION_ACTION_SLOT_REFERENCE& slot : stage.Slots)
			{
				...
				{
					outStatus = "Kakul reference slot does not resolve to the target model.";
					return false;
				}
			}
		}
		...
		{
			outStatus = "Kakul action review status does not match its holdouts.";
			return false;
		}
	}

	outStatus = "Validated " + std::to_string(document.Actions.size()) +
		" Kakul animation action reference(s).";
	return true;
}

bool_t Client::CKakulAnimationActionDocument::Validate_Authored(
	const KAKUL_ANIMATION_ACTION_AUTHORED_DOCUMENT& document,
	...
	std::string& outStatus)
{
	...
	{
		outStatus = "Kakul authored profile, reference revision, or authority is stale.";
		return false;
	}
	...
	for (const KAKUL_ANIMATION_ACTION_BINDING& binding : document.Bindings)
	{
		...
		{
			outStatus =
				"Kakul authored override is duplicate, stale, or missing from the target model.";
			return false;
		}
	}

	outStatus = "Validated " + std::to_string(document.Bindings.size()) +
		" sparse Kakul animation action override(s).";
	return true;
}

bool_t Client::CKakulAnimationActionDocument::Load_FromPaths(
	...
	std::string& outStatus)
{
	...
	outStatus = "Loaded Kakul animation action reference and sparse overrides.";
	return true;
}

bool_t Client::CKakulAnimationActionDocument::Load(
	const std::string_view profileId,
	const std::string_view expectedModelAssetId,
	const std::vector<std::string>& availableClips,
	KAKUL_ANIMATION_ACTION_REFERENCE_DOCUMENT& outReference,
	...
	std::string& outStatus)
{
	...
	{
		outStatus = "Kakul animation action profile path is invalid.";
		return false;
	}
...
bool_t Client::CKakulAnimationActionDocument::Save_Atomic(
	const KAKUL_ANIMATION_ACTION_AUTHORED_DOCUMENT& document,
	const KAKUL_ANIMATION_ACTION_REFERENCE_DOCUMENT& reference,
	const std::string_view expectedProfileId,
	const std::string_view expectedModelAssetId,
	...
	std::string& outStatus)
{
	...
	{
		outStatus = "Kakul authored destination is invalid.";
		return false;
	}
	...
	{
		outStatus =
			"Kakul reference changed since the authored document was loaded.";
		return false;
	}
	...
	if (directoryError)
	{
		outStatus = "Could not create the Kakul authored directory.";
		return false;
	}
	...
	{
		outStatus = "Could not open the temporary Kakul authored document.";
		return false;
	}
	...
	if (!wrote || !flushed || !closed)
	{
		...
		outStatus =
			"Could not durably write the temporary Kakul authored document.";
		return false;
	}

	std::string verificationText;
	KAKUL_ANIMATION_ACTION_AUTHORED_DOCUMENT reparsed;
	std::string verificationStatus;
	if (!Read_Text(temporary, MAX_AUTHORED_BYTES, verificationText,
			verificationStatus, "Temporary Kakul authored") ||
	...
	{
		Remove_Temporary(temporary);
		outStatus = "Kakul authored temp verification failed: " +
			verificationStatus;
		return false;
	}
	...
	{
		Remove_Temporary(temporary);
		outStatus = "Kakul reference revalidation failed during authored save: " +
			verificationStatus;
		return false;
	}
	...
	{
		Remove_Temporary(temporary);
		outStatus = "Kakul reference changed during authored save.";
		return false;
	}
	...
	{
		Remove_Temporary(temporary);
		outStatus = "Could not atomically replace the Kakul authored document.";
		return false;
	}
	outStatus = "Saved " + std::to_string(document.Bindings.size()) +
		" sparse Kakul animation action override(s) to " +
		destination.string();
	return true;
}
