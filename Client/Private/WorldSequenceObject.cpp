...
HRESULT CWorldSequenceObject::Render()
{
    ...
    if (FAILED(m_Shader->Bind_Matrix("g_WorldMatrix", &m_World)) ||
        ...
        FAILED(CGameInstance::Get().Bind_Transform(m_Shader, "g_ProjMatrix", D3DTS::PROJ))) return E_FAIL;
    ...
    if (!animated)
    {
        ...
        if (FAILED(m_Shader->Bind_Matrix("g_WorldInvTransposeMatrix", &normal))) return E_FAIL;
    }
    for (uint32_t mesh = 0; mesh < m_Model->Get_NumMeshes(); ++mesh)
    {
        ...
        if (FAILED(material) || (animated && FAILED(m_Model->Bind_BoneMatrices(m_Shader, "g_BoneMatrices", mesh))) ||
            FAILED(m_Shader->Begin(animated ? 0u : CMapAssetRenderUtils::Select_Pass(profile, false))) ||
            FAILED(m_Model->Render(mesh))) return E_FAIL;
    }
    return S_OK;
}
