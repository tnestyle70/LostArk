...
bool CEffectAuthoringSequencer::Sample_Row(EFFECT_ROW& row, const float4x4_t& root)
{
    ...
    float4x4_t pivot;
    XMStoreFloat4x4(&pivot, XMMatrixTranslation(row.offset.x, row.offset.y, row.offset.z) * XMLoadFloat4x4(&root));
    ...
    if (age >= row.recordedAge)
    {
        // These authoring model previews have a fixed initial root and suppress
        // gameplay root motion. A first seek may seed that known pose at zero.
        if (row.recordedAge < 0.f && age > 0.f && !row.history->Record(0.f, pivot, false, m_Status)) return false;
        const float dx = pivot._41 - row.recordedRoot._41, dy = pivot._42 - row.recordedRoot._42, dz = pivot._43 - row.recordedRoot._43;
        const bool discontinuity = row.recordedAge >= 0.f && dx*dx + dy*dy + dz*dz > 2500.f;
        if (!row.history->Record(age, pivot, discontinuity, m_Status)) return false;
        row.recordedAge = age; row.recordedRoot = pivot;
    }
    const auto history = row.history;
    if (row.key.eOwnerKind == EFFECT_RESOURCE_OWNER_KIND::V1_DOCUMENT)
    {
        if (!row.v1 && !Stage_Row(row, pivot)) return false;
        if (!Record_V1Anchors(row, pivot, age)) return false;
    ...
    if (age >= history.recordedAge)
    {
        ...
        if (ownsSequence)
        {
            ...
            for (auto step = first; double(step) / 60.0 < double(age) - 0.000001; ++step)
            {
                ...
                if (!Resolve_Root(owner)) { success = false; break; }
        ...
        if (movedModel && !Sample_Model(ClockMs())) return false;
...
{
    if (m_Effects.empty() && (!m_UseKouku && m_SelectedSequence.empty()))
    ...
    m_Effects = std::move(staged); m_Active = true; m_Paused = false;
    m_Status = "Playing model and Effects on the same cursor."; return true;
}
...
bool CEffectAuthoringSequencer::Preview(const EFFECT_RESOURCE_KEY& key, const std::uint32_t durationMs)
{
    ...
    if (!Begin_Model()) return false;
    float4x4_t root;
    if (!Resolve_Root(root) || !Stage_Row(staged, root) || !Sample_Row(staged, root))
    { Release_Row(staged); if (!wasActive) Stop(); return false; }
    for (auto& row : m_Effects) Release_Row(row);
    if (m_Transient) Release_Row(*m_Transient);
    m_Transient = std::move(staged); m_Active = true; m_Paused = false;
...
void CEffectAuthoringSequencer::Pause(const bool paused) { m_Paused = paused; }
...
void CEffectAuthoringSequencer::Update(const float dt, const bool active)
{
    ...
    if (!m_Active) return;
    if (!m_Paused && std::isfinite(dt) && dt >= 0.f) m_ClockMs = (std::min)(double(DurationMs()), m_ClockMs + dt * 1000.0);
    if (!Sample()) { Stop(); return; }
    if (!m_Paused && m_ClockMs >= DurationMs())
    {
        if (m_Loop)
        {
            if (m_Transient) { const auto key = m_Transient->key; const auto duration = m_Transient->durationMs; m_ClockMs = 0; Preview(key, duration); }
            else { m_ClockMs = 0; Play(); }
        }
        else m_Paused = true;
    }
}

bool CEffectAuthoringSequencer::Refresh_Effects()
{
    float4x4_t root = m_WorldRoot;
    if (m_Active && !Resolve_Root(root)) return false;
    if (m_Transient)
    {
        ...
        if (!Stage_Row(staged, root) || (m_Active && !Sample_Row(staged, root))) { Release_Row(staged); return false; }
        Release_Row(*m_Transient); m_Transient = std::move(staged); return true;
    }
    auto staged = m_Effects;
    for (auto& row : staged) { row.v1.reset(); row.v2 = 0u; row.sampledAge = -1.f; row.snapshot.reset(); row.anchorHistory.reset(); if (row.history) row.history = std::make_shared<EFFECT_V2_PIVOT_HISTORY>(*row.history); }
    for (auto& row : staged)
        if (!Stage_Row(row, root) || (m_Active && !Sample_Row(row, root)))
        { for (auto& rollback : staged) Release_Row(rollback); m_Status = "Preview preserved: " + m_Status; return false; }
    for (auto& row : m_Effects) Release_Row(row);
    m_Effects = std::move(staged); m_Status = "Effect edits applied at the current cursor."; return true;
}
...
void CEffectAuthoringSequencer::Render_Sequencer()
{
    ...
    ImGui::SameLine(); if (ImGui::Button("New sequence"))
    {
        ...
        else { Stop(); m_Effects.clear(); m_SelectedEffect.clear(); m_ClockMs = 0; m_NextEffectOrdinal = 1u;
...
bool CEffectAuthoringSequencer::Save_Sequence()
{
    ...
    for (std::size_t i = 0; i < m_Effects.size(); ++i)
    {
        ...
        out << (i ? ",\n" : "\n") << "    {\"occurrenceId\": \"" << CDataJson::Escape(row.id) << "\", \"owner\": \"" << Owner_Key(row.key.eOwnerKind)
            << "\", \"effectId\": \"" << CDataJson::Escape(row.key.strStableId) << "\", \"startMs\": " << row.startMs << ", \"durationMs\": " << row.durationMs
...
bool CEffectAuthoringSequencer::Load_Sequence(const bool discard)
{
    ...
    if (!CDataJson::Parse(text, root, m_Status) || !Json_Text(root, "schema", schema) || schema != "lostark.effect-authoring-sequence" ||
        !Json_U32(root, "formatVersion", version, 1u) || version != 1u || !Json_Text(root, "sequenceId", storedId) || storedId != id)
    ...
    Stop(); m_Effects = std::move(staged); m_ClockMs = 0; m_SelectedEffect.clear(); m_UseKouku = kind != "MODEL_SEQUENCE";
