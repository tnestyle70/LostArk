...
namespace
{
	...
	bool_t ParseBosses(const DATA_JSON_VALUE& root)
	{
		...
		for (const DATA_JSON_VALUE& value : pEntries->Get_Array())
		{
			...
			const DATA_JSON_VALUE* pArmor = value.Find("armorParts");
			...
			for (const DATA_JSON_VALUE& armor : pArmor->Get_Array())
			{
				...
				if (!armor.Is_Object() || 3u != armor.Get_Object().size() ||
					!ReadRequiredString(armor, "partId", armorPart.partId) ||
					!ReadRequiredU32(armor, "stateMask", armorPart.stateMask) ||
					!ReadRequiredString(
						armor, "modelAssetId", armorPart.modelAssetId) ||
