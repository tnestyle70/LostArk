	...
	const float4x4_t& RootWorld)
{
	...
	for (uint32_t iParticle = 0u; iParticle < iSpawnCount; ++iParticle)
	{
		...
		Particle.vPosition = float3_t(
			Random_Range(State, Desc.vInitialPositionMin.x, Desc.vInitialPositionMax.x),
			Random_Range(State, Desc.vInitialPositionMin.y, Desc.vInitialPositionMax.y),
			Random_Range(State, Desc.vInitialPositionMin.z, Desc.vInitialPositionMax.z));
