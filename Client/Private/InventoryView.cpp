...
bool_t Client::CInventoryView::Try_Consume_EquipRequest(string& outItemId)
{
...
bool_t Client::CInventoryView::Get_ScreenRect(
	f32_t& fX, f32_t& fY, f32_t& fWidth, f32_t& fHeight) const
{
	...
	if (!m_pBackgroundView->Get_SlotRect("Inventory_PanelBg", fRefX, fRefY, fRefWidth, fRefHeight))
		return false;
